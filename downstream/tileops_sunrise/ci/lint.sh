#!/bin/bash
# Run TileOPs pre-commit checks with TileOPs as the logical Git repository.
set -euo pipefail
...
# TileOPs subtree into a temporary Git repository so --all-files and hook-local
# config discovery cannot cross into TileLang, TVM, or TileKernels.
if [[ "$PROJECT_ROOT" != "$GIT_ROOT" ]]; then
    project_rel="${PROJECT_ROOT#"$GIT_ROOT"/}"
    if [[ "$project_rel" == "$PROJECT_ROOT" || "$project_rel" == *".."* ]]; then
        echo "Unable to resolve TileOPs relative to Git root: $PROJECT_ROOT" >&2
        exit 1
    fi
    SCOPED_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/tileops-precommit.XXXXXX")"
    ...
    git -C "$SCOPED_ROOT" -c user.name="TileOPs CI" -c user.email="ci@example.invalid" commit --allow-empty -q -m "ci: initialize scoped lint tree"
