...
def ptpu_device() -> torch.device:
    """Session-scoped PTPU device fixture."""
    return torch.device("ptpu")
...
def stream_setup() -> None:
    ...
    stream = torch.ptpu.current_stream()
    tvm_ffi.core._env_set_current_stream(
        20,
        stream.device.index,
        stream.ptpu_stream,
    )
