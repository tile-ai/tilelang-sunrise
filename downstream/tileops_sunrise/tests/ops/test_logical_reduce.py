...
class LogicalReduceTest(_AnyWorkload, TestBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor]:
        return tuple(
            x.ptpu() if isinstance(x, torch.Tensor) else x
            for x in super().gen_inputs()
        )
...
def _make_noncontig_input(m: int, n: int, dtype: torch.dtype) -> torch.Tensor:
    ...
    if dtype == torch.bool:
        return torch.randint(0, 2, (m, n * 2), dtype=torch.bool)
...
def _make_1d_input(n: int, dtype: torch.dtype) -> torch.Tensor:
    ...
    if dtype == torch.bool:
        return torch.randint(0, 2, (n,), dtype=torch.bool)
...
def test_logical_reduce_accepts_bool(op_name: str) -> None:
    ...
    x = torch.randint(0, 2, (_M, _N), device="ptpu").bool()
    out = op(x)
...
def test_count_nonzero_returns_int64() -> None:
    ...
    x = torch.randn(_M, _N, dtype=torch.float16, device="ptpu")
    out = op(x)
