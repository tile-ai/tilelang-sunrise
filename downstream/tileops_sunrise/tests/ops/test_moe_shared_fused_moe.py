...
def test_shared_fused_moe_basic():
    ...
    shared_out, routed_out = op(
        ...
        shared_w_gate_up=shared_w_gate_up.ptpu(),
        shared_w_down=shared_w_down.ptpu(),
    )
    ...
    torch.testing.assert_close(shared_out.cpu(), shared_ref, rtol=1e-2, atol=1e-2)
    ...
    routed_ref = op_routed(hidden.ptpu(), gating.ptpu(), w_gate_up.ptpu(), w_down.ptpu())
    torch.testing.assert_close(routed_out.cpu(), routed_ref.cpu(), rtol=1e-5, atol=1e-5)
...
def test_shared_fused_moe_tp():
    ...
    partial_sum = torch.zeros(T, H, dtype=torch.float32)
    for tp_rank in range(tp_size):
        ...
        partial_sum += shared_partial.cpu().float()
...
def test_shared_fused_moe_tp_rejects_local_shards():
    ...
    with pytest.raises(ValueError, match="full weights"):
        op(hidden.ptpu(), gating.ptpu(), w_gate_up.ptpu(), w_down.ptpu(),
           shared_w_gate_up=bad_gate_up.ptpu(), shared_w_down=good_w_down.ptpu())
    ...
    with pytest.raises(ValueError, match="full weights"):
        op(hidden.ptpu(), gating.ptpu(), w_gate_up.ptpu(), w_down.ptpu(),
           shared_w_gate_up=good_gate_up.ptpu(), shared_w_down=bad_w_down.ptpu())
