...
class RopeTest(TestBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor]:
        ...
        if self.layout == "1d":
            x = torch.randn(self.seq_len, self.head_dim, device="ptpu", dtype=self.dtype)
        else:
...
def test_rope_neox_position_ids_thd(rotary_dim: int | None) -> None:
    ...
    dtype = torch.float16
    x = torch.randn(num_tokens, num_heads, head_dim, device="ptpu", dtype=dtype)
    position_ids_cpu = (
        torch.arange(num_tokens, dtype=torch.int32) * 3 + 17
    ) % max_position
    position_ids = position_ids_cpu.ptpu()
    cos, sin = _compute_freqs_cis_base(table_dim, max_position, dtype=dtype)
    ref = ref_rope_neox_position_ids(
        x.cpu(), cos, sin, position_ids_cpu.long(), rotary_dim=rotary_dim,
    )
    ...
    torch.testing.assert_close(output.cpu(), ref, atol=5e-3, rtol=1e-5)
...
def test_rope_neox_position_ids_validates_range() -> None:
    ...
    x = torch.randn(2, 1, 16, device="ptpu", dtype=torch.float16)
    with pytest.raises(ValueError, match="position_ids"):
        op(x, torch.tensor([0, 8], device="ptpu", dtype=torch.int32))
...
def test_rope_neox_position_ids_none_rotary_dim_reinfers_head_dim() -> None:
    ...
    max_position = 64
    position_ids_cpu = torch.arange(8, dtype=torch.int32)
    position_ids = position_ids_cpu.ptpu()
    op = RopeNeoxPositionIdsOp(max_position=max_position, rotary_dim=None)

    x1 = torch.randn(8, 2, 16, device="ptpu", dtype=torch.float16)
    cos1, sin1 = _compute_freqs_cis_base(16, max_position, dtype=x1.dtype)
    ref1 = ref_rope_neox_position_ids(
        x1.cpu(), cos1, sin1, position_ids_cpu.long(), rotary_dim=None,
    )
    out1 = op(x1, position_ids)
    torch.ptpu.synchronize()
    torch.testing.assert_close(out1.cpu(), ref1, atol=5e-3, rtol=1e-5)
    assert op.rotary_dim == 16

    x2 = torch.randn(8, 2, 32, device="ptpu", dtype=torch.float16)
    cos2, sin2 = _compute_freqs_cis_base(32, max_position, dtype=x2.dtype)
    ref2 = ref_rope_neox_position_ids(
        x2.cpu(), cos2, sin2, position_ids_cpu.long(), rotary_dim=None,
    )
    out2 = op(x2, position_ids)
    torch.ptpu.synchronize()
    torch.testing.assert_close(out2.cpu(), ref2, atol=5e-3, rtol=1e-5)
...
def test_rope_longrope_1d(batch: int, seq_len: int, num_heads: int,
    ...
    half = head_dim // 2
    rescale = torch.linspace(1.0, 2.0, half)
    max_pos = 16384
    orig_max_pos = 4096
...
def test_rope_rejects_wrong_rank_2d() -> None:
    ...
    op = RopeNeoxOp(layout="2d")
    x = torch.randn(4, 4, device="ptpu", dtype=torch.float16)
...
def test_rope_noncontiguous_1d_works() -> None:
    ...
    base = torch.randn(head_dim, seq_len, device="ptpu", dtype=torch.float32)
    ...
    out_c = op(x_c)
    torch.ptpu.synchronize()
    torch.testing.assert_close(out_nc.cpu(), out_c.cpu(), atol=1e-5, rtol=1e-5)
