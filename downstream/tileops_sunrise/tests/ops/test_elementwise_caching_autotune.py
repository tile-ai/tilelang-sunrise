...
class TestUnaryCaching:
    ...
    def test_unary_forward_uses_cached_fn(self):
        ...
        fn1 = k._compiled_fn
        x = torch.randn(N, dtype=torch.float16, device="ptpu")
        _ = k(x)
...
class TestFusedGatedCaching:
    ...
    def test_fused_gated_forward_uses_cached_fn(self):
        ...
        fn1 = k._compiled_fn
        x = torch.randn(32, 128, dtype=torch.float16, device="ptpu")
        _ = k(x)
        assert k._compiled_fn is fn1
...
class TestCachingCorrectness:
    ...
    def test_unary_relu_correctness(self):
        ...
        x = torch.randn(N, dtype=torch.float16, device="ptpu")
        out = k(x)
        ref = torch.relu(x.cpu().float()).to(torch.float16)
    ...
    def test_fused_gated_silu_correctness(self):
        ...
        x = torch.randn(M, 2 * Nhalf, dtype=torch.float16, device="ptpu")
        out = k(x)
        x_cpu = x.cpu()
        gate = x_cpu[:, :Nhalf].float()
        value = x_cpu[:, Nhalf:].float()
    ...
    def test_custom_leaky_relu_correctness(self):
        ...
        x = torch.randn(N, dtype=torch.float16, device="ptpu")
        out = k(x)
        ref = torch.nn.functional.leaky_relu(x.cpu().float(), 0.01).to(torch.float16)
        torch.testing.assert_close(out.cpu(), ref, atol=1e-3, rtol=1e-3)
