...
class UnaryActivationTest(TestBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor]:
        ...
        return (torch.randn(self.n_total, device="ptpu", dtype=self.dtype),)
...
def _make_activation_test(n_total, dtype, gen_fn, ref_fn, op_cls, **op_kwargs):
    ...
    if dtype == torch.float16:
        ...
        # Preserve the established TANG fp32 tolerance: the hardware tanh
        # approximation can differ from the CPU reference by about 5e-5.
...
def test_sigmoid_edge(n_total: int, dtype: torch.dtype) -> None:
    ...
    def _extreme(n, dtype):
        x = torch.zeros(n, device="ptpu", dtype=dtype)
        x[:n // 2] = -50.0
        x[n // 2:] = 50.0
        return x
...
def test_prelu(n_total: int, dtype: torch.dtype) -> None:
    ...
    shape = (1, C, H)
    x = torch.randn(shape, device="ptpu", dtype=dtype)
    weight = torch.randn(C, device="ptpu", dtype=dtype).abs() * 0.1 + 0.01
    ref = F.prelu(x.cpu().float(), weight.cpu().float()).to(dtype)
...
def test_prelu_batch_dim() -> None:
    ...
    C = 4
    x = torch.randn(shape, device="ptpu", dtype=dtype)
    weight = torch.tensor([0.1, 0.2, 0.3, 0.4], device="ptpu", dtype=dtype)
    ref = F.prelu(x.cpu(), weight.cpu())
    op = PreluFwdOp(shape=shape, dtype=dtype, num_channels=C)
    out = op(x, weight)
...
def test_prelu_rejects_mismatched_shape_same_numel() -> None:
    ...
    op = PreluFwdOp(shape=shape, dtype=dtype, num_channels=C)
    weight = torch.tensor([0.1, 0.2, 0.3, 0.4], device="ptpu", dtype=dtype)
    bad = torch.randn((2, 8, 4), device="ptpu", dtype=dtype)
