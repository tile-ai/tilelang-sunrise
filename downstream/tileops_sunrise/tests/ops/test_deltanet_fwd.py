...
def test_deltanet_fwd(
    ...
    # Device principle: run the torch reference on CPU (it uses torch.eye /
    # torch.linalg.inv / masked_select, none of which are implemented on the
    # ptpu backend), run the tilelang kernel on PTU, and compare on CPU.
    cpu_inputs = tuple(t.cpu() for t in inputs)
    ref_o = test.ref_program(*cpu_inputs)
    op_o, _S, _Aw, _Au, _w, _u = op(*inputs)
    torch.testing.assert_close(op_o.cpu(), ref_o.cpu(), **tols)
