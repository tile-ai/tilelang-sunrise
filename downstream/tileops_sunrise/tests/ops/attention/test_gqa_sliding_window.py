...
class TestGroupedQueryAttentionSlidingWindowFwdOpValidation:
    ...
    def test_dtype_mismatch_raises(self, float16_op):
        q = torch.randn(1, 64, 4, 64, dtype=torch.bfloat16).ptpu()
        k = torch.randn(1, 64, 2, 64, dtype=torch.bfloat16).ptpu()
        v = torch.randn(1, 64, 2, 64, dtype=torch.bfloat16).ptpu()
    ...
    def test_cpu_tensor_raises(self, float16_op):
        ...
        with pytest.raises(ValueError, match="ptpu"):
            float16_op.forward(q, k, v)
