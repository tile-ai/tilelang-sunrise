...
pytestmark = pytest.mark.skipif(
    not is_hopper(), reason="DeepSeek DSA decode requires Hopper SM90 WGMMA")
...
class DsaDecodeTest(_DsaDecodeTestWorkload, TestBase):
    def ref_program(self, q: torch.Tensor, kv: torch.Tensor,
        ...
        compressed_causal_mask = torch.arange(
            ...
            device="cpu").view(-1, 1) >= torch.arange(
                self.stride_kv - 1,
                sk * self.stride_kv,
                self.stride_kv,
                dtype=torch.int32,
        ...
        score = torch.einsum("bmghd,bngd->bghmn", q, k).cpu()
        sm_scale = dim_q**-0.5 if self.sm_scale is None else self.sm_scale
        score = score.masked_fill(~mask.cpu(), float("-inf")).mul(sm_scale)
        score = score.ptpu()
        p = score.softmax(dim=-1)
        p = p.view(b, g_index, h_index, -1, sq, sk)
        p = p.view(b, g, -1, sq, sk)
        o = torch.einsum("bghmn,bngd->bmghd", p.type(v.dtype), v.ptpu())
