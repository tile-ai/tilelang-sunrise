    ...
    def _func(threads=256):
        ...
        def bwd_parallel_kernel(
            ...
            with T.Kernel(num_chunks, batch, head, threads=threads) as (tid, bid, hid):
                ...
                q_c_t = T.alloc_shared([block_C, dim_k], dtype)  # A-transposed role
                q_c_b = T.alloc_shared([block_C, dim_k], dtype)  # B role
                k_c_t = T.alloc_shared([block_C, dim_k], dtype)  # B-transposed role
                w_c_t = T.alloc_shared([block_C, dim_k], dtype)  # A-transposed role
                do_c = T.alloc_shared([block_C, dim_v], dtype)
                # do_c is used both as an A operand (d_attn / dq) and as a B
                # operand (d_v_new / dh). On S2/TANG the A- and B-operand swizzle
                # layouts differ, so the B-operand role needs its own buffer to
                # avoid a LayoutInference "Get different layout" conflict.
                do_c_b = T.alloc_shared([block_C, dim_v], dtype)
                ...
                T.copy(h_c, h_c_t)
                T.gemm(do_c, h_c_t, d_q_c_frag, transpose_B=True)
                # dh from q: q^T @ do
                T.copy(q_c, q_c_t)
                T.gemm(q_c_t, do_c_b, dh_frag, transpose_A=True)
                ...
                T.gemm(w_c_t, d_v_new_c_b, dh_sub_frag, transpose_A=True)
    ...
    def _func(num_stages, threads=256):
        ...
        def dh_recurrence_bwd_kernel(
            ...
            with T.Kernel(batch, head, threads=threads) as (bid, hid):
                ...
                # dh_buf is a B operand (k @ dh_buf) and a B-transposed operand
                # (v_new @ dh_buf^T); the two roles need different swizzle layouts
                # on S2/TANG, so the transposed role gets its own buffer.
