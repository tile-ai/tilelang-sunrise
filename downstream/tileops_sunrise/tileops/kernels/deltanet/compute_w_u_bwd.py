    ...
    def _kernel_func(num_stages, threads=128):
        ...
        def compute_w_u_bwd(
            ...
            with T.Kernel(batch, head, num_chunks, threads=threads) as (bid, hid, by):
                ...
                k_s_b = T.alloc_shared([block_C, dim_k], accum_dtype)      # B role
                k_s_t = T.alloc_shared([block_C, dim_k], accum_dtype)      # B-transposed
                k_beta_s_t = T.alloc_shared([block_C, dim_k], accum_dtype)  # B-transposed
                Aw_s_b = T.alloc_shared([block_C, block_C], accum_dtype)   # B-transposed
                dP_s_b = T.alloc_shared([block_C, block_C], accum_dtype)   # B role
                dP_s_t = T.alloc_shared([block_C, block_C], accum_dtype)   # A-transposed
                # --- Fragments ---
                ...
                # dAw = dw @ k_beta^T
                T.clear(dAw_frag)
                T.copy(k_beta_s, k_beta_s_t)
                T.gemm(dw_s, k_beta_s_t, dAw_frag, transpose_B=True)
                ...
                T.clear(dA_frag)
                T.copy(Aw_s, Aw_s_b)
                T.gemm(dA_inv_s, Aw_s_b, dA_frag, transpose_B=True)
                T.copy(dA_frag, dP_s)
                dA_frag2 = T.alloc_fragment([block_C, block_C], accum_dtype)
                T.clear(dA_frag2)
                T.copy(dP_s, dP_s_b)
                T.gemm(Aw_s, dP_s_b, dA_frag2, transpose_A=True)
                ...
                T.clear(kkt_frag)
                T.copy(k_s, k_s_t)
                T.gemm(k_s, k_s_t, kkt_frag, transpose_B=True)
                for i, j in T.Parallel(block_C, block_C):
