    ...
    def _fused_func(num_stages, threads=128):
        ...
        def _fused_body(
            ...
            with T.Kernel(batch, head, seq_len // block_C, threads=threads) as (bid, hid, by):
                ...
                # Separate copy of P used as the B operand of the P@P self-gemm.
                # On the S2/TANG backend a shared buffer used as both a GEMM A
                # operand and B operand gets two incompatible swizzle layouts, so
                # the B-operand role needs its own buffer to avoid a
                # LayoutInference "Get different layout" conflict.
                P_shared_b = T.alloc_shared([block_C, block_C], dtype)
                # S_shared is a B operand inside the Neumann-series loop (P @ S)
                # but an A operand afterwards (A^{-1} @ k_beta / v_beta). Same
                # dual-role swizzle conflict on S2/TANG → keep the B-operand use
                # on its own buffer.
