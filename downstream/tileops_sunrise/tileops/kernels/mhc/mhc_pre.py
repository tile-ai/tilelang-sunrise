    ...
    def _mhc_func(block_x_b, block_C, num_stages, threads=128):
        ...
        def _get_H_0_no_split(
            ...
            with T.Kernel(batch // block_x_b, threads=threads) as (bx):
                ...
                # xsqr is accumulated across the k-loop below, so it must start
                # at zero — otherwise it carries garbage from the fragment's
                # uninitialized memory (manifests as wrong r at large c_x).
