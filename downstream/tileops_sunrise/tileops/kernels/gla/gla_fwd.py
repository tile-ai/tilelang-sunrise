        ...
        def _main(
            ...
            with T.Kernel(batch * heads * num_kv,
                ...
                for i_c in T.Pipelined(num_chunks, num_stages=num_stages):
                    ...
                    # g_last and k_adj both in shared so g_last[i_k] and
                    # k_adj[i_t,i_k] index the k-dim consistently (fragment lane
                    # distributions disagreed -> wrong decay factor per element).
                    g_last = T.alloc_shared([dim_k_part], accum_dtype)
                    ...
                    # k_adj staged in shared (then GEMM reads it as A)
                    k_adj_s = T.alloc_shared([chunk_size, dim_k_part], dtype)
                    ...
                    delta_h = T.alloc_fragment([dim_k_part, dim_v_part],
                                              accum_dtype)
                    T.fill(delta_h, 0.0)
                    T.gemm(k_adj_s, v_s, delta_h, transpose_A=True,
...
def _gla_fwd_wrapped_kernel(
    ...
    # Strict data dependency g_cumsum -> h_out -> o across three async PTPU
    # kernel launches; synchronize between stages so a later kernel does not
    # read a buffer the previous one is still writing (latent on short
    # sequences, corrupts long/many-chunk ones nondeterministically).
    _is_ptpu = q.device.type == "ptpu"
    g_cumsum = g_fn(g)
    if _is_ptpu:
