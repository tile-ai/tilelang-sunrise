    ...
    def _func(block_m, threads):
        ...
        def main(
            ...
            with T.Kernel(T.ceildiv(M, block_m), threads=threads) as pid_m:
                ...
                has_nan = T.alloc_fragment((block_m,), "float32")
                ...
                if op_kind == "l1":
                    ...
                    # inf norm: max(|x|) with NaN propagation.
                    # Detect NaN rows via T.isnan and OR them into
                    # has_nan using reduce_max over a 0/1 float array.
                    # This avoids the data race of multiple threads
                    # writing the same has_nan[i] in T.Parallel.
                    for i, j in T.Parallel(block_m, N_padded):
                        transformed[i, j] = T.abs(x_f32[i, j])
                    T.reduce_max(transformed, acc, dim=1)

                    # Reuse transformed for NaN detection.  Use T.isnan
                    # rather than ``x != x``: the latter is folded to False
                    # under the backend's fast-math / NaN-unsafe codegen.
                    for i, j in T.Parallel(block_m, N_padded):
                        transformed[i, j] = T.Select(
                            T.isnan(x_f32[i, j]),
                            T.float32(1.0),
                            T.float32(0.0),
                        )
                    T.reduce_max(transformed, has_nan, dim=1)

                    for i in T.Parallel(block_m):
                        acc[i] = T.Select(
                            has_nan[i] > T.float32(0.0),
                            T.cast(T.float32(float("nan")), "float32"),
                            acc[i],
                        )
            ...
            with T.Kernel(T.ceildiv(M, block_m), threads=threads) as pid_m:
                ...
                for t in T.Serial(num_tiles):
                    ...
                    if op_kind == "l1":
                        ...
                        # Reuse transformed for NaN detection.  Use T.isnan
                        # rather than ``x != x``: the latter is folded to
                        # False under the backend's fast-math codegen.
                        for i, j in T.Parallel(block_m, tile_n):
                            transformed[i, j] = T.Select(
                                T.isnan(x_f32[i, j]),
                                T.float32(1.0),
                        ...
                        T.reduce_max(transformed, tile_acc, dim=1)
                        for i in T.Parallel(block_m):
                            has_nan[i] = T.max(has_nan[i], tile_acc[i])
...
class VectorNormKernel(Kernel):
    ...
    def __init__(
        ...
        # Force tiling when per-thread register footprint
        # (2 × fp32 arrays of N_padded/threads elements) at 128 threads
        # would constrain the block_m search space to block_m=1 only,
        # or when tiled halves the per-thread work for more occupancy.
        _per_thread_bytes = self.N_padded * self._elem_bytes * 2 * 2  # 2 fp32 arrays
        self._needs_tiling = (
            self.N_padded > MAX_SINGLE_TILE_COLS
            or self.N_padded * self._elem_bytes > self._smem_budget
            or _per_thread_bytes > 128 * 512  # spills even at bm=1
            or self.N_padded >= 4096  # tiling halves the per-thread work
        )
        self.kernel = None
    ...
    def _tile_n_for_block_m(self, block_m: int) -> int:
        ...
        Caps tile_n to avoid register spill: the tiled kernel holds 2 fp32
        fragment arrays of shape (block_m, tile_n).  With 128 threads and
        512 B register budget, per-thread capacity is 128 floats.  Two
        fp32 arrays consume 2× tile_n/128 × 4 B each; capping at 32
        floats/array (128 B) leaves 256 B headroom for scalars, reduction
        intermediates, and compiler temporaries.
        """
        budget = self._smem_budget
        max_tile_n = self.N_padded
        if self.N_padded <= MAX_SINGLE_TILE_COLS:
            max_tile_n = compute_tile_n(
                block_m, self._elem_bytes, self.N_padded, budget=budget,
            )
            if max_tile_n == self.N_padded and self._needs_tiling:
                candidate = self.N_padded
                while candidate > 0:
                    candidate //= 2
                    per_thread_floats = 2 * block_m * candidate // 128
                    if per_thread_floats <= 32 and (
                        candidate % DEFAULT_ALIGNMENT == 0
                        or candidate < DEFAULT_ALIGNMENT
                    ):
                        return candidate

        col_budget = MAX_SINGLE_TILE_COLS * block_m * self._elem_bytes
        effective_budget = min(budget, col_budget)
        candidate = compute_tile_n(
        ...
        while candidate > 0:
            ...
            if per_thread_floats <= 32 and (
                ...
                return candidate
            candidate //= 2
        return DEFAULT_ALIGNMENT  # fallback: minimal tile
    ...
    def autotune(self, warmup: int = 10, rep: int = 10) -> None:
        ...
        x = torch.randn(self.M, self.N, dtype=self.dtype).ptpu()
