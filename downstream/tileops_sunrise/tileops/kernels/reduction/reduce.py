        ...
        def main(
            ...
            with T.Kernel(T.ceildiv(M, block_m), threads=threads) as pid_m:
                ...
                for i in T.Parallel(block_m):
                    ...
                    signed_prod_val = T.if_then_else(
                        neg_count_mod2 > 0.5, -prod_val, prod_val,
                    )
                    out_local[i] = T.cast(signed_prod_val, dtype)
...
class ReduceKernel(Kernel):
    ...
    def default_config(self) -> dict:
        if not self._needs_tiling:
            # TANG's lowering of the product kernel's consecutive row
            # reductions is not reliable when multiple rows share a block.
            if self.op_kind == "prod" and hasattr(torch, "ptpu"):
                return {"block_m": 1, "threads": 128}
    ...
    def autotune_configs(self) -> list[dict]:
        if not self._needs_tiling:
            ...
            block_ms = (
                [1]
                if self.op_kind == "prod" and hasattr(torch, "ptpu")
                else [bm for bm in [1, 2, 4, 8] if bm <= max_block_m]
            )
    ...
    def autotune(self, warmup: int = 10, rep: int = 10) -> None:
        ...
        device = torch.ptpu.current_device()
        ...
        for cfg in configs:
            ...
            # Benchmark
            start = torch.ptpu.Event(enable_timing=True)
            end = torch.ptpu.Event(enable_timing=True)
            start.record()
            for _ in range(rep):
                self.forward(x)
            end.record()
