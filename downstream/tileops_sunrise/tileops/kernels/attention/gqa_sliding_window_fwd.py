    ...
    def _gqa_sw_fwd_func(block_m: int, block_n: int, num_stages: int,
        ...
        def _gqa_sw_fwd_main(
            ...
            with T.Kernel(
                ...
                # Guard the epilogue against reading acc_o before the last
                # iteration's PV accumulation has landed. The in-loop barriers
                # only cover the shared-memory operands; without a barrier here
                # the drain block's T.gemm result is read straight into the
                # rescale below, yielding values orders of magnitude too large
                # in a few accumulator lanes of a random block (nondeterministic
                # across runs). Any non-zero barrier slot works; a bare
                # T.sync_threads() aliases the implicit barrier-0 and is elided.
                T.sync_threads(7, threads)
                for i, j in T.Parallel(block_m, dim):
                    acc_o[i, j] /= logsum[i]
