    ...
    def _func(block_M, block_N, num_split, num_stages, threads):
        ...
        def combine(
            ...
            with T.Kernel(T.ceildiv(seqlen_q, block_M), heads, batch, threads=128) as (bx, by, bz):
                ...
                T.fill(lse_max_local, -T.infinity(accum_dtype))
                for k in T.Pipelined(num_split):
                    T.copy(
                        glse[bz, by, k, bx * block_M:(bx + 1) * block_M],
                        lse_local_split,
                    )
                    for i in T.Parallel(block_M):
                        lse_max_local[i] = T.max(lse_max_local[i], lse_local_split[i])
                for k in T.Pipelined(num_split):
                    T.copy(
                        glse[bz, by, k, bx * block_M:(bx + 1) * block_M],
                        lse_local_split,
