...
def flatten_for_multidim(
    ...
    perm = kept_dims + dims
    x = x.permute(perm)
    # PTPU complex clone preserves the transposed stride, while TileLang's
    # packed ABI requires the flattened input to be row-major.
    x = (
        x.cpu().contiguous().ptpu()
        if x.is_complex() and x.is_ptpu
        else x.contiguous()
    )
