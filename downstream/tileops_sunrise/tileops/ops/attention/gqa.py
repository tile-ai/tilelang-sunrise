...
class GroupedQueryAttentionFwdOp(Op):
    ...
    def __init__(self,
        ...
        tang_kernel_cls = self.kernel_map.get("gqa_fwd_kernel", GQAFwdKernel)
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        v = v.contiguous()
        if q.is_ptpu:
            return _attention_output(self._tang_kernel(q, k, v))
        kernel = (
...
class GroupedQueryAttentionPrefillFwdOp(Op):
    ...
    def _validate_common_shapes(
        ...
        for tensor, name in (
            ...
            if tensor.device.type not in ("cuda", "ptpu"):
...
class GroupedQueryAttentionSlidingWindowFwdOp(Op):
    ...
    def forward(
        ...
        for t, name in [(q, 'q'), (k, 'k'), (v, 'v')]:
            if t.device.type != 'ptpu':
                raise ValueError(
                    f"{name} must be on a ptpu device, got {t.device}")
