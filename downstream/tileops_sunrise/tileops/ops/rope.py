...
def _base_freqs(head_dim: int, seq_len: int, base: float = 10000.0,
    ...
    compute_device = torch.device("cpu") if output_device.type == "ptpu" else output_device
    half = head_dim // 2
    freqs = 1.0 / (
        base ** (torch.arange(0, half, device=compute_device, dtype=torch.float32) / half)
    )
    t = torch.arange(seq_len, device=compute_device, dtype=torch.float32)
    angles = torch.outer(t, freqs)
    return (
        torch.cos(angles).to(dtype).to(output_device),
        torch.sin(angles).to(dtype).to(output_device),
    )
...
def _yarn_freqs(head_dim: int, seq_len: int, base: float = 10000.0,
    ...
    compute_device = torch.device("cpu") if output_device.type == "ptpu" else output_device
    half = head_dim // 2
    dim_indices = torch.arange(0, half, device=compute_device, dtype=torch.float32)
    ...
    return (
        (torch.cos(angles) * attn_factor).to(dtype).to(output_device),
        (torch.sin(angles) * attn_factor).to(dtype).to(output_device),
    )
...
def _longrope_freqs(head_dim: int, seq_len: int, base: float = 10000.0,
    ...
    if rescale_factors is not None:
        rf = rescale_factors.to(device=compute_device, dtype=torch.float32)
        divisor = rf * divisor
    ...
    angles = torch.outer(t, freqs)
    return (
        (torch.cos(angles) * scaling_factor).to(dtype).to(output_device),
        (torch.sin(angles) * scaling_factor).to(dtype).to(output_device),
    )
...
class RopeNeoxPositionIdsOp(Op):
    ...
    def _validate_and_prepare(
        ...
        self.rotary_dim = rotary_dim
        self.dtype = x.dtype
        if not (position_ids.is_cuda or position_ids.is_ptpu):
