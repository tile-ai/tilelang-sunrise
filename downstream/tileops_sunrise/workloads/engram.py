...
class EngramGateConvFwdTest(WorkloadBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor, ...]:
        H = torch.randn(self.M, self.seq_len, self.d, dtype=self.dtype).ptpu()
        k = torch.randn(self.M, self.seq_len, self.d, dtype=self.dtype).ptpu() * 0.1
        v = torch.randn(self.M, self.seq_len, self.d, dtype=self.dtype).ptpu() * 0.1
        rms_w_h = torch.ones(self.d, dtype=self.dtype, device="ptpu")
        rms_w_v = torch.ones(self.d, dtype=self.dtype, device="ptpu")
        conv_w = torch.randn(CONV_KERNEL_SIZE, self.d, dtype=self.dtype).ptpu() * 0.02
...
class EngramGateConvBwdTest(WorkloadBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor, ...]:
        ...
        rms_w_v = torch.ones(self.d, dtype=self.dtype, device="ptpu")
        conv_w = torch.randn(CONV_KERNEL_SIZE, self.d, dtype=self.dtype).ptpu() * 0.02
        dY = torch.randn(self.M, self.seq_len, self.d, dtype=self.dtype).ptpu() * 0.1
        ...
        def _rmsnorm(x, w):
            ...
            return (x_f * rrms * w_cpu.float()).ptpu(), rrms.squeeze(-1).ptpu()
...
class EngramDecodeTest(WorkloadBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor, ...]:
        e_t = torch.randn(self.batch, self.d_mem, dtype=self.dtype).ptpu() * 0.1
        h_t = torch.randn(self.batch, self.d, dtype=self.dtype).ptpu()
        # Full conv_state (max_conv_len entries)
        conv_state = torch.randn(self.batch, self.max_conv_len, self.d,
            dtype=self.dtype).ptpu() * 0.1
        W_K = torch.randn(self.d_mem, self.d, dtype=self.dtype).ptpu() * 0.02
        W_V = torch.randn(self.d_mem, self.d, dtype=self.dtype).ptpu() * 0.02
        rms_w_h = torch.ones(self.d, dtype=self.dtype, device="ptpu")
        rms_w_v = torch.ones(self.d, dtype=self.dtype, device="ptpu")
        conv_w = torch.randn(self.conv_kernel_size, self.d, dtype=self.dtype).ptpu() * 0.02
