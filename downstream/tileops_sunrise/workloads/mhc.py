...
class MHCPreTest(WorkloadBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor,
        ...
        phi = torch.randn([n_expand * c_x, n_expand * n_expand + 2 * n_expand],
                          dtype=torch.float32).ptpu()
        x = torch.randn([batch, n_expand * c_x], dtype=torch.bfloat16).ptpu()
        b = torch.randn([n_expand * n_expand + 2 * n_expand], dtype=torch.float32).ptpu()
...
class MHCPostTest(WorkloadBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        x_layer_out = torch.randn([batch, c_x], dtype=self.dtype).ptpu()
        h_post = torch.randn([batch, n_expand], dtype=torch.float32).ptpu()
        x_res = torch.randn([batch, n_expand * c_x], dtype=self.dtype).ptpu()
