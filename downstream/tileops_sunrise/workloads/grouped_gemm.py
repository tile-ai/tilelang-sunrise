...
class GroupedGemmTest(WorkloadBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor, ...]:
        ...
        if not self.transpose_a:
            ...
            A = torch.randn(batch_sum, K, dtype=dtype).to(device=device)
            if self.transpose_b:
                # NT: B is (batch_count, N, K)
                B = torch.randn(batch_count, N, K, dtype=dtype).to(device=device)
            else:
                # NN: B is (batch_count, K, N)
                B = torch.randn(batch_count, K, N, dtype=dtype).to(device=device)
        else:
            # TN / TT: A is (batch_sum, N)
            A = torch.randn(batch_sum, N, dtype=dtype).to(device=device)
            if self.transpose_b:
                # TT: B is (K, batch_sum)
                B = torch.randn(K, batch_sum, dtype=dtype).to(device=device)
            else:
                # TN: B is (batch_sum, K)
                B = torch.randn(batch_sum, K, dtype=dtype).to(device=device)
