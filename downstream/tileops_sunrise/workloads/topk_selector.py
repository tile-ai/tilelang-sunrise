...
class TopkSelectorTest(WorkloadBase):
    ...
    def gen_inputs(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        index_score = torch.randn(self.batch,
            self.seq_len,
            self.seq_len_kv,
            self.kv_group,
            dtype=self.in_dtype).ptpu()
        starts = torch.zeros(self.batch, self.seq_len, dtype=self.out_dtype, device="ptpu")
        ends = torch.ones(self.batch, self.seq_len, dtype=self.out_dtype,
                          device="ptpu") * self.seq_len_kv
