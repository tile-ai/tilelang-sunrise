...
def test_gated_deltanet_vs_fla_bwd(
    ...
    q = torch.randn(B, H, S, DK, dtype=dtype).ptpu() * 0.1
    k = torch.randn(B, H, S, DK, dtype=dtype).ptpu() * 0.1
    v = torch.randn(B, H, S, DV, dtype=dtype).ptpu() * 0.1
    g = -torch.rand(B, H, S, dtype=dtype).ptpu()
    beta = torch.rand(B, H, S, dtype=dtype).ptpu() * 0.5
    do = torch.randn(B, H, S, DV, dtype=dtype).ptpu() * 0.1
...
def test_gated_deltanet_vs_fla_fwdbwd(
    ...
    q = (torch.randn(B, H, S, DK, dtype=dtype).ptpu() * 0.1).detach().requires_grad_(True)
    k = (torch.randn(B, H, S, DK, dtype=dtype).ptpu() * 0.1).detach().requires_grad_(True)
    v = (torch.randn(B, H, S, DV, dtype=dtype).ptpu() * 0.1).detach().requires_grad_(True)
    g = (-torch.rand(B, H, S, dtype=dtype).ptpu()).detach().requires_grad_(True)
    beta = (torch.rand(B, H, S, dtype=dtype).ptpu() * 0.5).detach().requires_grad_(True)
    do = torch.randn(B, H, S, DV, dtype=dtype).ptpu() * 0.1
