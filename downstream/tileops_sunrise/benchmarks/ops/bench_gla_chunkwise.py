...
class GLATest(WorkloadBase):
    ...
    def gen_inputs(self):
        ...
        q = torch.randn(B, T, H, K, dtype=self.dtype).ptpu() * 0.1
        k = torch.randn(B, T, H, K, dtype=self.dtype).ptpu() * 0.1
        v = torch.randn(B, T, H, V, dtype=self.dtype).ptpu() * 0.1
        g = -torch.rand(B, T, H, K, dtype=self.dtype).ptpu()
        return q, k, v, g
...
def test_gla_bwd_bench(
    ...
    do = torch.randn(B, T, H, V, dtype=dtype).ptpu() * 0.1
    ...
    dht = torch.zeros(B, H, K, V, device="ptpu", dtype=torch.float32)
