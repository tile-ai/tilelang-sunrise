...
_MEAN_POOLING_BENCH_PARAMS = [
    ...
    pytest.param(
        ...
        torch.tensor([0, 2048, 4096, 6144, 8192], dtype=torch.int32, device="ptpu"),
        id="varlen-long",
    ),
    pytest.param(
        1, 1000, 64, 128, 32, torch.float16, torch.float32, True,
        torch.tensor([0, 100, 300, 600, 1000], dtype=torch.int32, device="ptpu"),
        id="varlen-tail",
    ),
]
...
def test_mean_pooling_bench(batch_size: int, seq_len: int, heads: int, dim: int, chunk_size: int,
    ...
    if offsets is not None:
        ...
        indices = torch.empty((chunks_per_bacth, 2), dtype=torch.int32, device='ptpu')
        seq_num = batch_size
        use_offsets = 0
