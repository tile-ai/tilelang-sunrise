...
class _MHCPreTestBaseline(MHCPreTest):
    ...
    def ref_program(self, phi: torch.Tensor, x: torch.Tensor, b: torch.Tensor,
        ...
        x_res_ref = torch.zeros([batch, n_expand, c_x], device="ptpu", dtype=torch.bfloat16)
        x_layer_ref = torch.zeros([batch, c_x], device="ptpu", dtype=torch.bfloat16)
