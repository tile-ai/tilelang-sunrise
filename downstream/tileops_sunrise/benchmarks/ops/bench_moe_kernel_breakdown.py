...
def bench(fn, *args, warmup=WARMUP, iters=ITERS) -> float:
    """Return median single-call latency in ms using PTPU events."""
    for _ in range(warmup):
        fn(*args)
    torch.ptpu.synchronize()
    s = torch.ptpu.Event(enable_timing=True)
    e = torch.ptpu.Event(enable_timing=True)
    s.record()
...
def profile_vllm(T, E, K, H, F, scoring_func, renormalize, iters=5):
    """Run vLLM fused_experts under torch.profiler; return sorted PrivateUse1 kernel table."""
    hidden, gating, w_gu, w_down = gen_inputs(T, E, K, H, F)
    topk_op = FusedTopKOp(
        top_k=K, scoring_func=scoring_func, renormalize=renormalize,
    )
...
def main():
    assert torch.ptpu.is_available()
    ...
    for (T, E, K, H, F, scoring, renorm) in CONFIGS:
        ...
        if _VLLM_AVAILABLE and scoring == "softmax":
            print("\n[vLLM CUTLASS top PrivateUse1 kernels]")
