...
def test_get_fused_mapping(params):
    ...
    for num_sms in generate_num_sms():
        ...
        results = func()
        results = [x.cpu() if hasattr(x, 'cpu') else x for x in results]
        pos_to_expert, pos_to_token, pos_to_token_topk, token_topk_to_pos, expert_start, expert_end, num_tokens_per_expert, num_tokens_per_expert_list = results
        ...
        if non_negative_mask.any():
            ...
            expected_indices = torch.arange(pos_to_token_topk.numel(), device='cpu')[non_negative_mask]
