...
def test_top2_sum_gate(params):
    ...
    def construct_input(scoring_func: ScoringFunc):
        while True:
            logits = torch.randn((num_tokens + num_padded_tokens, num_routed_experts), dtype=torch.float)
            bias = torch.randn(num_routed_experts, dtype=torch.float)
    ...
    def get_kwargs(use_shared_as_routed: bool, num_extra_experts: int, num_tokens: int, num_padded_tokens: int, fix_routing: bool, device):
        mask = None
        if num_padded_tokens > 0:
            mask = torch.ones(num_tokens + num_padded_tokens, dtype=torch.bool, device=device)
            mask[-num_padded_tokens:] = False

        to_physical_map = None
        logical_count = None
        fix_routing_mask = None
        unmapped_topk_idx = torch.zeros((num_tokens + num_padded_tokens, num_topk), dtype=torch.int64, device=device)

        # Test to physical map
        if use_shared_as_routed:
            assert num_shared_experts <= num_extra_experts
            to_physical_map = torch.arange(0, num_routed_experts + num_shared_experts, dtype=torch.int, device=device)
            to_physical_map = to_physical_map.view(-1, 1).expand(-1, num_extra_experts + 1).contiguous()
            logical_count = torch.ones((num_routed_experts + num_shared_experts,), dtype=torch.int, device=device)
        else:
            assert 0 <= num_extra_experts
            to_physical_map = torch.arange(0, num_routed_experts, dtype=torch.int, device=device)
            to_physical_map = to_physical_map.view(-1, 1).expand(-1, num_extra_experts + 1).contiguous()
            logical_count = torch.ones((num_routed_experts,), dtype=torch.int, device=device)

        # Test fix routing mask
        if fix_routing:
            fix_routing_mask = torch.ones((num_tokens + num_padded_tokens,), dtype=torch.bool, device=device)
            # NOTES: Use separate generator to generate the same value for same (num_tokens, num_topk)
            unmapped_topk_idx_generator = torch.Generator(device='cpu').manual_seed(42)
                ...
                for use_shared_as_routed in (False, True) if use_shared_as_routed_valid else (False,):
                    for scoring_func in ScoringFunc:
                        ...
                        for fix_routing in (False, True):
                            ...
                            kwargs = get_kwargs(use_shared_as_routed, num_extra_experts, num_tokens, num_padded_tokens, fix_routing, device='ptpu')
                            kwargs_ref = get_kwargs(use_shared_as_routed, num_extra_experts, num_tokens, num_padded_tokens, fix_routing, device='cpu')

                            topk_idx, topk_weights = tile_kernels.moe.top2_sum_gate(logits.ptpu(), bias.ptpu(), *args[2:], **kwargs)
                            torch.ptpu.synchronize()
                            topk_idx = topk_idx.cpu()
                            topk_weights = topk_weights.cpu()
                            unmapped_topk_idx = kwargs['unmapped_topk_idx'].cpu()

                            topk_idx_ref, topk_weights_ref, unmapped_topk_idx_ref = torch_top2_sum_gate(*args, **kwargs_ref)
                            ...
                            sorted_topk_idx = sorted_topk_idx.cpu()
                            sorted_unmapped_topk_idx = sorted_unmapped_topk_idx.cpu()
                            sorted_topk_weights = sorted_topk_weights.cpu()

                            unmapped_topk_idx_ref = unmapped_topk_idx_ref.cpu()
                            sorted_topk_idx_ref = sorted_topk_idx_ref.cpu()
                            sorted_unmapped_topk_idx_ref = sorted_unmapped_topk_idx_ref.cpu()
                            sorted_topk_weights_ref = sorted_topk_weights_ref.cpu()
    ...
    tp_rank = 0
    logits = torch.zeros((num_tokens + num_padded_tokens, num_routed_experts), dtype=torch.float, device='ptpu')
    bias = torch.zeros(num_routed_experts, dtype=torch.float, device='ptpu')
    args = (
        logits,
        bias,
        num_topk,
    ...
    assert torch.all(
        topk_idx[:num_tokens, :num_experts_on_rank] == torch.arange(0, num_experts_on_rank, dtype=topk_idx.dtype)
    )
...
def generate_benchmark_test_case(params):
    ...
    logits = torch.randn((num_tokens + num_padded_tokens, num_routed_experts), dtype=torch.float).ptpu()
    bias = torch.randn(num_routed_experts, dtype=torch.float).ptpu()

    # noinspection PyShadowingNames
    def get_kwargs(use_shared_as_routed, num_extra_experts, num_tokens, num_padded_tokens, fix_routing):
        mask = None
        if num_padded_tokens > 0:
            mask = torch.ones(num_tokens + num_padded_tokens, dtype=torch.bool, device='ptpu')
            mask[-num_padded_tokens:] = False

        to_physical_map = None
        logical_count = None
        fix_routing_mask = None
        unmapped_topk_idx = torch.zeros((num_tokens + num_padded_tokens, num_topk), dtype=torch.int64, device='ptpu')

        if use_shared_as_routed:
            assert num_shared_experts <= num_extra_experts
            to_physical_map = torch.arange(0, num_routed_experts + num_shared_experts, dtype=torch.int, device='ptpu')
            to_physical_map = to_physical_map.view(-1, 1).expand(-1, num_extra_experts + 1).contiguous()
            logical_count = torch.ones((num_routed_experts + num_shared_experts,), dtype=torch.int, device='ptpu')
        else:
            assert 0 <= num_extra_experts
            to_physical_map = torch.arange(0, num_routed_experts, dtype=torch.int, device='ptpu')
            to_physical_map = to_physical_map.view(-1, 1).expand(-1, num_extra_experts + 1).contiguous()
            logical_count = torch.ones((num_routed_experts,), dtype=torch.int, device='ptpu')

        if fix_routing:
            fix_routing_mask = torch.ones((num_tokens + num_padded_tokens,), dtype=torch.bool, device='ptpu')
            unmapped_topk_idx_generator = torch.Generator(device='cpu').manual_seed(42).ptpu()
            unmapped_topk_idx = torch.randint(
                0,
                num_routed_experts,
                (num_tokens + num_padded_tokens, num_topk),
