...
def generate_test_data_expand_to_fused(params):
    ...
    x = torch.randn((num_tokens, hidden), dtype=torch.bfloat16).to(device='ptpu')
...
def generate_test_params_expand(is_benchmark: bool) -> list[dict]:
    # 开关: TK_FULL_TEST=1 跑全部 hidden size，否则只跑 4096
    do_full_test = os.getenv('TK_FULL_TEST') in ['1', 'true', 'True']
    if do_full_test and not is_benchmark:
        _hidden_sizes = generate_hidden_sizes()
    else:
        _hidden_sizes = [h for h in (4096,) if h in generate_hidden_sizes()]
    return [
        {**moe, 'hidden': hidden}
...
@pytest.mark.skip(reason='PTPU/TANG does not support FP8 (e4m3)')
