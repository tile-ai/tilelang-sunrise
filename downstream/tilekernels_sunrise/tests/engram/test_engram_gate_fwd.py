...
def test_engram_gate_fwd(params):
    ...
    out_save = out_save.cpu()
    dot = dot.cpu()
    gate_score = gate_score.cpu()
    rstd_x = rstd_x.cpu()
    rstd_k = rstd_k.cpu()
    ...
    assert diff_out < 1e-6, f'out_save mismatch: {diff_out:.6e}'
    diff_dot = calc_diff(dot, dot_ref)
    assert diff_dot < 1e-6, f'dot mismatch: {diff_dot:.6e}'
    diff_gate = calc_diff(gate_score, gate_score_ref)
    assert diff_gate < 1e-6, f'gate_score mismatch: {diff_gate:.6e}'
    diff_rstd_x = calc_diff(rstd_x, rstd_x_ref)
    assert diff_rstd_x < 1e-6, f'rstd_x mismatch: {diff_rstd_x:.6e}'
    diff_rstd_k = calc_diff(rstd_k, rstd_k_ref)
    assert diff_rstd_k < 1e-6, f'rstd_k mismatch: {diff_rstd_k:.6e}'
    ...
    assert diff_out < 1e-6, f'out_no_save mismatch: {diff_out:.6e}'
