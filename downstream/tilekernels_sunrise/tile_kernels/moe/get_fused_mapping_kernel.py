...
def get_fused_mapping(
    ...
    # Allocate output
    num_sms = get_num_sms()
    pos_to_expert = torch.empty((num_expanded_tokens, ), dtype=torch.int32).ptpu()
    pos_to_token = torch.empty((num_expanded_tokens, ), dtype=torch.int32).ptpu()
    pos_to_token_topk = torch.empty((num_expanded_tokens, ), dtype=torch.int32).ptpu()
    token_topk_to_pos = torch.empty((num_tokens, num_topk), dtype=torch.int32).ptpu()
    expert_start = torch.empty((num_experts, ), dtype=torch.int32).ptpu()
    expert_end = torch.empty((num_experts, ), dtype=torch.int32).ptpu()
    num_tokens_per_expert = torch.empty((num_experts, ), dtype=torch.int32).ptpu()
    num_experts_per_sm = torch.empty((num_sms, num_experts), dtype=torch.int32).ptpu()
