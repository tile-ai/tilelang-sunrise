        ...
        with T.Kernel(hc_mult, num_persistent_blocks, threads=threads) as (pid_h, pid_b):
            ...
            for i_s in T.Serial(t_start, t_end):
                ...
                for i_b in T.Serial(1, num_blk):
                    ...
                    # T.ptx_wait_group(2)
                    # [SUNRISE] use async copy with the generic block barrier
                    T.sync_threads()
                    with T.attr('default', 'async_scope', 1):
                ...
                # T.ptx_wait_group(0)
                # [SUNRISE] replace wait group with sync threads
                T.sync_threads()
...
def engram_gate_fwd(
    ...
    # Always allocate buffers for the kernel even when save_for_backward=False;
    # passing None causes a segfault in the TVM runtime.
