...
from .ops.pre_big_fuse import mhc_pre_big_fuse, mhc_pre_big_fuse_post
...
def mhc_pre(
    ...
    *,
    ...
    + post_fwd into a single fused kernel when gradients are disabled (inference
    mode).  The fused kernel eliminates the ``layer_input`` HBM round-trip and
    returns the post-layer output directly.
    ...
    Returns:
        out: tensor of shape [..., mhc_mult, hidden_size]  (fused: already post_fwd)
        ctx: opaque tuple (post_mix, comb_mix) to pass to mhc_post for backward
    """
    if not torch.is_grad_enabled():
        # Fused kernel: pre_big_fuse + post_fwd in one pass.
        # Eliminates the layer_input HBM round-trip.
        # Returns out [..., MHC, H] instead of layer_input [..., H].
        post_mix, comb_mix, out = mhc_pre_big_fuse_post(
            residual,
            fn,
            scale,
            base,
