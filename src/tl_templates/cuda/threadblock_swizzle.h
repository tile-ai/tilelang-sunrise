...
template <int panel_width> TL_DEVICE dim3 rasterization2DRow() {
  ...
  // stride_last = gy - panel_width * panel_idx (no div, simplified from
  //   (grid_size - panel_idx * panel_size) / gx); every term in the numerator
  //   carries a factor gx, so the division is exact and cancels.
  const unsigned int stride =
      panel_idx + 1 < total_panel ? panel_width : gy - panel_width * panel_idx;
  const unsigned int col_idx =
      (panel_idx & 1) ? gx - 1 - panel_offset / stride : panel_offset / stride;
