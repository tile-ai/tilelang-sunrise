...
Stmt GuardedAssert(const PrimExpr &nullable_guard, const PrimExpr &cond,
                   const Array<StringImm> &message) {
  // The trailing no-op makes this a sequential nest: MergeNest replaces that
  // no-op with the following body while leaving the guarded assertion before
  // it.  A bare IfThenElse would instead be treated as a control-flow nest and
  // incorrectly place the kernel body inside the non-null branch.
  return SeqStmt(
      {IfThenElse(Not(nullable_guard),
                  AssertStmt(cond, StringImm("RuntimeError"), message),
                  Evaluate(0)),
       Evaluate(0)});
}
...
void BinderAddAssert(arith::Analyzer *ana, PrimExpr cond,
  ...
  if (!is_one(scond)) {
    if (nullable_guard.defined()) {
      // Keep nullable validation behind a statement-level branch.  Encoding
      // this as `nullable_guard || scond` lets simplification fold nested
      // `if_then_else` guards and LLVM may then speculate DLTensor field loads
      // from a null handle before evaluating the OR.
      asserts->emplace_back(GuardedAssert(
          nullable_guard, scond,
          Array<StringImm>({StringImm(ConstraintMessage(arg_name))})));
    } else {
      asserts->emplace_back(AssertStmt(
          scond, StringImm("RuntimeError"),
          Array<StringImm>({StringImm(ConstraintMessage(arg_name))})));
    }
  }
}
...
void TVMFFIABIBuilder::BindDLTensors(
  ...
  for (const auto &[handle, buffer] : buffer_def) {
    ...
    init_nest_.emplace_back(GuardedAssert(
        is_null, ndim_ok, Array<StringImm>({StringImm(ndim_msg.str())})));
    // type checks
    ...
    if (!data_is_subtype) {
      ...
      asserts_.emplace_back(GuardedAssert(
          is_null, cond, Array<StringImm>({StringImm(dtype_msg.str())})));
    }
    ...
    if (data_is_subtype) {
      ...
      if (buffer->strides.empty()) {
        ...
        if (!conds.empty()) {
          ...
          asserts_.emplace_back(GuardedAssert(
              Or(is_null, v_strides_is_null), all_ok,
    ...
    // by binding above)
    {
      ...
      asserts_.emplace_back(GuardedAssert(
          is_null, ok, Array<StringImm>({StringImm(device_msg.str())})));
    }
