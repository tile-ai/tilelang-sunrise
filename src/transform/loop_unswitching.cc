...
/*!
 * \brief Check whether every direct loop-var use in a condition is the same
 * floor division by a constant group size.
 */
class GroupedLoopVarUseChecker : public ExprVisitor {
public:
  explicit GroupedLoopVarUseChecker(Var loop_var)
      : loop_var_(std::move(loop_var)) {}

  void VisitExpr_(const FloorDivNode *op) final {
    const auto *group_size = op->b.as<IntImmNode>();
    if (op->a.same_as(loop_var_) && group_size && group_size->value > 1) {
      if (group_size_.has_value() && group_size_.value() != group_size->value) {
        valid_ = false;
      } else {
        group_size_ = group_size->value;
      }
      return;
    }
    ExprVisitor::VisitExpr_(op);
  ...
  void VisitExpr_(const VarNode *op) final {
    if (GetRef<Var>(op).same_as(loop_var_)) {
      valid_ = false;
    }
  }

  std::optional<int64_t> group_size() const {
    if (!valid_) {
      return std::nullopt;
    }
    return group_size_;
...
/*!
 * \brief Replace floor_div(loop_var, group_size) with the grouped outer loop
 * variable.
 */
class GroupedConditionRewriter : public ExprMutator {
public:
  GroupedConditionRewriter(Var loop_var, int64_t group_size, Var outer_var)
      : loop_var_(std::move(loop_var)), group_size_(group_size),
        outer_var_(std::move(outer_var)) {}

  PrimExpr VisitExpr_(const FloorDivNode *op) final {
    const auto *group_size = op->b.as<IntImmNode>();
    if (op->a.same_as(loop_var_) && group_size &&
        group_size->value == group_size_) {
      return outer_var_;
    }
...
/*!
 * \brief Find a condition that is invariant within fixed-size groups of loop
 * iterations.
 */
class GroupedHoistableIfFinder : public StmtVisitor {
public:
  Optional<IfThenElse> found;
  int64_t group_size{0};

  GroupedHoistableIfFinder(
      Var loop_var, const std::unordered_set<const VarNode *> &written_vars,
      const std::unordered_set<const VarNode *> *disallowed_vars)
      : loop_var_(std::move(loop_var)), written_vars_(written_vars),
        disallowed_vars_(disallowed_vars) {}
  ...
  void VisitStmt_(const IfThenElseNode *op) final {
    ...
    if (disallowed_vars_ &&
        UsesVarsThroughLetBindings(op->condition, *disallowed_vars_,
                                   &let_bindings_)) {
      return;
    }

    WrittenBufferReadChecker read_checker(written_vars_, &let_bindings_);
    read_checker(op->condition);
    if (read_checker.reads_written) {
      return;
    }

    CallNodeChecker call_checker;
    call_checker(op->condition);
    if (call_checker.has_call) {
      return;
    }

    LetVarCollector let_collector(let_bindings_);
    let_collector(op->condition);
    // A grouped condition is rewritten outside the original loop body.  A
    // Let/Bind used by that condition is defined inside the body and would be
    // out of scope after the rewrite, even when its value is loop-invariant.
    // Keep grouped unswitching conservative; the regular unswitching path has
    // separate logic for hoisting these bindings safely.
    if (!let_collector.used_let_bindings.empty()) {
      return;
    }

    GroupedLoopVarUseChecker grouped_use_checker(loop_var_);
    grouped_use_checker(op->condition);
    std::optional<int64_t> detected_group_size =
        grouped_use_checker.group_size();
    if (detected_group_size.has_value()) {
      found = GetRef<IfThenElse>(op);
      group_size = detected_group_size.value();
      return;
    }
...
private:
  Var loop_var_;
  const std::unordered_set<const VarNode *> &written_vars_;
  const std::unordered_set<const VarNode *> *disallowed_vars_;
  std::unordered_map<const VarNode *, PrimExpr> let_bindings_;
};

Optional<Stmt> TryGroupedLoopUnswitch(
    const ForNode *op, const Stmt &body,
    const std::unordered_set<const VarNode *> &written_vars,
    const std::unordered_set<const VarNode *> *disallowed_vars) {
  if (op->thread_binding.defined() ||
      (op->kind != ForKind::kSerial && op->kind != ForKind::kUnrolled)) {
    return Optional<Stmt>();
  }

  const int64_t *loop_min = as_const_int(op->min);
  const int64_t *loop_extent = as_const_int(op->extent);
  if (!loop_min || *loop_min != 0 || !loop_extent || *loop_extent <= 0) {
    return Optional<Stmt>();
  }

  GroupedHoistableIfFinder finder(op->loop_var, written_vars, disallowed_vars);
  finder(body);
  if (!finder.found.defined() || *loop_extent % finder.group_size != 0) {
    return Optional<Stmt>();
  }

  IfThenElse if_node = finder.found.value();
  CallCheckerExcludingIf call_checker;
  call_checker.excluded_condition = if_node->condition;
  call_checker(body);
  if (call_checker.has_call) {
    return Optional<Stmt>();
  }

  std::vector<std::pair<Var, PrimExpr>> no_hoisted_let_bindings;
  Stmt then_body =
      IfBranchReplacer(if_node->condition, true, no_hoisted_let_bindings)(body);
  Stmt else_body = IfBranchReplacer(if_node->condition, false,
                                    no_hoisted_let_bindings)(body);
  if (!IsSideEffectFreeStmt(else_body)) {
    return Optional<Stmt>();
  }

  DataType index_dtype = op->loop_var.dtype();
  PrimExpr zero = make_const(index_dtype, 0);
  PrimExpr group_extent = make_const(index_dtype, finder.group_size);
  PrimExpr outer_extent =
      make_const(index_dtype, *loop_extent / finder.group_size);

  Var outer_var(op->loop_var->name_hint + "_outer", index_dtype);
  Var then_inner_var(op->loop_var->name_hint + "_inner", index_dtype);
  Var else_inner_var(op->loop_var->name_hint + "_inner_else", index_dtype);

  PrimExpr then_index = outer_var * group_extent + then_inner_var;
  PrimExpr else_index = outer_var * group_extent + else_inner_var;
  then_body = Substitute(then_body, {{op->loop_var, then_index}});
  else_body = Substitute(else_body, {{op->loop_var, else_index}});

  For then_loop(then_inner_var, zero, group_extent, ForKind::kSerial,
                then_body);
  For else_loop(else_inner_var, zero, group_extent, ForKind::kSerial,
                else_body);

  PrimExpr grouped_condition = GroupedConditionRewriter(
      op->loop_var, finder.group_size, outer_var)(if_node->condition);
  Stmt grouped_body =
      IfThenElse(grouped_condition, std::move(then_loop), std::move(else_loop));
  return For(outer_var, zero, outer_extent, op->kind, std::move(grouped_body),
             Optional<IterVar>(), op->annotations);
}
...
public:
  ...
  Stmt VisitStmt_(const ForNode *op) final {
    ...
    Stmt result;
    if (!finder.found) {
      Optional<Stmt> grouped_result = TryGroupedLoopUnswitch(
          op, body, collector.written, &thread_idx_vars_in_scope_);
      if (grouped_result.defined()) {
        result = grouped_result.value();
