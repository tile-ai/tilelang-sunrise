...
struct TileLangThreadSyncPlanner : public ConstrVisitor {
  ...
  void VisitStmt_(const WhileNode *op) final {
    StmtEntry s;
    {
      ...
      s.stmt = op;
      // WhileNode has no loop_var, so Summarize's iteration-shift substitution
      // (loop_var -> loop_var + step) cannot apply and loop-carried hazards go
      // unchecked (stream-k reuses shared memory across iterations). Require a
      // barrier at the iteration boundary when the body both reads and writes
      // the sync scope — the only combination that can carry a WAR/RAW across
      // iterations.
      bool body_reads_scope = false;
      bool body_writes_scope = false;
      for (const StmtEntry &entry : scope_.back()) {
        for (const AccessEntry &acc : entry.access) {
          if (acc.scope != sync_scope_) {
            continue;
          }
          if (acc.type == kRead) {
            body_reads_scope = true;
          } else if (acc.type == kWrite) {
            body_writes_scope = true;
          }
        }
      }
      const bool needs_loop_carry_sync = body_reads_scope && body_writes_scope;

      s.access = Summarize(std::move(scope_.back()), nullptr);
      scope_.pop_back();
      if (needs_loop_carry_sync) {
        // syncs_inserted_ means "emit a barrier before this statement", so
        // marking the body head fences the previous iteration's tail against
        // this iteration's first access.
        insert_syncs(op->body.get());
      }
      if (!cond_access.empty()) {
        s.access.insert(s.access.begin(), cond_access.begin(),
                        cond_access.end());
