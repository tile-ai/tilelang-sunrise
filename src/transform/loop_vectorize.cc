  ...
  Array<PrimExpr> TransformIndices(const Array<PrimExpr> &indices,
    ...
    if (layout_map_.defined() && layout_map_.count(buffer)) {
      ...
      if (transformed_indices.size() != buffer->shape.size()) {
        // Step 1: Compute linear offset.
        //
        // Layout::Forward passes through any indices beyond its InputDim() as
        // leading dimensions (see LayoutNode::Forward), so Forward's result can
        // be wider than OutputShape: a layout mapping the trailing 2 dims to a
        // single linear index, applied to a 3-D buffer (e.g. the [stages, ...]
        // shape a pipelined shared buffer gets in lower_tile_op.cc), yields
        // 1 leading + 1 mapped = 2 indices against a 1-entry OutputShape. Give
        // those leading indices the extents they were indexing -- the leading
        // dims of buffer->shape -- and only use OutputShape for the trailing
        // indices the layout actually produced.
        ...
        const size_t num_leading =
            transformed_indices.size() - output_shape.size();
        ICHECK_LE(num_leading, buffer->shape.size())
            << "Layout forward produced " << num_leading
            << " leading indices, more than buffer rank "
            << buffer->shape.size();
        Array<PrimExpr> index_extents;
        index_extents.reserve(transformed_indices.size());
        for (size_t i = 0; i < num_leading; ++i) {
          index_extents.push_back(buffer->shape[i]);
        }
        for (const auto &e : output_shape) {
          index_extents.push_back(e);
        }
        PrimExpr linear_offset = 0;
        PrimExpr stride = 1;
        for (int i = index_extents.size() - 1; i >= 0; --i) {
          linear_offset = linear_offset + transformed_indices[i] * stride;
          stride = stride * index_extents[i];
        }
