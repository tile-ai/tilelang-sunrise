...
private:
  ...
  Stmt MakeAllreduce(const CallNode *call) {
    ...
    if (IsWarpReduction(types, group_extent, reduce_extent,
      ...
      // TANG's warp shuffle needs an explicit full mask (0xffffffff); it has
      // no __activemask intrinsic. The allreduce never relies on divergence
      // masking, so a full mask is correct.
      PrimExpr mask =
          target_->kind->name == "tang"
              ? PrimExpr(make_const(mask_dtype, 0xffffffff))
              : PrimExpr(Call(mask_dtype, builtin::tvm_warp_activemask(), {}));
      ...
      // Store the reduced value via a real BufferStore instead of remapping
      // loads to the staging buffer. A remapped load's consumer sits outside
      // this SeqStmt, so RemoveRedundantSyncs sees the trailing barrier with
      // nothing after it and drops it (RAW race). Storing here makes the read
      // a sibling of the barrier in the IR the pass sees.
      for (size_t idx = 0; idx < size; ++idx) {
        ICHECK(!load_remap_.count(buffers[idx]->data.get()));
        // The result is a single scalar stored at index 0; require the buffer
        // be scalar (extent 1) so consumers read that same index.
        ICHECK_EQ(buffers[idx]->shape.size(), 1)
            << "Allreduce buffer must be 1-D (scalar)";
        ICHECK(is_zero(buffers[idx]->shape[0]) ||
               buffers[idx]->shape[0].as<IntImmNode>()->value == 1)
            << "Allreduce buffer must have extent 1, got shape[0] = "
            << buffers[idx]->shape[0];
        BufferLoad load(shared_bufs[idx],
                        {BufIndex(make_zero(reduce_index.dtype()), group_index,
                                  reduce_extent)});
        ICHECK_EQ(load->dtype, types[idx]);
        seq.emplace_back(BufferStore(buffers[idx], load, {0}));
      }
    }
  ...
  bool IsWarpReduction(const std::vector<DataType> &types, int group_extent,
    ...
    if ((target_->kind->name != "cuda") && (target_->kind->name != "rocm") &&
        (target_->kind->name != "metal") && (target_->kind->name != "tang")) {
      return false;
    }
