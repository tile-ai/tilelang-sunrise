...
private:
  ...
  Map<String, Any> MakeAllocateAnnotations(const Var &buffer_var,
    ...
    // Preserve per-buffer byte alignment from the original AllocBuffer so it
    // survives the rewrite and reaches codegen (emitted as __align__(N)).
    if (!e.allocs.empty()) {
      auto it = e.allocs[0]->annotations.find(tirx::attr::storage_alignment);
      if (it != e.allocs[0]->annotations.end()) {
        annotations.Set(tirx::attr::storage_alignment, (*it).second);
      }
    }
    return annotations;
  }
    ...
    for (auto &kv : attach_map_) {
      ...
      for (size_t i = 0; i < vec.size(); ++i) {
        ...
        if (all_allocs_identical) {
          ...
          Map<String, Any> annotations =
              MakeAllocateAnnotations(e->alloc_var, *e);
  ...
  void NewAllocTagMerged(StorageEntry *e) {
    ...
    Map<String, Any> annotations = MakeAllocateAnnotations(e->alloc_var, *e);
