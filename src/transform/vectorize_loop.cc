...
inline int GetMaxAtomicVectorSize(DataType dtype, Target target) {
  // The local TANG atomic implementation is scalar for its validated
  // float32/int32/uint32 surface.  In particular, do not inherit CUDA's
  // fp16/bf16 packed-atomic planning: STCU has no matching Tang 0.25 atomic
  // instruction or packed fallback.
...
public:
  ...
  PrimExpr MutateAtomicAddExpr_(const CallNode *op) {
    ...
    if (vector_size > max_vec_size) {
      // The enclosing vectorized loop has already been removed by
      // LoopVectorizer.  Returning the untouched call here would leave its
      // scalar loop variable dangling.  Re-materialize the whole body as a
      // serial loop so unsupported atomics retain their scalar semantics.
      need_scalarize_ = true;
      return GetRef<PrimExpr>(op);
    }
