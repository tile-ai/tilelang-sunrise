...
public:
  static KernelInfo Collect(const GlobalVar &gvar, const PrimFunc &func) {
    ...
    // STCUV2 emits the merged shared.dyn allocation as a statically aligned
    // array.  Passing the same extent as launch-time dynamic shared memory
    // would double-count the per-block budget.
    bool tang_static_dyn_smem = collector.info_.target.defined() &&
                                TargetTangIsSTCUV2(collector.info_.target);
    // The dynamic shared memory is required to be the last launch parameter.
    if (collector.dyn_shmem_size && !tang_static_dyn_smem) {
...
public:
  ...
  PrimFunc UpdateKernelAttributes(const GlobalVar &gvar, PrimFunc func) const {
    ...
    ICHECK(!is_kernel_launch || !is_call_extern)
...
private:
  PrimExpr VisitExpr_(const CallNode *op) override {
    ...
    Map<Var, PrimExpr> param_map = [&]() {
      Map<Var, PrimExpr> param_map;
      ICHECK_EQ(node->args.size(), dev_info.params.size())
