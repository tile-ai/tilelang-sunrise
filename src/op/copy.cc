...
// Memory scope levels used to pick the copy loop domain: global < shared <
// local.fragment. The loop is driven by the side in the lower-level scope.
static int CopyScopeLevel(const Buffer &b) {
  String s = b.scope();
...
Array<IterVar> CopyNode::MakeIterVars() const {
  ...
  // (fragment)
  int src_level = CopyScopeLevel(src);
  int dst_level = CopyScopeLevel(dst);
  ...
  size_t idx = 0;
  // Fragment buffers carry a 2-D MMA layout whose forward_thread mapping only
  // stays invertible when every logical dim is represented.  Skipping a unit
  // extent here would collapse the loop to fewer dims than the fragment layout,
  // producing a degenerate forward_thread that Layout::Inverse cannot normalize
  // (e.g. a peeled M tail of size 1).  Keep unit dims for fragment copies so
  // the loop rank matches the fragment layout; the unit dim is a 1-iteration
  // loop and is otherwise harmless.
  bool keep_unit_dims = (IsFragmentBuffer(src) || IsFragmentBuffer(dst)) &&
                        src_range.size() == dst_range.size();
  for (size_t i = 0; i < base_ranges.size(); i++) {
    if (is_one(base_ranges[i]->extent) && !keep_unit_dims)
      continue;
...
Array<PrimExpr> CopyNode::MakeIndices(const Array<IterVar> &ivs,
  ...
  bool keep_unit_dims = (IsFragmentBuffer(src) || IsFragmentBuffer(dst)) &&
                        src_range.size() == dst_range.size();
  bool base_is_src = CopyScopeLevel(src) >= CopyScopeLevel(dst);
  const Array<Range> &base_ranges = base_is_src ? src_range : dst_range;
  // b walks base_ranges in lockstep with the consumed loop vars. A unit dim
  // only maps to a loop var when base_ranges has a matching unit dim at the
  // aligned position (e.g. a peeled fragment tail of size 1). Extra unit dims
  // (e.g. the double-buffer version dim that pipelining prepends to a shared
  // operand) carry no loop var and are indexed by their min.
  size_t b = 0;
  for (size_t i = 0; i < ranges.size(); i++) {
    if (is_one(ranges[i]->extent)) {
      if (keep_unit_dims && b < base_ranges.size() &&
          is_one(base_ranges[b]->extent)) {
        indices.push_back(ranges[i]->min + ivs[b]->var);
        ++b;
      } else {
  ...
  ICHECK(b == ivs.size()) << "b = " << b << ", ivs.size() = " << ivs.size()
                          << "src name = " << src->name
                          << ", dst name = " << dst->name;
  return indices;
}
...
PrimExpr CopyNode::MakePredicate(arith::Analyzer *analyzer,
  ...
  size_t b = 0;
  for (size_t i = 0; i < ranges.size(); i++) {
    bool is_unit = is_one(ranges[i]->extent);
    bool matched_unit = is_unit && keep_unit_dims && b < base_ranges.size() &&
                        is_one(base_ranges[b]->extent);
    if (is_unit && !matched_unit)
      continue; // extra unit dim (e.g. version dim) — always in-bounds
    PrimExpr cond = ranges[i]->min + ivs[b]->var < extents[i];
    if (!analyzer->CanProve(cond, arith::ProofStrength::kSymbolicBound)) {
      cond_list.push_back(cond);
    }
    cond = ranges[i]->min + ivs[b]->var >= 0;
    if (!analyzer->CanProve(cond, arith::ProofStrength::kSymbolicBound)) {
      cond_list.push_back(cond);
    }
    ++b;
