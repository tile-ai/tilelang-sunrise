...
def _compute_version() -> str:
    ...
    try:
        from importlib.metadata import PackageNotFoundError, version as _dist_version  # py3.8+

        for dist_name in ("tilelang-sunrise", "tilelang"):
            try:
    ...
    warnings.warn(
        "tilelang version metadata unavailable from 'tilelang-sunrise' or 'tilelang'; using development version.",
        RuntimeWarning,
        stacklevel=2,
    )
    return "0.0.dev0"
...
def _compute_build_sha() -> str:
    """Return the source/build identity used to isolate compiled caches."""
    build_sha = os.environ.get("TILELANG_BUILD_SHA")
    if build_sha:
        return build_sha

    try:
        from version_provider import get_git_commit_id  # type: ignore

        build_sha = get_git_commit_id()
        if build_sha:
            return build_sha
    except Exception:
        pass
...
if not env.is_light_import():
    ...
    from .engine import lower, register_cuda_postproc, register_hip_postproc, register_tang_postproc, register_c_postproc  # noqa: F401
