...
class KernelCache:
    ...
    def _generate_key(
        ...
        target_kind = target.kind.name if isinstance(target, Target) else str(target).split()[0]
        if target_kind == "tang":
            from tilelang._ptcc import compiler_identity, default_jit_options
            from tilelang.contrib.ptcc import get_ptcc_compiler

            key_data["ptcc"] = compiler_identity(get_ptcc_compiler())
            key_data["ptcc_jit_defaults"] = default_jit_options()
