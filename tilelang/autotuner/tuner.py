...
class AutoTuner:
        ...
        if captured_inputs is not None:
            ...
            def supply_prog(device, _frozen_inputs=frozen_inputs, _cached_tensors_by_device=cached_tensors_by_device):
                if not isinstance(device, (int, str, torch.device)):
                    device = _frozen_inputs[0].device if _frozen_inputs else "cpu"
    ...
    def run(
        ...
        if hasattr(self, "_prim_func_for_validation"):
            prim_func_for_validation = self._prim_func_for_validation
            if callable(prim_func_for_validation):
                prim_func_for_validation = prim_func_for_validation(**top_config)
            self._validate_input_supply_requirements(prim_func_for_validation, self.compile_args.out_idx)
        else:
            # Direct AutoTuner users do not install the decorator callback;
            # elaborate the first concrete configuration for validation.
...
class AutoTuneImpl(Generic[_P, _T]):
    ...
    def __call__(self, *args: _P.args, **kwargs: _P.kwargs) -> JITKernel | _T:
        ...
        def prim_func_for_validation(**config_arg):
            merged = dict(kwargs)
            merged.update(config_arg)
            # ``__pass_configs__`` is an internal transport key added to
            # autotune candidate configs.  It is consumed by the compile
            # path, but must not be forwarded to the user's JIT function
            # while materializing the validation PrimFunc.
            merged.pop(_PASS_CONFIGS_KEY, None)
            return self.jit_impl.get_tir(*args, **merged)

        autotuner._prim_func_for_validation = prim_func_for_validation
