...
from tilelang.jit.adapter.utils import is_cuda_target, is_hip_target, is_cpu_target, is_metal_target, is_tang_target
...
class CythonKernelAdapter(BaseKernelAdapter):
    ...
    def __init__(
        ...
        kernel_source = self.get_kernel_source(kernel_only=True)
        self.host_kernel_source = self.wrapper.wrap(kernel_source)

        if is_tang_target(self.target):
            # The TANG device source is compiled separately by ptcc; hand it to
            # the library generator so compile_lib() can embed the object.
            self.lib_generator.assign_device_source(kernel_source)
    ...
    def _process_buffer_device(self) -> dict[tirx.Var, tuple[int, torch.device]]:
        ...
        if is_cuda_target(self.target) or is_hip_target(self.target):
            ...
            device = torch.device("ptpu", torch.ptpu.current_device())
        else:
