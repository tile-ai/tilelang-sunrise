...
cdef class CythonKernelWrapper:
    ...
    cdef object _infer_output_device(self, list inputs):
        ...
        # Fallback for kernels with no tensor inputs (pure-output or all-T.ptr).
        # Must return a torch.device (not an int) so callers can read `.type`;
        # get_current_device() is backend-aware (ptpu/cuda/mps/cpu).
    ...
    cpdef forward(self, list inputs, int64_t stream = -1, bint skip_tensor_validation = False):
        ...
        # Use current stream of the target device if none specified
        if stream == -1:
            device = self._infer_output_device(inputs)
            if device.type == 'ptpu':
                stream = torch.ptpu.current_stream().ptpu_stream
