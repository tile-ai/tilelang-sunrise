...
# TANG host-side initialization code. The device object (produced by ptcc) is
# embedded as a C byte array in place of __TANG_DEVICE_CODE__, then loaded at
# runtime through the TANG driver API (taModuleLoadData/taModuleGetFunction).
TANG_INIT_FUNC = """
#include <tang.h>
#include <cstdio>
#include <cstdint>
...
static TAmodule g_module = nullptr;
static TAfunction g_func = nullptr;
static unsigned char g_device_code[] = {{
__TANG_DEVICE_CODE__
}};
...
extern "C" TL_EXPORT int init() {{
    error_buf[0] = '\\0';
    TAresult r = taModuleLoadData(&g_module, g_device_code, sizeof(g_device_code));
    if (r != TANG_SUCCESS) {{
        const char* msg = nullptr;
        taGetErrorName(r, &msg);
        snprintf(error_buf, ERROR_BUF_SIZE, "taModuleLoadData failed with error %s", msg ? msg : "unknown");
        return -1;
    }}
{0}
    return 0;
...
TANG_GET_FUNCTION = """
    r = taModuleGetFunction(&g_func, g_module, "{0}");
    if (r != TANG_SUCCESS) {{
        const char* msg = nullptr;
        taGetErrorName(r, &msg);
        snprintf(error_buf, ERROR_BUF_SIZE, "taModuleGetFunction failed with error %s", msg ? msg : "unknown");
        return -1;
    }}
"""

TANG_KERNEL_LAUNCH_FUNC_CODE = """
\t{{
\t\tTAstream tstream = (TAstream)stream;
\t\tif (tstream == nullptr) {{
\t\t\ttstream = TA_STREAM_LEGACY;
\t\t}}
\t\tvoid* args[] = {{{7}}};
\t\tTAresult r = taLaunchKernel(g_func, {0}, {1}, {2}, {3}, {4}, {5}, {6}, tstream, args, nullptr);
\t\tif (r != TANG_SUCCESS) {{
\t\t\tconst char* msg = nullptr;
\t\t\ttaGetErrorName(r, &msg);
\t\t\tsnprintf(error_buf, ERROR_BUF_SIZE, "taLaunchKernel failed with error %s", msg ? msg : "unknown");
\t\t\treturn -1;
\t\t}}
\t\ttaStreamSynchronize(tstream);
\t}}
"""
...
class TLCUDASourceWrapper:
    ...
    def create_dispatch_func(self, code, function_informations):
        ...
        for function_name, function_info in function_informations.items():
            ...
            if self.use_cooperative_groups[function_name]:
                ...
                kernel_launch_code += self.get_launch_error_check_code(function_name)
    ...
    def get_launch_error_check_code(self, function_name) -> str:
        return f'\tTILELANG_CHECK_LAST_ERROR("{function_name}");\n'
    ...
    def _build_function_informations(self, code: str) -> dict:
    ...
    def update_lib_code(self, code: str):
        ...
        function_informations = self._build_function_informations(code)
...
class TLTangSourceWrapper(TLCUDASourceWrapper):
    """Wrapper for the TANG backend.

    Unlike CUDA/HIP, the TANG device source is compiled separately by ``ptcc``
    into an ELF object, which is then embedded as a byte array in the host
    wrapper and loaded at runtime through the TANG driver API. The host wrapper
    therefore contains no device source; ``update_lib_code`` emits only the
    ``init()``/``call()`` glue, with ``__TANG_DEVICE_CODE__`` marking where the
    object bytes are substituted by ``LibraryGenerator.compile_lib``.
    """

    # g++ (the host compiler) has no ``__fp16``/``half_t``; buffer parameters are
    # only passed through to taLaunchKernel as raw pointers, so any 16-bit-wide
    # integer type is a valid stand-in for a half-typed buffer.
    _TYPE_MAP = {
        "float32": "float",
        "float16": "uint16_t",
        "bfloat16": "uint16_t",
        "float8_e4m3": "uint8_t",
        "float8_e4m3fn": "uint8_t",
        "float8_e5m2": "uint8_t",
        "float64": "double",
        "int64": "int64_t",
        "int32": "int",
    ...
    def get_stream_type(self) -> dict[str, str]:
        return {"name": "stream", "type": "void*"}

    def get_kernel_launch_code(self, function_name, grid_str, block_str, smem_str, call_args, cluster_dims):
        grid = self.grid_info[function_name]
        block = self.block_info[function_name]
        gx, gy, gz = (self._pythonic_expr(v) for v in grid)
        bx, by, bz = (self._pythonic_expr(v) for v in block)
        arg_names = [a.strip() for a in call_args.split(",") if a.strip()]
        args_array = ", ".join(f"(void*)&{a}" for a in arg_names)
        return TANG_KERNEL_LAUNCH_FUNC_CODE.format(gx, gy, gz, bx, by, bz, smem_str, args_array)

    def get_launch_error_check_code(self, function_name) -> str:
        # taLaunchKernel's return code is already checked inline by
        # get_kernel_launch_code; TILELANG_CHECK_LAST_ERROR relies on the
        # tang_runtime.h API which is unavailable in the host (driver-API) build.
        return ""

    def get_init_func(self):
        call_str = ""
        for function_name in self.function_names:
            call_str += TANG_GET_FUNCTION.format(function_name)
        return TANG_INIT_FUNC.format(call_str)
    ...
    def update_lib_code(self, code: str):
        ...
        function_informations = self._build_function_informations(code)
        host_func = self.create_dispatch_func(code, function_informations)
        # No device source here: the object is embedded in init() via
        # __TANG_DEVICE_CODE__, and the device source is compiled by ptcc in
        # LibraryGenerator.compile_lib.
        lib_code = init_func + host_func
        return lib_code
...
class TLWrapper(BaseWrapper):
    ...
    def wrap(self, c_source: str):
        ...
        elif is_tang_target(self.target):
            wrapper_class = TLTangSourceWrapper
        else:
