...
def copy(
    ...
    *,
    ...
    Args:
        ...
        drain_warps (Optional[int], keyword-only): TANG stcuv2 only. For a TMEM->global
            accumulator drain (``T.copy(C_tmem, C_global)``) this caps how many warps of
            the block cooperatively run the warp-collective ldt writeback. Defaults to all
            warps of the block. Use ``1`` to have a single warp drain (decoupling the
            writeback from the copy/MMA warps) while every thread still reaches the
            MMA-publish block barrier. Ignored for non-TMEM copies.
    ...
    # TANG stcuv2 TMEM->global drain: cap the number of writeback warps.
    if drain_warps is not None and "drain_warps" not in ann:
        assert isinstance(drain_warps, int) and drain_warps >= 1, "drain_warps must be a positive integer"
        ann["drain_warps"] = drain_warps
