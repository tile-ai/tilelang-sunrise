...
def Pipelined(
    ...
    num_stages : Optional[int]
        ...
        Leave it unset to get the default single-stage pipeline, which keeps
        ``T.copy`` on the async path that honours shared-memory swizzle layouts.
        Pass ``0`` to explicitly request a plain serial loop with no
        multi-buffering (needed by backends that cannot take the wide vectors
        the async path emits). For manually scheduled pipelines, prefer
        specifying ``order`` and ``stage`` without ``num_stages``; the pipeline
        depth is inferred as ``max(stage) + 1``.
    ...
    if group is None:
        group = []
    # -1 tells the C++ side "no depth requested", which it defaults to a
    # single-stage pipeline. An explicit 0 is passed through unchanged and keeps
    # the loop plainly serial.
