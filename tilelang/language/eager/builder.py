...
def get_type_hints(func):
    ...
    for name, value in annot.items():
        ...
        if isinstance(value, str):
            ...
            # such as ``T.Tensor((N,), T.float32)``. Evaluating them produces a
            # TIR Buffer, which is a valid TileLang annotation but not a Python
            # type accepted by typing.ForwardRef.
            try:
                evaluated = eval(value, globalns, localns)
            except Exception:
                pass
            else:
                if isinstance(evaluated, Buffer):
