...
def _equalize_attributes(actual: torch.Tensor, expected: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    ptpu_host_readback = actual.device.type == "ptpu" or expected.device.type == "ptpu"
    if ptpu_host_readback or actual.device != expected.device:
        _synchronize_ptpu_for_host_readback(actual, expected)
        actual = actual.cpu()
        expected = expected.cpu()
...
def _synchronize_ptpu_for_host_readback(*tensors: torch.Tensor) -> None:
    """Synchronize PTPU tensors immediately before a CPU comparison."""
    synchronized_devices: set[torch.device] = set()
    for tensor in tensors:
        tensor_device = getattr(tensor, "device", None)
        if getattr(tensor_device, "type", None) != "ptpu" or tensor_device in synchronized_devices:
            continue
        torch.ptpu.synchronize(tensor_device)
        synchronized_devices.add(tensor_device)
...
def torch_assert_close(
    ...
    if tensor_a.dtype.is_floating_point or tensor_a.dtype.is_complex:
        ...
        # Discrete types: isclose's rtol would misjudge large ints as close
        # (10000 vs 10001), so compare exactly.
        mismatched = tensor_a != tensor_b
        # bool has no sub/abs; cast to int64 first.
        diff = torch.abs(tensor_a.long() - tensor_b.long())
        abs_b = torch.abs(tensor_b.long())
    ...
    if num_mismatched > max_allowed_mismatched:
        raise AssertionError(
            ...
            f"Greatest relative difference: {(diff / (abs_b + 1e-12)).max().item()}"
            f"\n{base_name}: {tensor_a}"
            f"\n{ref_name}: {tensor_b}"
        )
    else:
