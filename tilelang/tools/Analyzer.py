...
class Analyzer:
    ...
    def calculate(self) -> AnalysisResult:
        ...
        bandwidth_GBps = self.device.bandwidth[1] / 1000 if self.device.bandwidth[1] > 0 else None
        ...
        if bandwidth_GBps:
            ...
            mem_time = 0
        compute_time = self.total_flops / (peak_tflops * 1e12) if peak_tflops is not None else None
        estimated_time = max(mem_time, compute_time) if peak_tflops is not None else (mem_time if bandwidth_GBps is not None else None)
