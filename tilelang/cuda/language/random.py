...
def rng_init(seed, seq=None, off=0, generator="curandStatePhilox4_32_10_t") -> tirx.PrimExpr:
    """Initialize a backend random-number generator state.

    CUDA uses cuRAND. TANG selects the corresponding Philox, MRG32k3a, or
    XORWOW implementation; its state derivation is deterministic but is not
    promised to be bit-identical to cuRAND.
...
def rng_rand_float(bit=32, dist="uniform") -> tirx.PrimExpr:
    ...
    bit : int = [32, 64]
        ...
        The TANG S2 backend supports only bit=32; bit=64 fails during codegen
        because S2 has no native double-precision RNG path.
