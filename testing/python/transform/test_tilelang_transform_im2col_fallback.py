...
def test_im2col_tang_compiles_and_matches_reference():
    func = _make_im2col_kernel()
    kernel = tilelang.JITKernel(func, target={"kind": "tang", "arch": "stcu"}, out_idx=[])
    data = torch.randn((1, 8, 8, 32), device="ptpu", dtype=torch.float16)
    weight = torch.randn((3, 3, 32, 32), device="ptpu", dtype=torch.float16)
    out = torch.empty((1, 8, 8, 32), device="ptpu", dtype=torch.float16)

    kernel(data, weight, out)
    ref = (
        torch.nn.functional.conv2d(
            data.to("cpu").permute(0, 3, 1, 2).float(),
            weight.to("cpu").permute(3, 2, 0, 1).float(),
            padding=1,
        )
        .permute(0, 2, 3, 1)
        .to(torch.float16)
