...
def _get_lower_ldgstg():
    if str(tl.env.get_default_target()).startswith("tang"):
        return tl.tang.transform.LowerLDGSTG
    return tl.cuda.transform.LowerLDGSTG
...
def _apply_passes(mod, enable_non_predicated=False, enable_predicated=False):
    ...
    target_kind = "tang" if str(tl.env.get_default_target()).startswith("tang") else "cuda"
...
def test_non_cuda_target_skip():
    """Test that the selected backend pass is skipped for an LLVM target."""
    ...
    print(mod)
    assert not _check_has_intrinsic(mod, "ldg"), "LLVM targets should NOT use ldg intrinsics"
    assert not _check_has_intrinsic(mod, "stg"), "LLVM targets should NOT use stg intrinsics"


def test_tang_async_scope_is_preserved():
    if not str(tl.env.get_default_target()).startswith("tang"):
        pytest.skip("TANG target required")
    ...
    has_async_scope = [False]

    def visitor(obj):
        if isinstance(obj, tirx.AttrStmt) and obj.attr_key == "async_scope":
            has_async_scope[0] = True

    tirx.stmt_functor.post_order_visit(mod["main"].body, visitor)
    assert has_async_scope[0]
