...
def test_pipeline_planning_before_after_explicit_plan_drops_replayable_bind_slot():
    ...
    def after(A: T.Tensor((64,), T.float16), C: T.Tensor((64,), T.float16)):
        ...
        for i in T.serial(
            4,
            annotations={
                # `before` passes only order/stage, leaving num_stages unset,
                # which PipelinedFor defaults to a single-stage pipeline. Hence
                # the annotation here. (An explicit num_stages=0 would instead
                # stay serial and emit no annotation.)
                "num_stages": T.int32(1),
                "software_pipeline_async_producer_groups": [
                    T.int32(0),
                    T.int32(-1),
                ],
...
def test_pipeline_planning_copy_then_read_modify_write_is_allowed():
    # A copy stage (global→shared X) followed by a compute stage that both
    # reads and writes X is a read-modify-write on the same double-buffer slot.
    # AnalyzeCopyLastUse must exempt it from the multi-writer check (5c9695f3)
    # instead of flagging the compute stage's write as a second writer to X.
    @T.prim_func
    def before(A: T.Tensor((64,), T.float32)):
        with T.Kernel(1, threads=16):
            X_shared = T.alloc_shared((16,), T.float32)
            for i in T.Pipelined(4, num_stages=2):
                T.copy(A[i * 16], X_shared)
                for j in T.Parallel(16):
                    X_shared[j] = X_shared[j] * 0.5

    # Reaching here without raising is the check: the read-modify-write
    # compute stage must not trip the multi-writer LOG(FATAL).
    _run_pipeline_planning(before, sm80_target)


def test_pipeline_planning_copy_then_write_only_overlap_is_rejected():
    # The negative counterpart: a copy stage (global→shared X) followed by a
    # compute stage that only WRITES X (no read) is a true second writer to the
    # same double-buffer slot.  Pipeline planning cannot schedule two writers to
    # overlapping regions, so AnalyzeCopyLastUse must raise.
    @T.prim_func
    def before(A: T.Tensor((64,), T.float32)):
        with T.Kernel(1, threads=16):
            X_shared = T.alloc_shared((16,), T.float32)
    ...
    with pytest.raises(tvm.TVMError, match="Multiple writes"):
        _run_pipeline_planning(before, sm80_target)
