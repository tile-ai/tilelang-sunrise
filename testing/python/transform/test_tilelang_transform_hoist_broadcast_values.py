...
_IS_TANG_TARGET = str(tilelang.env.get_default_target()).startswith("tang")
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            torch.float8_e4m3fn,
            marks=pytest.mark.skipif(_IS_TANG_TARGET, reason="STCU does not define FP8 e4 codegen types"),
        ),
        pytest.param(
            torch.float8_e5m2,
            marks=pytest.mark.skipif(_IS_TANG_TARGET, reason="STCU does not define FP8 e5 codegen types"),
        ),
        pytest.param(
            _FLOAT8_E8M0FNU,
            marks=pytest.mark.skipif(
                _FLOAT8_E8M0FNU is None or _IS_TANG_TARGET,
                reason="torch or the STCU backend lacks float8_e8m0fnu support",
            ),
            id="float8_e8m0fnu",
        ),
        torch.float16,
...
def test_hoist_broadcast(dtype):
    ...
    a = torch.empty((32,), device=device, dtype=dtype)
    b = torch.empty((16,), device=device, dtype=dtype)
    c = torch.empty((8,), device=device, dtype=dtype)
    d = torch.empty((4,), device=device, dtype=dtype)
    e = torch.empty((2,), device=device, dtype=dtype)
    kernel(a, b, c, d, e)
