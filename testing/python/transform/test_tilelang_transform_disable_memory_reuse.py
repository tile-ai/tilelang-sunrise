...
def test_disable_reuse_data_integrity():
    ...
    c = torch.randn(N, device=device, dtype=torch.float16)
    d = torch.empty(N, device=device, dtype=torch.float16)

    kernel(a, b, c, d)
    ref = a + b + c
    if device.type == "ptpu":
        torch.ptpu.synchronize()
    torch.testing.assert_close(d.cpu(), ref.cpu(), rtol=1e-3, atol=1e-3)
...
def test_disable_reuse_no_overlap():
    ...
    a = torch.randn(N, device=device, dtype=torch.float16)
    b = torch.randn(N, device=device, dtype=torch.float16)
    a_out = torch.empty(N, device=device, dtype=torch.float16)
    b_out = torch.empty(N, device=device, dtype=torch.float16)

    # Correctness: both should produce correct results
    kernel_no_reuse(a, b, a_out, b_out)
    if device.type == "ptpu":
        torch.ptpu.synchronize()
    torch.testing.assert_close(a_out.cpu(), a.cpu(), rtol=0, atol=0)
    torch.testing.assert_close(b_out.cpu(), b.cpu(), rtol=0, atol=0)

    a_out2 = torch.empty(N, device=device, dtype=torch.float16)
    b_out2 = torch.empty(N, device=device, dtype=torch.float16)
    kernel_reuse(a, b, a_out2, b_out2)
    if device.type == "ptpu":
        torch.ptpu.synchronize()
    torch.testing.assert_close(a_out2.cpu(), a.cpu(), rtol=0, atol=0)
    torch.testing.assert_close(b_out2.cpu(), b.cpu(), rtol=0, atol=0)
