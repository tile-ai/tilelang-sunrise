...
def test_nullable_shared_shape():
    ...
    # Create test tensors
    tensor_a = torch.randn((m,), device=get_current_device(), dtype=torch.float32).to(torch.int32)
    tensor_b = torch.randn((m,), device=get_current_device(), dtype=torch.float32).to(torch.int32)
    tensor_c = torch.randn((m,), device=get_current_device(), dtype=torch.float32).to(torch.int32)
...
def test_nullable_single_source_shape():
    ...
    x = torch.randn((m,), device=get_current_device(), dtype=torch.float32).to(torch.int32)
    kernel(x)
...
def test_nullable_shared_shape_with_no_source_buffers_but_other_tensor_present():
    ...
    out = torch.randn((1,), device=device, dtype=torch.float16)
    out_ref = out.clone()
    ...
    torch.testing.assert_close(out.cpu(), out_ref.cpu())
