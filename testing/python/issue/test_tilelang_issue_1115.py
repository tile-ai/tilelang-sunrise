...
def test_int64_address():
    ...
    D = 2
    S = 10
    cache = torch.rand((S, D), device=device, dtype=torch.float32)
    value = torch.rand((S, D), device=device, dtype=torch.float32)
    pos_int64 = torch.arange(S, device=device, dtype=torch.int64)
    pos_int32 = torch.arange(S, device=device, dtype=torch.int32)
    kernel_int64 = set_cache_kernel(S, D, "int64")
    kernel_int32 = set_cache_kernel(S, D, T.int32)
    kernel_int64(pos_int64, value, cache)
    torch.testing.assert_close(cache.cpu(), value.cpu())
