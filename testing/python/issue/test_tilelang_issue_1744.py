...
def test():
    ...
    test_S = torch.randn((8), dtype=torch.bfloat16, device=device)
    test_D = torch.zeros((4, 64), dtype=torch.bfloat16, device=device)
    _buggy_kernel(test_S, test_D)
    ref_D = test_S[:4].view(4, 1).repeat(1, 64)
    torch.testing.assert_close(test_D.cpu(), ref_D.cpu())
