...
def test_loop_layout_identity():
    ...
    if is_ptpu_available():
        # S2 deliberately caps vector planning at 32 bits, so the same identity
        # mapping is emitted as scalar loads/stores rather than CUDA float4.
        assert "B[((((int)threadIdx.x) * 32) + i)] = A[((((int)threadIdx.x) * 32) + i)];" in code
    else:
