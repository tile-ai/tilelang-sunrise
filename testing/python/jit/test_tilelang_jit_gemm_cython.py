...
def run_gemm_jit_kernel(
    ...
    A = torch.randn(M, K, dtype=in_dtype).ptpu()
    B = torch.randn(K, N, dtype=in_dtype).ptpu()
    ...
    tilelang.testing.torch_assert_close(C.cpu(), ref_C, atol=1e-2, rtol=1e-2, max_mismatched_ratio=0.05)
...
def run_cython_kernel_multi_stream(
    ...
    tensor_a = torch.randn(M, K, dtype=in_dtype).ptpu()
    tensor_b = torch.randn(K, N, dtype=in_dtype).ptpu()

    if trans_A:
        tensor_a = tensor_a.T
    if trans_B:
        tensor_b = tensor_b.T
    tensor_c = torch.randn(M, N, dtype=out_dtype).ptpu()

    num_streams = 4
    for _ in range(num_streams):
        stream = torch.ptpu.Stream()
        with torch.ptpu.stream(stream):
...
def run_cython_dynamic_shape(
    ...
    tilelang.testing.torch_assert_close(tensor_c.cpu(), tensor_ref_c, atol=1e-2, rtol=1e-2, max_mismatched_ratio=0.05)
...
def run_matmul_int_variable(M, N, K, block_M, block_N, block_K, trans_A, trans_B, in_dtype, out_dtype, dtypeAccum, num_stages, threads):
    ...
    tensor_ref_c = torch.matmul(tensor_a.cpu(), tensor_b.cpu()).to(out_dtype) + 1
    tilelang.testing.torch_assert_close(tensor_c.cpu(), tensor_ref_c, rtol=1e-2, atol=1e-2)
...
def run_matmul_float_variable(M, N, K, block_M, block_N, block_K, trans_A, trans_B, in_dtype, out_dtype, dtypeAccum, num_stages, threads):
    ...
    tensor_ref_c = torch.matmul(tensor_a.cpu(), tensor_b.cpu()).to(out_dtype) + 1.0
    tilelang.testing.torch_assert_close(tensor_c.cpu(), tensor_ref_c, rtol=1e-2, atol=1e-2)
