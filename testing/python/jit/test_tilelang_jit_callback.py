...
def run_gemm(
    ...
    def tilelang_callback_tang_postproc(code, _):
        code = f"// {stramp}\n" + code
        return code

    register_tang_postproc(tilelang_callback_tang_postproc)
...
def run_gemm_jit_kernel(
    ...
    A = torch.randn(M, K, dtype=torch.__getattribute__(in_dtype)).to(get_current_device())
    B = torch.randn(K, N, dtype=torch.__getattribute__(in_dtype)).to(get_current_device())
