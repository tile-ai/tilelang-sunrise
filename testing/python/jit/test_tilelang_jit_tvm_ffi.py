...
def run_gemm_jit_kernel(
    ...
    A = torch.randn(M, K, dtype=in_dtype, device=device)
    B = torch.randn(K, N, dtype=in_dtype, device=device)
    ...
    def ref_program(A, B):
        ...
        C = torch.matmul(A.cpu().to(torch.float), B.cpu().to(torch.float))
        C = C.to(out_dtype)
        return C
    ...
    tilelang.testing.torch_assert_close(C.cpu(), ref_C.cpu(), atol=1e-2, rtol=1e-2, max_mismatched_ratio=0.05)
...
def run_tvm_ffi_kernel_multi_stream(
    ...
    out_dtype = T.dtype(out_dtype).as_torch()
    device = get_current_device()
    tensor_a = torch.randn(M, K, dtype=in_dtype, device=device)
    tensor_b = torch.randn(K, N, dtype=in_dtype, device=device)

    if trans_A:
        tensor_a = tensor_a.T
    if trans_B:
        tensor_b = tensor_b.T
    tensor_c = torch.randn(M, N, dtype=out_dtype, device=device)

    num_streams = 4
    accelerator = torch.ptpu if str(tilelang.env.get_default_target()).startswith("tang") else torch.cuda
...
def run_tvm_ffi_dynamic_shape(
    ...
    if device.type == "ptpu":
        torch.ptpu.synchronize(device)
    tensor_ref_c = torch.matmul(tensor_a.cpu().to(torch.float), tensor_b.cpu().to(torch.float)).to(out_dtype)
    tilelang.testing.torch_assert_close(tensor_c.cpu(), tensor_ref_c.cpu(), atol=1e-2, rtol=1e-2, max_mismatched_ratio=0.05)


@pytest.mark.parametrize(
    ("dynamic_m", "dynamic_n", "dynamic_k"),
    [(True, False, False), (True, True, False), (True, True, True)],
    ids=["dynamic-m", "dynamic-mn", "dynamic-mnk"],
)
def test_tvm_ffi_dynamic_shape(dynamic_m, dynamic_n, dynamic_k):
    M = T.dynamic("m") if dynamic_m else 1024
    N = T.dynamic("n") if dynamic_n else 1024
    K = T.dynamic("k") if dynamic_k else 768
    run_tvm_ffi_dynamic_shape(M, N, K, False, False, T.float16, T.float16, T.float32, 128, 256, 32, 2)
...
def test_tvm_ffi_l2_persistent_map():
    ...
    if not str(tilelang.env.get_default_target()).startswith("tang"):
    ...
    if device.type == "ptpu":
        torch.ptpu.synchronize(device)
    tilelang.testing.torch_assert_close(c.cpu(), ref_c.cpu(), atol=1e-5, rtol=1e-5)
