...
def _test_device():
    ...
    return "ptpu" if target.kind.name == "tang" else "cuda"
...
def _call_and_count_new_entries(fn, M, N, K, threads=None):
    ...
    a = torch.randn(M, K, dtype=torch.float16, device=_test_device())
    b = torch.randn(N, K, dtype=torch.float16, device=_test_device())
...
def test_do_not_specialize_kwargs_and_args():
    ...
    a = torch.randn(512, 256, dtype=torch.float16, device=_test_device())
    b = torch.randn(256, 256, dtype=torch.float16, device=_test_device())
