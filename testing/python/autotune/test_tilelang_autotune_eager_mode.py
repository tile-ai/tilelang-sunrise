...
def run_autotune(M, N, K, M_value=None, N_value=None, K_value=None, return_kernel=False):
    ...
    a_cpu = torch.randn(actual_M, actual_K, dtype=torch.float16)
    b_cpu = torch.randn(actual_N, actual_K, dtype=torch.float16)
    a = a_cpu.to(device)
    b = b_cpu.to(device)
    ...
    ref_c = ref_program(a_cpu, b_cpu)
    torch.testing.assert_close(c.to("cpu"), ref_c, rtol=1e-2, atol=1e-2)
...
def test_autotune_matmul():
    ...
    This test constructs random accelerator tensors, autotunes the JIT-compiled block-level matrix-multiplication kernel,
