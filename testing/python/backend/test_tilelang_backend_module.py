...
def test_builtin_backend_modules_are_explicit():
    expected = {
        ...
        "tang": (("tang",), ["simulator", "tvm_ffi", "cython"]),
    }
...
def test_backend_owns_compile_callbacks():
    ...
    assert "tilelang_callback_tang_compile" in get_backend("tang").callbacks
    ...
    assert tvm.ffi.get_global_func("tilelang_callback_tang_compile")
