...
def evaluate_gemv_simt(
    ...
    kernel = JITKernel(program, target="tang")
    ...
    if in_dtype in {torch.int8, torch.int32}:
        A = torch.randint(-128, 128, (M, K), dtype=torch.int8).to(in_dtype).ptpu()
        B = torch.randint(-128, 128, (N, K), dtype=torch.int8).to(in_dtype).ptpu()
        Bias = torch.randint(-128, 128, (N,), dtype=torch.int32).to(accum_dtype).ptpu()
    elif in_dtype in {torch.float8_e4m3fn, torch.float8_e5m2}:
        A = torch.randn(M, K).to(in_dtype).ptpu()
        B = torch.randn(N, K).to(in_dtype).ptpu()
        Bias = torch.randn(N).to(accum_dtype).ptpu()
    else:
        A = torch.randn(M, K).to(in_dtype).ptpu() - 0.5
        B = torch.randn(N, K).to(in_dtype).ptpu() - 0.5
        Bias = torch.randn(N).to(accum_dtype).ptpu() - 0.5

    C = torch.zeros(M, N).to(out_dtype).ptpu()
    ...
    ref_c = torch.mm(A.cpu().to(torch.float32), B.cpu().T.to(torch.float32))
    if with_bias:
        ref_c += Bias.cpu().to(torch.float32)
    ref_c = ref_c.to(out_dtype).ptpu()
