...
def tl_matmul_simt(
    ...
    dp4a_size = 4
    use_dp4a = not target_is_tang(determine_target(return_object=True)) and in_dtype == T.int8 and accum_dtype == T.int32
...
def assert_tl_matmul_correctness(M, N, K, in_dtype, out_dtype, accum_dtype):
    ...
    if in_dtype == T.int8:
        A = torch.randint(-128, 127, (M, K), device=get_current_device(), dtype=torch.int8)
        B = torch.randint(-128, 127, (N, K), device=get_current_device(), dtype=torch.int8)
    else:
        A = torch.rand(M, K, device=get_current_device(), dtype=getattr(torch, in_dtype))
        B = torch.rand(N, K, device=get_current_device(), dtype=getattr(torch, in_dtype))
    ...
    ref_c = torch.matmul(A.cpu().to(torch.float32), B.T.cpu().to(torch.float32)).to(getattr(torch, out_dtype))
    print(C.cpu())
    print(ref_c)
