...
def test_nested_parallels():
    ...
    _assert_close_on_host(result1, data + 1.0)
    _assert_close_on_host(result2, data + 1.0)
...
def test_mixed_sp():
    ...
    _assert_close_on_host(result3, data + 1.0)
...
def test_tiled_op_with_parallel():
    ...
    if data.device.type == "ptpu":
        torch.ptpu.synchronize()
    data_host = data.cpu()
    _assert_close_on_host(result1, torch.relu(data_host))
    kernel2 = customize_op_with_parallel(length=256, block=16)
    result2 = kernel2(data)
    _assert_close_on_host(result2, data_host + 1)
