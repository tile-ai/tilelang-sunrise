...
class PostProcCounter:
    ...
    def register_callback(self, backend: str):
        ...
        if get_current_device().type == "ptpu":
            global_func = "tilelang_callback_tang_postproc"
        else:
...
def setup_module_env():
    ...
    tvm_ffi.register_global_func("tilelang_callback_tang_postproc", f=lambda code, _: code, override=True)
...
def test_disk_cache_with_postproc(clean_cache_env, backend):
    ...
    PTPU runs the tvm_ffi and cython backends; CUDA runs the CUDA backend matrix.
    ...
    if device.type == "ptpu":
        c1, c2, ref = c1.cpu(), c2.cpu(), ref.cpu()
