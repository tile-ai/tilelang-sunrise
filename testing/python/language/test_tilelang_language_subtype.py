...
def test_subtype_basic_shape_binding():
    ...
    t = torch.randint(0, 256, (4, 8), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_noncontiguous_tensor():
    ...
    t_large = torch.randint(0, 256, (8, 8), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_different_m_values(m):
    ...
    t = torch.randint(0, 256, (m, 8), dtype=torch.uint8, device=get_current_device())
    basic_shape_kernel(t)
...
def test_subtype_different_strides(stride_multiplier):
    ...
    t_large = torch.randint(0, 256, (4 * stride_multiplier, 8), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_symbolic_last_dim():
    ...
    t = torch.randint(0, 256, (4, 16), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_symbolic_last_dim_various_sizes(n_runtime):
    ...
    t = torch.randint(0, 256, (4, n_runtime), dtype=torch.uint8, device=get_current_device())
    symbolic_last_dim_kernel(t)
...
def test_subtype_symbolic_last_dim_strided():
    ...
    t_large = torch.randint(0, 256, (8, 16), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_shared_symbolic(m):
    ...
    """
    x = torch.randint(0, 256, (m, 8), dtype=torch.uint8, device=get_current_device())
    y = torch.randint(0, 256, (m * 4, 8), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_shared_symbolic_strided(m):
    ...
    x = torch.randint(0, 256, (m, 8), dtype=torch.uint8, device=get_current_device())
    y = torch.randint(0, 256, (m * 2, 8), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_shared_symbolic_strided_noncontig():
    ...
    x_large = torch.randint(0, 256, (8, 8), dtype=torch.uint8, device=get_current_device())
    y_large = torch.randint(0, 256, (16, 8), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_complex_expressions():
    ...
    m, n = 4, 16
    x = torch.randint(0, 256, (m, n), dtype=torch.uint8, device=get_current_device())
    y = torch.randint(0, 256, (m * 2, n // 2), dtype=torch.uint8, device=get_current_device())
...
def test_subtype_fp4_dynamic_stride_store(block_size):
    ...
    jit_kw = dict(out_idx=None, pass_configs={"tl.disable_data_race_check": True})

    def make_buf():
        row = fp4_bytes + padding
        back = torch.zeros(num_blocks, block_size * row, dtype=torch.uint8, device=device)
        fp4 = back[:, : block_size * fp4_bytes].view(num_blocks, block_size, fp4_bytes).view(torch.int8)
        return back, fp4

    torch.manual_seed(0)
    src = torch.randint(0, 256, (n, 64), dtype=torch.uint8, device=device).view(torch.int8)
    slots = torch.randperm(num_blocks * block_size, dtype=torch.int32)[:n].to(device)
...
def test_subtype_fp4_scalar_store_codegen(n):
    ...
    src = torch.randint(0, 256, (n, 64), dtype=torch.uint8, device=device).view(torch.int8)
    dst = torch.zeros(n, 64, dtype=torch.uint8, device=device).view(torch.int8)
    perm = torch.randperm(n, dtype=torch.int32).to(device)

    scatter_kern(src, dst, perm)

    # Invert the permutation and check src[inv[j]] == dst[j] at byte level
    inv = torch.empty_like(perm)
    inv[perm] = torch.arange(n, dtype=torch.int32).to(device)
