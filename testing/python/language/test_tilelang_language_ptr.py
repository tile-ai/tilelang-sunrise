...
def _compile_runtime_backends(program, **kwargs):
    """Compile every runtime backend supported by the active accelerator."""
    cython_kernel = None
    if not is_ptpu_available():
        cython_kernel = tl.compile(program, execution_backend="cython", **kwargs)
    return cython_kernel, tl.compile(program, execution_backend="tvm_ffi", **kwargs)
...
def run_matmul(M, N, K, block_M, block_N, block_K, dtype=T.float16, accum_dtype=T.float32):
    ...
    cython_jit_kernel, ffi_jit_kernel = _compile_runtime_backends(program)

    def ref_program(a, b):
        return (a @ b.T).to(torch.float32)

    device = get_current_device()
    a = torch.randn(M, K, device=device, dtype=dtype.as_torch())
    b = torch.randn(N, K, device=device, dtype=dtype.as_torch())
    ffi_c = torch.zeros(M, N, device=device, dtype=accum_dtype.as_torch())
    cython_c = torch.zeros(M, N, device=device, dtype=accum_dtype.as_torch()) if cython_jit_kernel is not None else None

    ffi_jit_kernel(a, b, ffi_c, M, N, K)
    tilelang.testing.torch_assert_close(ffi_c, ref_program(a, b), atol=1e-2, rtol=1e-2)
    if cython_jit_kernel is not None:
        cython_jit_kernel(a.data_ptr(), b.data_ptr(), cython_c.data_ptr(), M, N, K)
        tilelang.testing.torch_assert_close(cython_c, ffi_c, atol=1e-2, rtol=1e-2)
...
def run_pointer_table_copy(N, dtype=T.float16):
    ...
    cython_jit_kernel, ffi_jit_kernel = _compile_runtime_backends(program)
    device = get_current_device()
    src = torch.randn(N, device=device, dtype=dtype.as_torch())
    src_ptrs = torch.tensor([src.data_ptr()], device=device, dtype=torch.int64)
    ffi_out = torch.empty(N, device=device, dtype=dtype.as_torch())
    cython_out = torch.empty(N, device=device, dtype=dtype.as_torch()) if cython_jit_kernel is not None else None

    ffi_jit_kernel(src_ptrs, ffi_out)
    tilelang.testing.torch_assert_close(ffi_out, src)
    if cython_jit_kernel is not None:
        cython_jit_kernel(src_ptrs, cython_out)
        tilelang.testing.torch_assert_close(cython_out, src)
        tilelang.testing.torch_assert_close(cython_out, ffi_out)
...
def run_pointer_table_multi_copy(G, N, dtype=T.float16):
    ...
    cython_jit_kernel, ffi_jit_kernel = _compile_runtime_backends(program)
    device = get_current_device()
    srcs = [torch.randn(N, device=device, dtype=dtype.as_torch()) for _ in range(G)]
    src_ptrs = torch.tensor([src.data_ptr() for src in srcs], device=device, dtype=torch.int64)
    ref = torch.stack(srcs, dim=0)
    ffi_out = torch.empty((G, N), device=device, dtype=dtype.as_torch())
    cython_out = torch.empty((G, N), device=device, dtype=dtype.as_torch()) if cython_jit_kernel is not None else None

    ffi_jit_kernel(src_ptrs, ffi_out)
    tilelang.testing.torch_assert_close(ffi_out, ref)
    if cython_jit_kernel is not None:
        cython_jit_kernel(src_ptrs, cython_out)
        tilelang.testing.torch_assert_close(cython_out, ref)
        tilelang.testing.torch_assert_close(cython_out, ffi_out)
...
def run_pointer_table_grouped_matmul(batch_sizes_list, N, K, block_M, block_N, block_K, dtype=T.float16, accum_dtype=T.float32):
    ...
    cython_jit_kernel, ffi_jit_kernel = _compile_runtime_backends(program, **compile_kwargs)
    ...
    cython_buffers = build_backend_buffers() if cython_jit_kernel is not None else None
    ...
    for out, expected, size in zip(ffi_c_list, ref, batch_sizes_list):
        tilelang.testing.torch_assert_close(out[:size].to(torch_accum_dtype), expected, atol=1e-2, rtol=1e-2)
    if cython_jit_kernel is not None:
        _, cython_c_list, cython_a_ptrs, cython_b_ptrs, cython_c_ptrs = cython_buffers
