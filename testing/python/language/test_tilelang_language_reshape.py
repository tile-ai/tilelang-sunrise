...
def test_reduce_absmax_after_reshape_3d():
    ...
    A_torch = torch.randint(-100, 100, (M, N), dtype=getattr(torch, dtype), device=get_current_device())
    B_torch = jit_kernel(A_torch)

    ref = A_torch.cpu().abs().reshape(M, num_groups, num_per_channels).max(dim=2).values
    torch.testing.assert_close(B_torch.cpu(), ref)
