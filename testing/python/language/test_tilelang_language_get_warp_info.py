...
def _check_tang_availability():
    """Check if TANG is available on the system."""
    return True


_IS_HIP_AVAILABLE = check_hip_availability()
_IS_TANG_AVAILABLE = _check_tang_availability()
_DEFAULT_WARPS_PER_GROUP = 32 if _IS_TANG_AVAILABLE else 4
...
def run_get_lane_id(num_threads: int = 128, warp_size: Optional[int] = None):
    ...
    ref = torch.arange(num_threads, dtype=A.dtype) % expected_warp_size
...
def run_get_warp_group_idx(
    ...
    ref = torch.arange(num_threads, dtype=A.dtype, device="cpu") // threads_per_group
...
@pytest.mark.skip(reason="S2 not support yet, TODO:")
