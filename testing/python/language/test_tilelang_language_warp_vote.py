...
def test_match_any_sync():
    b = torch.zeros((32,), device=get_current_device(), dtype=torch.int32)
    kernel = kernel_match_any_sync()
    src = kernel.get_kernel_source()
    assert "__match_any_sync" in src, f"Expected __match_any_sync in source:\n{src}"
    kernel(b)
    # The second mask is 0xFFFF0000, represented as signed int32 -65536.
    expected = torch.tensor(
        [0x0000FFFF] * 16 + [-0x00010000] * 16,
        dtype=torch.int32,
