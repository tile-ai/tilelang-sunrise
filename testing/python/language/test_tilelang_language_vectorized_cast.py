...
def run_vectorized_cast_tang(src_dtype: T.dtype, dst_dtype: T.dtype, lanes: int = 2):
    """Run S2 vectorized-cast paths on the TANG backend."""
    M = 128 * lanes

    @tilelang.jit(target="tang")
    def kernel_tang(M: int, dtype_A: str, dtype_B: str):
        @T.prim_func
        def main(
            A: T.Tensor[(M,), dtype_A],  # noqa: F821
            B: T.Tensor[(M,), dtype_B],  # noqa: F821
    ...
    @tilelang.jit(target="tang")
    def kernel_parallel_tang(M: int, dtype_A: str, dtype_B: str):
        @T.prim_func
        def main(
            A: T.Tensor[(M,), dtype_A],  # noqa: F821
            B: T.Tensor[(M,), dtype_B],  # noqa: F821
    ...
    kernel = kernel_tang(M, src_dtype, dst_dtype)
    kernel_parallel = kernel_parallel_tang(M, src_dtype, dst_dtype)
    assert kernel.get_kernel_source()
    assert kernel_parallel.get_kernel_source()

    device = get_current_device()
    A_float = torch.randn(M, dtype=torch.float32, device=device)
    A = A_float.to(src_dtype.as_torch())
    B = torch.zeros(M, dtype=dst_dtype.as_torch(), device=device)
    C = torch.zeros(M, dtype=dst_dtype.as_torch(), device=device)
    ...
    expected = A.to(dst_dtype.as_torch()).cpu()
    torch.testing.assert_close(expected, B.cpu())
    torch.testing.assert_close(expected, C.cpu())


@pytest.mark.parametrize(
    "src_dtype, dst_dtype, lanes",
    [
        (T.float16, T.float32, 2),
        (T.float16, T.float32, 4),
        (T.float32, T.float16, 2),
        (T.float32, T.float16, 4),
        (T.bfloat16, T.float32, 2),
        (T.bfloat16, T.float32, 4),
        (T.float32, T.bfloat16, 2),
        (T.float32, T.bfloat16, 4),
        (T.float16, T.bfloat16, 2),
        (T.float16, T.bfloat16, 4),
        (T.float16, T.bfloat16, 8),
        (T.bfloat16, T.float16, 2),
        (T.bfloat16, T.float16, 4),
        (T.bfloat16, T.float16, 8),
    ],
)
def test_vectorized_cast_tang(src_dtype, dst_dtype, lanes):
    run_vectorized_cast_tang(src_dtype, dst_dtype, lanes)
