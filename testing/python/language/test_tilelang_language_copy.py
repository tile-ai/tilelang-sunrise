...
@pytest.mark.parametrize("annotation_warps", [None, 2])
def test_scalar_copy_drain_warps_preserved(annotation_warps):
    annotations = {} if annotation_warps is None else {"drain_warps": annotation_warps}
    src = tvm.tirx.decl_buffer((1,), "float32", name="src")
    dst = tvm.tirx.decl_buffer((1,), "float32", name="dst")
    call = T.copy(src[0], dst[0], drain_warps=1, annotations=annotations)
    assert isinstance(call, tvm.tirx.Call)
    assert int(call.annotations["drain_warps"]) == (annotation_warps or 1)
    assert annotations == ({} if annotation_warps is None else {"drain_warps": annotation_warps})
