...
def run_tilelang_transpose(M=128, N=128, block_M=128, block_N=128, dtype=T.float16):
    ...
    a = torch.randn(M, N, device=get_current_device(), dtype=getattr(torch, dtype))
    b = kernel(a)
    expected = a.T
    tilelang.testing.torch_assert_close(b, expected, rtol=1e-2, atol=1e-2)
...
def run_tilelang_transpose_square(M=256, block_M=128, dtype=T.float16):
    ...
    a = torch.randn(M, M, device=get_current_device(), dtype=getattr(torch, dtype))
    b = kernel(a)
    expected = a.T
    tilelang.testing.torch_assert_close(b, expected, rtol=1e-2, atol=1e-2)
...
def _smem_optin_bytes() -> int:
    """Per-block dynamic shared-memory limit for the current accelerator.

    CUDA exposes its opt-in limit through device properties. The current PTPU
    runtime does not expose the equivalent property, so use the 64-KiB S2
    product limit when selecting a test tier.
