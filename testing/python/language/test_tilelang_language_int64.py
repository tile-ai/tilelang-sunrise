...
def _select_int64_stress_n(
    ...
    if device.type == "ptpu":
        free_bytes, total_bytes = torch.ptpu.mem_get_info()
    elif device.type == "cuda":
        free_bytes, total_bytes = torch.cuda.mem_get_info()
    else:
        pytest.skip("int64 stress test requires a CUDA or PTPU device")
...
def run_fill_symbolic(n: int):
    ...
    device = get_current_device()
    dtype = torch.float16 if device.type == "ptpu" else torch.bfloat16
    dtype_name = "float16" if device.type == "ptpu" else "bfloat16"
    x = torch.zeros(n, dtype=dtype, device=device)
    fill_symbolic(1.0, dtype_name)(x)
    if device.type == "ptpu":
        torch.ptpu.synchronize()
    x_host = x.cpu()
    assert x_host.min() == 1.0 and x_host.max() == 1.0
...
def run_fill_static(n: int):
    ...
    dtype_name = "float16" if device.type == "ptpu" else "bfloat16"
    x = torch.zeros(n, dtype=dtype, device=device)
    fill_static(n, 1.0, dtype_name)(x)
    if device.type == "ptpu":
        torch.ptpu.synchronize()
    x_host = x.cpu()
    assert x_host.min() == 1.0 and x_host.max() == 1.0
