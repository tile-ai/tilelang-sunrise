...
def run_shared_reinterpret_view(shape, dtype, new_shape, new_dtype, swizzled):
    ...
    lanes = torch.randn(shape[0], shape[1] * torch_dtype.itemsize // 2, device=get_current_device(), dtype=torch.bfloat16)
    a = lanes.view(dtype=torch_dtype)
    d = jit_kernel(a)
    ref = a.view(dtype=T.dtype(new_dtype).as_torch()).view(new_shape)
    torch.testing.assert_close(d.cpu().view(torch.int16), ref.cpu().view(torch.int16), rtol=0, atol=0)
