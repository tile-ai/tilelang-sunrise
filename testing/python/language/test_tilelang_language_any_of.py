...
def run_block_sparse_matmul_global(M=1024, N=1024, K=1024, sparsity=0.5, condition_dim=2, device="cuda"):
    block_M = 128
    block_N = 128
    block_K = 32
    num_stages = 2
    ...
    ref_c = ref_program(a.cpu(), b.cpu(), block_mask.cpu(), block_M, block_N, block_K)
    torch.testing.assert_close(c.cpu(), ref_c, rtol=1e-2, atol=1e-2)


def run_block_sparse_matmul_shared(M=1024, N=1024, K=1024, sparsity=0.5, condition_dim=2, device="cuda"):
    block_M = 128
    block_N = 128
    block_K = 32
    num_stages = 2
...
def run_block_sparse_matmul_local(M=1024, N=1024, K=1024, sparsity=0.5, condition_dim=2, device="cuda"):
    block_M = 128
    block_N = 128
    block_K = 32
    num_stages = 2
...
@pytest.mark.skipif(not _target_is_tang(), reason="Tang target required")
def test_block_sparse_matmul_global_tang():
    run_block_sparse_matmul_global(M=1024, N=1024, K=1024, sparsity=0.5, condition_dim=2, device="ptpu")


@pytest.mark.skipif(not _target_is_tang(), reason="Tang target required")
def test_block_sparse_matmul_shared_tang():
    run_block_sparse_matmul_shared(M=1024, N=1024, K=1024, sparsity=0.5, condition_dim=2, device="ptpu")


@pytest.mark.skipif(not _target_is_tang(), reason="Tang target required")
def test_block_sparse_matmul_local_tang():
    run_block_sparse_matmul_local(M=1024, N=1024, K=1024, sparsity=0.5, condition_dim=2, device="ptpu")
