...
def run_single_bitwise_reduce(
    ...
    if name == "reduce_bitand":
        expected = torch.full((M,), -1, device=get_current_device(), dtype=torch.int32)
    elif name == "reduce_bitor" or name == "reduce_bitxor":
        expected = torch.full((M,), 0, device=get_current_device(), dtype=torch.int32)
    else:
        raise ValueError("Invalid name: {}".format(name))

    output = kernel(a, expected)

    expected = reference_bitwise_reduce(name, a.cpu())
...
def bitwise_reduce_input():
    ...
    return a.to(device=get_current_device())
