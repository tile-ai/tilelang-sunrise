...
def run_single_arg_mathop_test(
    ...
    if str(tilelang.env.get_default_target()).startswith("tang") and cuda_mathop_name == "rsqrt":
        assert "/ sqrtf(" in source_no_fastmath, "TANG rsqrt must use the default 1/sqrtf legalize rule"
    else:
...
def run_two_arg_mathop_test(mathop_name, mathop_func, M=32, N=32, block_M=32, block_N=32, dtype=T.float32):
    ...
    torch.testing.assert_close(c_no_fastmath.cpu(), c_fastmath.cpu(), rtol=1e-3, atol=1e-3)
...
def run_abs_test():
    ...
    # Test numerical correctness
    a = torch.randn(M, N, device=get_current_device(), dtype=torch.float32)
    b = kernel(a)
    expected = torch.abs(a)

    torch.testing.assert_close(b.cpu(), expected.cpu(), rtol=1e-5, atol=1e-5)
...
def run_fastmath_mathop_test(mathop_name, mathop_func, M=32, N=32, block_M=32, block_N=32, dtype=T.float32):
    ...
    # Test numerical correctness
    torch_dtype = dtype.as_torch()
    a = torch.randn(M, N, device=get_current_device(), dtype=torch_dtype)
    ...
    torch.testing.assert_close(b_fastmath.cpu(), expected.cpu(), rtol=1e-3, atol=1e-3)
    print(f"✓ {mathop_name} numerical test passed")
