...
def debug_print_buffer(M=16, N=16, dtype=T.float16):
    ...
    profiler = jit_kernel.get_profiler(tensor_supply_type=tilelang.TensorSupplyType.Uniform)
    profiler.run_once()
...
def test_debug_print_buffer(dtype):
    if get_current_device().type == "ptpu" and dtype == T.float64:
        pytest.skip("ptpu/S2 has no native double")
