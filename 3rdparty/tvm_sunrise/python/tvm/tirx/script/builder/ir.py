...
def alloc_buffer(
    ...
    align : int
        The byte alignment requirement of the allocation. When > 0, it is
        recorded in ``annotations["storage_alignment"]`` so the backend can
        emit an aligned allocation (e.g. ``__align__(N)`` for shared memory).
    ...
    if align > 0:
        ...
        annotations.setdefault("storage_alignment", IntImm("int32", align))
