...
def _is_ptpu_available(torch_module: Any) -> bool:
    """Return whether the optional torch_ptpu backend is available."""
    if not hasattr(torch_module, "ptpu"):
        try:
            import torch_ptpu  # noqa: F401, PLC0415
        except Exception as err:
            warnings.warn(
                f"Failed to initialize optional torch_ptpu backend: {err}. "
                "Falling back to another available torch backend.",
                RuntimeWarning,
                stacklevel=2,
            )
            return False
    return bool(hasattr(torch_module, "ptpu") and torch_module.ptpu.is_available())


def _detect_torch_device(torch_module: Any) -> str:
    """Select the optional extension backend without requiring PTPU on CPU."""
    if _is_ptpu_available(torch_module):
        return "ptpu"
...
def load_torch_c_dlpack_extension() -> Any:  # noqa: PLR0912, PLR0915
    try:
        ...
        prefer_backend_override = bool(
            _is_ptpu_available(torch)
            or (torch.cuda.is_available() and torch.version.hip is not None)
        )
        if _check_and_update_dlpack_c_exchange_api(torch.Tensor) and not prefer_backend_override:
    ...
    try:
        ...
        # A separately installed torch_c_dlpack_ext can target another torch
        # ABI or backend and fail while importing its shared library. This also
        # covers the AttributeError raised by a torch ABI mismatch. Keep trying
        # the backend-specific JIT extension below.
        pass
