...
#ifdef BUILD_WITH_PTPU
      ctx.device_type = DLDeviceType::kDLTANG;
#else
...
struct TorchDLPackExchangeAPI : public DLPackExchangeAPI {
 ...
 private:
  ...
  // Get the current accelerator work stream.
...
#endif
#ifdef BUILD_WITH_PTPU
      if (device_type == kDLTANG) {
        py::object stream = py::module_::import("torch").attr("ptpu").attr("current_stream")(
            device_id);
        *out_stream = reinterpret_cast<void*>(stream.attr("ptpu_stream").cast<uintptr_t>());
        return 0;
      }
#endif
...
def main() -> None:  # noqa: PLR0912, PLR0915
    ...
    parser.add_argument(
        "--build-with-ptpu",
        action="store_true",
        help="Build with PTPU PrivateUse1 support.",
    )
    parser.add_argument(
        "--libname",
        type=str,
    ...
    enabled_accelerators = sum(
        (args.build_with_cuda, args.build_with_rocm, args.build_with_ptpu)
    )
    if enabled_accelerators > 1:
        raise ValueError("Cannot enable more than one accelerator backend at a time.")
    ...
    with FileLock(str(output_dir / (libname + ".lock"))):
        ...
        if args.build_with_cuda:
            ...
            cflags.append("-DBUILD_WITH_PTPU")
