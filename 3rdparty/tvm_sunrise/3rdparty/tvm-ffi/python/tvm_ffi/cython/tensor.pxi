...
cdef inline int _from_dlpack_legacy_object(
    object ext_tensor, int require_alignment,
    int require_contiguous, TVMFFIObjectHandle* out
) except -1:
    """Import legacy DLPack while preserving known PrivateUse1 provenance."""
    cdef int result = _from_dlpack(
        ext_tensor.__dlpack__(), require_alignment, require_contiguous, out
    )
    cdef DLTensor* tensor
    if result == 0 and getattr(ext_tensor, "is_ptpu", False):
        tensor = TVMFFITensorGetDLTensorPtr(out[0])
        if tensor.device.device_type != kDLTANG:
            # torch_ptpu legacy DLPack may report PrivateUse1 as kDLExtDev or
            # kDLCPU.  Preserve the original tensor's authoritative backend.
            tensor.device.device_type = kDLTANG
    return result
...
cdef inline int _from_dlpack_universal(
    ...
    if hasattr(ext_tensor, "__dlpack__"):
        if favor_legacy_dlpack:
            return _from_dlpack_legacy_object(
                ext_tensor, require_alignment, require_contiguous, out
            )
        else:
            try:
