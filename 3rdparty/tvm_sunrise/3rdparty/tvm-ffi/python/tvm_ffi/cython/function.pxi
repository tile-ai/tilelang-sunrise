...
cdef int TVMFFIPyArgSetterTorchFallback_(
    ...
    is_cuda = arg.is_cuda
    is_ptpu = getattr(arg, "is_ptpu", False)
    arg = from_dlpack(torch.utils.dlpack.to_dlpack(arg))
    out.type_index = kTVMFFITensor
    out.v_ptr = (<Tensor>arg).chandle
    temp_dltensor = TVMFFITensorGetDLTensorPtr((<Tensor>arg).chandle)
    if is_ptpu and temp_dltensor.device.device_type != kDLTANG:
        # The torch_ptpu legacy exporter is not stable across releases: it can
        # report PrivateUse1 as kDLExtDev or even kDLCPU.  The original tensor's
        # is_ptpu flag is the authoritative provenance at this FFI boundary.
        temp_dltensor.device.device_type = kDLTANG
    ...
    elif is_ptpu and ctx.device_type == -1:
        # torch_ptpu exposes the runtime stream handle via ptpu_stream. Do not
        # route PrivateUse1 through CUDA's stream API.
        import torch_ptpu  # noqa: F401
        ctx.device_type = temp_dltensor.device.device_type
        ctx.device_id = temp_dltensor.device.device_id
        temp_ptr = torch.ptpu.current_stream(ctx.device_id).ptpu_stream
        ctx.stream = <TVMFFIStreamHandle>temp_ptr
