...
..  Modified by TileLang-Sunrise contributors in 2026 to document the
    vendored source layout used by this distribution.
...
Download or unpack the complete source tree. The TileLang-Sunrise source
distribution already includes DLPack and libbacktrace under ``3rdparty/``.
For the standalone upstream repository, use its documented dependency setup.

.. code-block:: bash

   git clone https://github.com/apache/tvm-ffi
   cd tvm-ffi
...
- **Vendored dependencies missing**: obtain a complete source distribution that
  includes ``3rdparty/dlpack`` and ``3rdparty/libbacktrace``.
