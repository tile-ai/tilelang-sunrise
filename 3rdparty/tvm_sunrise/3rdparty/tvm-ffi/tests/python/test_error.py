...
def test_error_no_cyclic_reference() -> None:
    ...
    try:
        ...
        wrefs = [may_create_cyclic_reference() for _ in range(100)]

        # Repeated exceptions must not retain any probe while cyclic GC is disabled.
        assert all(wref() is None for wref in wrefs), (
            "Cyclic reference occurs inside error handling pipeline"
        )
