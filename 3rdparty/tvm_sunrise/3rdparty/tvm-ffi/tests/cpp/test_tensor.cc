...
struct TangLegacyManagedTensor {
  DLManagedTensor managed{};
  int64_t shape[1] = {4};
  int8_t data[4] = {};
  int* deleter_count;

  explicit TangLegacyManagedTensor(int* count) : deleter_count(count) {
    managed.dl_tensor = DLTensor{data, {kDLTANG, 7}, 1, {kDLInt, 8, 1}, shape, nullptr, 0};
    managed.manager_ctx = this;
    managed.deleter = [](DLManagedTensor* tensor) {
      auto* self = static_cast<TangLegacyManagedTensor*>(tensor->manager_ctx);
      ++*self->deleter_count;
      delete self;
    };
  }
...
struct TangVersionedManagedTensor {
  DLManagedTensorVersioned managed{};
  int64_t shape[1] = {4};
  int8_t data[4] = {};
  int* deleter_count;

  explicit TangVersionedManagedTensor(int* count) : deleter_count(count) {
    managed.version = {DLPACK_MAJOR_VERSION, DLPACK_MINOR_VERSION};
    managed.dl_tensor = DLTensor{data, {kDLTANG, 9}, 1, {kDLInt, 8, 1}, shape, nullptr, 0};
    managed.manager_ctx = this;
    managed.deleter = [](DLManagedTensorVersioned* tensor) {
      auto* self = static_cast<TangVersionedManagedTensor*>(tensor->manager_ctx);
      ++*self->deleter_count;
      delete self;
    };
  }
...
TEST(Tensor, TangDLPackLegacyRoundTripDeletesOnce) {
  int deleter_count = 0;
  {
    Tensor imported = Tensor::FromDLPack(&(new TangLegacyManagedTensor(&deleter_count))->managed);
    EXPECT_EQ(imported.device().device_type, kDLTANG);
    EXPECT_EQ(imported.device().device_id, 7);
    EXPECT_EQ(imported.dtype(), DLDataType({kDLInt, 8, 1}));
    DLManagedTensor* exported = imported.ToDLPack();
    Tensor roundtrip = Tensor::FromDLPack(exported);
    EXPECT_EQ(roundtrip.device().device_type, kDLTANG);
    EXPECT_EQ(roundtrip.device().device_id, 7);
  }
  EXPECT_EQ(deleter_count, 1);
}

TEST(Tensor, TangDLPackVersionedRoundTripDeletesOnce) {
  int deleter_count = 0;
  {
    auto* source = new TangVersionedManagedTensor(&deleter_count);
    Tensor imported = Tensor::FromDLPackVersioned(&source->managed);
    EXPECT_EQ(imported.device().device_type, kDLTANG);
    EXPECT_EQ(imported.device().device_id, 9);
    EXPECT_EQ(imported.dtype(), DLDataType({kDLInt, 8, 1}));
    DLManagedTensorVersioned* exported = imported.ToDLPackVersioned();
    Tensor roundtrip = Tensor::FromDLPackVersioned(exported);
    EXPECT_EQ(roundtrip.device().device_type, kDLTANG);
    EXPECT_EQ(roundtrip.device().device_id, 9);
  }
  EXPECT_EQ(deleter_count, 1);
}
