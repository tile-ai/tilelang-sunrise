...
def prepare_input(
    ...
    w = torch.randn(B, S, H, DK, dtype=gate_dtype).to(DEVICE)
...
def prepare_output(
    ...
    dq = torch.randn(B, S, H, DK, dtype=torch.float32).to(DEVICE)
    dk = torch.randn(B, S, H, DK, dtype=torch.float32).to(DEVICE)
    dw = torch.randn(B, S, H, DK, dtype=gate_dtype).to(DEVICE)
...
def run_test(
    ...
    q, k, v_new, _, g, h, dv, do, dh = prepare_input(B, S, H, DK, DV, chunk_size, getattr(torch, input_dtype), getattr(torch, gate_dtype))
    ...
    dg_ref = torch.zeros(B, S, H, DK)
    for b in range(B):
        for hh in range(H):
            for c in range(NT):
                sl = slice(c * chunk_size, (c + 1) * chunk_size)
                bg = gf[b, sl, hh]
                gn = gf[b, c * chunk_size + chunk_size - 1, hh]
                bh_ = hf[b, c, hh]
                bdh = dhf[b, c, hh]
                bv = vf[b, sl, hh]
                bdo = dof[b, sl, hh]
                ...
                dw_ref[b, sl, hh] = -b_dw
                b_dgk = b_dgk * (2.0**gn)
                b_dq = b_dq * scale * (2.0**bg)
                b_dk = b_dk * (2.0 ** (gn[None, :] - bg))
                b_dgk = b_dgk + (b_dk * bk).sum(0)
                b_dg = bq * b_dq - bk * b_dk
                b_dg[-1] = b_dg[-1] + b_dgk
                dq_ref[b, sl, hh] = b_dq
                dk_ref[b, sl, hh] = b_dk
                dg_ref[b, sl, hh] = b_dg

    assert_similar(dq.cpu().float(), dq_ref, eps=1e-3, name="dq", raise_assert=False)
    assert_similar(dk.cpu().float(), dk_ref, eps=1e-3, name="dk", raise_assert=False)
    assert_similar(dw.cpu().float(), dw_ref, eps=1e-3, name="dw", raise_assert=False)
    assert_similar(dg.cpu().float(), dg_ref, eps=1e-3, name="dg", raise_assert=False)
