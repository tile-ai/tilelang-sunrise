...
from test_utils_kda import assert_similar
...
def prepare_input(
    ...
    Aqk = torch.empty(B, S, H, chunk_size, dtype=input_dtype).to(DEVICE)
    Akk_diag = torch.ones(B, S, H, sub_chunk_size, dtype=torch.float32).to(DEVICE)
...
def tilelang_chunk_kda_fwd_inter_fused(
    ...
    def kernel(
        ...
        with T.Kernel(T.ceildiv(S, block_S), B * H, threads=threads) as (bs, bbh):
            bb, bh = bbh // H, bbh % H
            for i_s, j_s in T.Parallel(BS, BS):
                Aqk[bb, bs * BS + i_s, bh, j_s] = 0
                Akk[bb, bs * BS + i_s, bh, j_s] = 0
            ...
            Akk21_rhs_shared = T.alloc_shared((BC, BC), dtype=T.float32)
            ...
            Akk32_rhs_shared = T.alloc_shared((BC, BC), dtype=T.float32)
            ...
            for i_k in T.Pipelined(T.ceildiv(DK, block_DK), num_stages=num_stages):
                ...
                if i_tc1 < S:
                    ...
                    T.copy(GK[bb, i_tc1, bh, i_k * block_DK : (i_k + 1) * block_DK], b_gn1_shared)  # first token's GK in the subblock
            ...
            if i_tc2 < S:
                ...
                T.copy(Akk21_fragment, Akk21_rhs_shared)
            if i_tc3 < S:
                ...
                T.copy(Akk32_fragment, Akk32_rhs_shared)
            ...
            Ai_00_rhs_shared = T.alloc_shared((BC, BC), dtype=T.float32)
            Ai_11_rhs_shared = T.alloc_shared((BC, BC), dtype=T.float32)
            Ai_22_rhs_shared = T.alloc_shared((BC, BC), dtype=T.float32)
            T.copy(Ai_00_shared, Ai_00_rhs_shared)
            T.copy(Ai_11_shared, Ai_11_rhs_shared)
            T.copy(Ai_22_shared, Ai_22_rhs_shared)
            ...
            T.gemm(Ai_10_inv_shared, Ai_00_rhs_shared, Ai_10_final_frag, clear_accum=True)
            ...
            T.gemm(Ai_22_shared, Akk21_rhs_shared, Ai_21_inv_frag, clear_accum=True)
            T.copy(Ai_21_inv_frag, Ai_21_inv_shared)
            T.gemm(Ai_21_inv_shared, Ai_11_rhs_shared, Ai_21_final_frag, clear_accum=True)
            for i_bc, j_bc in T.Parallel(BC, BC):
                Ai_21_final_frag[i_bc, j_bc] = -Ai_21_final_frag[i_bc, j_bc]
            T.copy(Ai_21_final_frag, Ai_21_shared)
            # ---------- Ai_32 = - (Ai33@Akk32)@Ai22 ----------
            T.gemm(Ai_33_shared, Akk32_rhs_shared, Ai_32_inv_frag, clear_accum=True)
            T.copy(Ai_32_inv_frag, Ai_32_inv_shared)
            T.gemm(Ai_32_inv_shared, Ai_22_rhs_shared, Ai_32_final_frag, clear_accum=True)
            for i_bc, j_bc in T.Parallel(BC, BC):
            ...
            T.gemm(Akk20_shared, Ai_00_rhs_shared, Ai20_t0_frag, clear_accum=True)
            ...
            T.gemm(Akk31_shared, Ai_11_rhs_shared, Ai31_t0_frag, clear_accum=True)
            ...
            T.gemm(Akk30_shared, Ai_00_rhs_shared, Ai30_t0_frag, clear_accum=True)
...
def run_test(
    ...
    q, k, gk, beta, _, Akk_diag = prepare_input(
        B,
        S,
        H,
        DK,
    ...
    # CPU reference (fp32): off-diagonal Aqk + solve_tril block inverse
    cs, BC = chunk_size, sub_chunk_size
    NC = cs // BC
    qf, kf, gf, betaf, adf = (t.cpu().float() for t in (q, k, gk, beta, Akk_diag))
    Aqk_ref = torch.zeros(B, S, H, cs)
    Akk_ref = torch.zeros(B, S, H, cs)
    ...
    def solve_tril(D):
        ...
        for i in range(2, BC):
            row = -D[i].clone()
            row[i:] = 0.0
            row = row + (row[:, None] * Ai).sum(0)
            Ai[i] = row
        return Ai + torch.eye(BC)
    ...
    for b in range(B):
        for hh in range(H):
            for c0 in range(0, S, cs):
                ...
                def rows(s, c0=c0):
                    return slice(c0 + s * BC, c0 + (s + 1) * BC)

                gn = {s: gf[b, c0 + s * BC, hh] for s in range(NC)}
                Akkb = {}
                for s in range(NC):
                    for r in range(s):
                        gqn = torch.exp2(gf[b, rows(s), hh] - gn[s][None, :])
                        qg = qf[b, rows(s), hh] * gqn
                        kg = kf[b, rows(s), hh] * gqn
                        kgt = kf[b, rows(r), hh] * torch.exp2(gn[s][None, :] - gf[b, rows(r), hh])
                        Aqk_ref[b, rows(s), hh, r * BC : (r + 1) * BC] = (qg @ kgt.T) * scale
                        Akkb[(s, r)] = (kg @ kgt.T) * betaf[b, rows(s), hh][:, None]
                Ai = {s: solve_tril(adf[b, rows(s), hh]) for s in range(NC)}
                Ai[(1, 0)] = -(Ai[1] @ Akkb[(1, 0)]) @ Ai[0]
                Ai[(2, 1)] = -(Ai[2] @ Akkb[(2, 1)]) @ Ai[1]
                Ai[(3, 2)] = -(Ai[3] @ Akkb[(3, 2)]) @ Ai[2]
                Ai[(2, 0)] = -Ai[2] @ (Akkb[(2, 0)] @ Ai[0] + Akkb[(2, 1)] @ Ai[(1, 0)])
                Ai[(3, 1)] = -Ai[3] @ (Akkb[(3, 1)] @ Ai[1] + Akkb[(3, 2)] @ Ai[(2, 1)])
                Ai[(3, 0)] = -Ai[3] @ (Akkb[(3, 0)] @ Ai[0] + Akkb[(3, 1)] @ Ai[(1, 0)] + Akkb[(3, 2)] @ Ai[(2, 0)])
                for s in range(NC):
                    Akk_ref[b, rows(s), hh, s * BC : (s + 1) * BC] = Ai[s]
                    for r in range(s):
                        Akk_ref[b, rows(s), hh, r * BC : (r + 1) * BC] = Ai[(s, r)]
