...
def assert_finite(*tensors):
    assert all(torch.isfinite(tensor.cpu()).all() for tensor in tensors)


def build_kernel(kernel, device, *args, **kwargs):
    if device.type == "ptpu" and hasattr(kernel, "jit_impl"):
        kernel = kernel.jit_impl
