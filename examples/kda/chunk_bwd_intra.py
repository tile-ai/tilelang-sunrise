...
def prepare_input(
    ...
    # Shape: (B, S, H, BT)
    dAqk = torch.randn(B, S, H, BT, dtype=input_dtype).to(DEVICE)
    dAkk = torch.randn(B, S, H, BT, dtype=input_dtype).to(DEVICE)

    # Initial gradients (will be updated by the kernel)
    dq = torch.randn(B, S, H, DK, dtype=input_dtype).to(DEVICE)
    dk = torch.randn(B, S, H, DK, dtype=input_dtype).to(DEVICE)
    db = torch.randn(B, S, H, dtype=input_dtype).to(DEVICE)
...
def prepare_output(
    ...
    dq = torch.empty(B, S, H, DK, dtype=output_dtype).to(DEVICE)
    dk = torch.empty(B, S, H, DK, dtype=output_dtype).to(DEVICE)
    db = torch.empty(NK, B, S, H, dtype=output_dtype).to(DEVICE)
...
def tilelang_chunk_bwd_intra(
    ...
    def kernel(
        ...
        with T.Kernel(T.ceildiv(DK, block_DK) * NC, NT, B * H, threads=threads) as (i_kc, i_t, i_bh):
            ...
            kg_shared = T.alloc_shared((BC, block_DK), dtype=accum_dtype)
            ...
            if i_i > 0:
                ...
                for i_j in T.Pipelined(i_i, num_stages=num_stages):  # i_j is index ofprevious sub_chunks
                    ...
                    T.copy(kg_fragment, kg_shared)
                    T.gemm(dAqk_prev_shared, kg_shared, dq2_fragment, clear_accum=False)
                    T.gemm(dAkk_prev_shared, kg_shared, dk2_fragment, clear_accum=False)
...
def run_test(
    ...
    db_tilelang = db_tilelang.cpu().sum(0) + db.cpu()
    chunks = S // chunk_size
    dg_tilelang = dg_tilelang.cpu().reshape(B, chunks, chunk_size, H, DK)
    dg_tilelang = dg_tilelang.flip(2).cumsum(2).flip(2).reshape(B, S, H, DK)

    # CPU reference (fp32) via autograd on the equivalent forward
    # Akk includes the diagonal (i>=j); dg divides the g-gradient by ln2 before adding the incoming term, then reverse-cumsum
    NT = S // chunk_size
    L = chunk_size
    LN2 = math.log(2.0)
    idx = torch.arange(L)
    ...
    gc = g.cpu().float()
    bec = beta.cpu().float()
    dAqkc = dAqk.cpu().float()
    dAkkc = dAkk.cpu().float()
    dqc = dq.cpu().float()
    dkc = dk.cpu().float()
    dbc = db.cpu().float()
    dgc = dg.cpu().float()

    def rcs(x):  # reverse (suffix) cumsum within chunk, == tl.cumsum(reverse=True)
        return x.flip(0).cumsum(0).flip(0)

    o_dq = torch.zeros(B, S, H, DK)
    o_dk = torch.zeros(B, S, H, DK)
    o_db = torch.zeros(B, S, H)
    o_dg = torch.zeros(B, S, H, DK)
    for b in range(B):
        for hh in range(H):
            for c in range(NT):
                sl = slice(c * chunk_size, (c + 1) * chunk_size)
                q_ = qc[b, sl, hh].detach().requires_grad_()
                k_ = kc[b, sl, hh].detach().requires_grad_()
                g_ = gc[b, sl, hh].detach().requires_grad_()
                be_ = bec[b, sl, hh].detach().requires_grad_()
                wgt = torch.exp2(g_[:, None, :] - g_[None, :, :])
                inner_qk = (q_[:, None, :] * k_[None, :, :] * wgt).sum(-1)
                inner_kk = (k_[:, None, :] * k_[None, :, :] * wgt).sum(-1)
                Aqk = inner_qk * (idx[:, None] >= idx[None, :])
                Akk = be_[:, None] * inner_kk * (idx[:, None] >= idx[None, :])  # include diagonal
                dq_A, dk_A, dg_A, db_A = torch.autograd.grad(
                    [Aqk, Akk],
                    [q_, k_, g_, be_],
                    [dAqkc[b, sl, hh], dAkkc[b, sl, hh]],
                )
                o_dq[b, sl, hh] = dq_A + dqc[b, sl, hh]
                o_dk[b, sl, hh] = dk_A + dkc[b, sl, hh]
                o_db[b, sl, hh] = db_A + dbc[b, sl, hh]
                o_dg[b, sl, hh] = rcs(dg_A / LN2 + dgc[b, sl, hh])  # divide by ln2

    assert_similar(dq_tilelang.cpu().float(), o_dq, eps=1e-3, name="dq", raise_assert=False)
    assert_similar(dk_tilelang.cpu().float(), o_dk, eps=1e-3, name="dk", raise_assert=False)
    assert_similar(db_tilelang.cpu().float(), o_db, eps=1e-3, name="db", raise_assert=False)
    assert_similar(dg_tilelang.cpu().float(), o_dg, eps=1e-3, name="dg", raise_assert=False)
