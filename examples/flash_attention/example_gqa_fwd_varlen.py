...
def ref_gqa_varlen_fwd(q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, heads, groups, dim, is_causal):
    """CPU reference for GQA varlen forward with right-aligned causal masking.

    Mirrors the kernel semantics: kv heads are shared by `groups` query heads
    (kv_head = head // groups); Q/K are right-aligned by `offset = k_len - q_len`,
    and causal visibility is `rel_k <= rel_q + offset`. The mask value -1e9
    matches the kernel's `-1e9` so that fully-masked rows softmax to a uniform
    distribution and are zeroed out afterwards (same as the kernel's epilogue).
    """
    batch = cu_seqlens_q.numel() - 1
    head_kv = heads // groups
    scale = (1.0 / dim) ** 0.5
    out = torch.empty(q_unpad.shape, dtype=torch.float32, device=q_unpad.device)
    for b in range(batch):
        ...
        rel_q = torch.arange(q_len, device=q_unpad.device, dtype=torch.float32)
        rel_k = torch.arange(k_len, device=q_unpad.device, dtype=torch.float32)
        for h in range(heads):
            kv_h = h // groups
            q_blk = q_unpad[q_start:q_end, h, :].float()  # [q_len, dim]
            k_blk = k_unpad[k_start:k_end, kv_h, :].float()  # [k_len, dim]
            v_blk = v_unpad[k_start:k_end, kv_h, :].float()  # [k_len, dim]
            scores = q_blk @ k_blk.T * scale  # [q_len, k_len]
            if is_causal:
                # visible keys satisfy rel_k <= rel_q + offset
                scores = scores.masked_fill(rel_k[None, :] > (rel_q[:, None] + offset), -1e9)
            probs = torch.softmax(scores, dim=-1)
            o = probs @ v_blk  # [q_len, dim]
            if is_causal:
                # tokens with rel_q + offset < 0 see nothing -> 0
                o[rel_q + offset < 0] = 0.0
...
def main(
    ...
    kernel = flashattn(batch, groups, UQ, UKV, heads, dim, is_causal, block_M=64, block_N=64, num_stages=2, threads=128)
    ...
    if _HAS_FLASH_ATTN:
        ...
        print("flash_attn not available, using CPU reference.")
        ref_out_unpad = ref_gqa_varlen_fwd(
            q_unpad.detach().float().cpu(),
            k_unpad.detach().float().cpu(),
            v_unpad.detach().float().cpu(),
            cu_seqlens_q.cpu(),
            cu_seqlens_k.cpu(),
            heads,
            groups,
        ...
        torch.testing.assert_close(out_unpad.float().cpu(), ref_out_unpad, rtol=1e-2, atol=1e-2)
        print("All checks passed (CPU reference).✅")
    latency = do_bench(
