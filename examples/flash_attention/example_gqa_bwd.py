...
def _ref_program_cpu(Q, K, V, is_causal, groups=1):
...
def ref_program(Q, K, V, is_causal, groups=1):
    ...
    if hasattr(torch, "ptpu") and torch.ptpu.is_available() and Q.device.type == "ptpu":
        return _ref_program_cpu(Q.cpu(), K.cpu(), V.cpu(), is_causal, groups).to(Q.device)
    return _ref_program_cpu(Q, K, V, is_causal, groups)
...
def main(
    ...
    # Compute reference on CPU (ptpu does not support tril / masked_fill)
    Q_cpu = Q.detach().cpu().requires_grad_(True)
    K_cpu = K.detach().cpu().requires_grad_(True)
    V_cpu = V.detach().cpu().requires_grad_(True)
    dO_cpu = dO.cpu()
    O_ref_cpu = _ref_program_cpu(Q_cpu, K_cpu, V_cpu, causal, groups)
    ...
    torch.testing.assert_close(O.cpu(), O_ref_cpu, rtol=1e-2, atol=1e-2)
