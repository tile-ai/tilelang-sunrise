...
def _ref_program_cpu(Q, K, V, padding_mask, is_causal, groups=1):
    """ref_program on CPU tensors only."""
    # Q: [B, T, HQ, D_QK]
    # K: [B, T, HK, D_QK]
    # V: [B, T, HV, D_V]
    # HQ = HKV * groups
...
def ref_program(Q, K, V, padding_mask, is_causal, groups=1):
    """PTPU-safe: automatically moves tensors to CPU if needed."""
    if hasattr(torch, "ptpu") and torch.ptpu.is_available() and Q.device.type == "ptpu":
        padding_mask_cpu = padding_mask.cpu() if padding_mask is not None else None
        return _ref_program_cpu(Q.cpu(), K.cpu(), V.cpu(), padding_mask_cpu, is_causal, groups).to(Q.device)
    return _ref_program_cpu(Q, K, V, padding_mask, is_causal, groups)
...
def main(
    ...
    head_kv = H // groups
    Q = torch.empty(BATCH, N_CTX, H, D_HEAD_QK, dtype=torch.half, device=device).normal_().requires_grad_()
    K = torch.empty(BATCH, N_CTX, head_kv, D_HEAD_QK, dtype=torch.half, device=device).normal_().requires_grad_()
    V = torch.empty(BATCH, N_CTX, head_kv, D_HEAD_V, dtype=torch.half, device=device).normal_().requires_grad_()
    dO = torch.empty(BATCH, N_CTX, H, D_HEAD_V, dtype=torch.half, device=device).normal_().requires_grad_()
    padding_mask = generate_random_padding_mask(N_CTX, BATCH, device, mode="random")
    ...
    # Compute reference on CPU (ptpu does not support tril / masked_fill / masked_fill_)
    Q_cpu = Q.detach().cpu().requires_grad_(True)
    K_cpu = K.detach().cpu().requires_grad_(True)
    V_cpu = V.detach().cpu().requires_grad_(True)
    dO_cpu = dO.cpu()
    padding_mask_cpu = padding_mask.cpu()
    O_ref_cpu = _ref_program_cpu(Q_cpu, K_cpu, V_cpu, padding_mask_cpu, causal, groups)
    O_ref_cpu.backward(dO_cpu, retain_graph=True)
    dQ_ref_cpu = Q_cpu.grad.clone()
    dK_ref_cpu = K_cpu.grad.clone()
    dV_ref_cpu = V_cpu.grad.clone()

    if hasattr(torch, "ptpu") and torch.ptpu.is_available():
        torch.ptpu.synchronize()
    torch.testing.assert_close(O.cpu(), O_ref_cpu.half(), rtol=1e-2, atol=1e-2)
    torch.testing.assert_close(dQ.cpu(), dQ_ref_cpu, rtol=1e-2, atol=1e-2)
    torch.testing.assert_close(dK.cpu(), dK_ref_cpu, rtol=1e-2, atol=1e-2)
    torch.testing.assert_close(dV.cpu(), dV_ref_cpu, rtol=1e-2, atol=1e-2)
    ...
    latency = do_bench(run_ref, warmup=500)
...
def run_regression_perf():
    ...
    return do_bench(run_kernel_only, backend="event")
