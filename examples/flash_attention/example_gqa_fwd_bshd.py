...
def main(
    ...
    if not tune:
        ...
        latency = profiler.do_bench(ref_program_processed, warmup=5, rep=20, backend="event", return_mode="median")
        print("Ref: {:.2f} ms".format(latency))
        print("Ref: {:.2f} TFlops".format(total_flops / latency * 1e-9))
        latency = profiler.do_bench(warmup=5, rep=20, backend="event", return_mode="median")
...
def run_regression_perf(
    ...
    kernel = flashattn(batch, heads, seq_len, dim, is_causal, groups=groups, block_M=64, block_N=64, num_stages=2, threads=128)
    profiler = kernel.get_profiler(tensor_supply_type=tilelang.TensorSupplyType.Normal)
    return profiler.do_bench(warmup=5, rep=20, backend="event", return_mode="median")
