...
def _ref_program_cpu(Q, K, V, is_causal):
...
def ref_program(Q, K, V, is_causal):
    ...
    if hasattr(torch, "ptpu") and torch.ptpu.is_available() and Q.device.type == "ptpu":
        return _ref_program_cpu(Q.cpu(), K.cpu(), V.cpu(), is_causal).to(Q.device)
    return _ref_program_cpu(Q, K, V, is_causal)
...
def main(
    ...
    Q = torch.empty(BATCH, N_CTX, H, D_HEAD, dtype=torch.half, device=device).normal_().requires_grad_()
    ...
    dO_cpu = dO.cpu()
    O_ref_cpu = _ref_program_cpu(Q_cpu, K_cpu, V_cpu, causal)
    ...
    assert torch.allclose(O.cpu(), O_ref_cpu, rtol=1e-2, atol=1e-2)
    assert torch.allclose(dV.cpu(), dV_ref_cpu, rtol=1e-2, atol=5e-2)
    assert torch.allclose(dK.cpu(), dK_ref_cpu, rtol=1e-2, atol=5e-2)
    assert torch.allclose(dQ.cpu(), dQ_ref_cpu, rtol=1e-2, atol=5e-2)
