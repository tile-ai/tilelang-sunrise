...
class _GroupedGEMM(torch.autograd.Function):
    ...
    def forward(ctx, a, b, batch_sizes, block_M, block_N, block_K, num_stages, threads):
        ...
        batch_sizes_cpu = batch_sizes.cpu().tolist()
        batch_offsets_list = [0]
        batch_padded_offsets_list = [0]
        for i in range(batch_count - 1):
            batch_offsets_list.append(batch_offsets_list[-1] + batch_sizes_cpu[i])
        for i in range(batch_count - 1):
            batch_padded_offsets_list.append(batch_padded_offsets_list[-1] + math.ceil((batch_sizes_cpu[i] + 1) / padding_M) * padding_M)
        ...
        for k in T.Pipelined(T.ceildiv(batch_sizes[bz], block_K), num_stages=num_stages):
            for i, j in T.Parallel(block_K, block_M):
                A_shared[i, j] = T.if_then_else(
                    k * block_K + i < batch_sizes[bz], A[batch_offsets[bz] + k * block_K + i, bx * block_M + j], 0
                )
            for i, j in T.Parallel(block_K, block_N):
                B_shared[i, j] = T.if_then_else(
                    k * block_K + i < batch_sizes[bz], B[batch_offsets[bz] + k * block_K + i, by * block_N + j], 0
                )
...
def run_tilelang_grouped_gemm(
    ...
    # Compute reference on CPU (PTPU autograd may not be fully supported)
    A_cpu = A.detach().cpu().float()
    B_cpu = B.detach().cpu().float().clone().detach().requires_grad_(True)
    O_ref = ref_program(A_cpu, B_cpu, batch_sizes.cpu())
    dO = torch.randn_like(O_ref)
    O_ref.backward(dO, retain_graph=True)
    dB_ref = B_cpu.grad.clone()
    B_cpu.grad = None

    GroupedGEMM = _GroupedGEMM.apply
    O = GroupedGEMM(A, B, batch_sizes, block_M, block_N, block_K, num_stages, threads)
    O.backward(dO.to(device).to(dtype), retain_graph=True)
    dB = B.grad.clone()
    B.grad = None

    if torch.allclose(O.cpu().float(), O_ref, rtol=1e-2, atol=1e-2) and torch.allclose(dB.cpu().float(), dB_ref, rtol=5e-2, atol=5e-2):
