...
A = torch.rand(m, k, device="ptpu", dtype=torch.float16) * 2 - 1
B = torch.rand(n, k, device="ptpu", dtype=torch.float16) * 2 - 1
...
def tl_matmul_streamk(
    ...
    with T.Kernel(streamk_programs, threads=threads) as pid:
        ...
        while start_iter[0] < last_iter:
            ...
            T.clear(C_local)
            for k in T.Pipelined(end_iter[0] - start_iter[0], num_stages=0):
                T.copy(
...
def main():
    b_c = torch.zeros((m, n), device="ptpu", dtype=torch.float32)
    ...
    # .cpu() before printing: tensor repr needs aten::masked_select, which some
    # backends (e.g. ptpu) do not implement.
    print(b_c.cpu())
    print(C.cpu())
    torch.testing.assert_close(C.cpu(), b_c.cpu(), rtol=1e-2, atol=1e-2)
