...
def mhc_pre_gemm_sqrsum_tilelang(
    ...
    with T.Kernel(T.ceildiv(num_tokens, token_block)) as px:
        ...
        for pz in T.Pipelined(hc_hidden_size // hidden_block, num_stages=2):
            # TANG TMMA requires the gemm A operand in shared memory (not a
            # local fragment), so load x (bf16) straight into an fp32 shared
            # tile, converting on the way. fp32 runs on the tf32 tensor cores.
            ...
            # sqrsum reads the shared tile directly: staging it through a
            # bf16->fp32 fragment conflicts with the gemm A shared layout.
