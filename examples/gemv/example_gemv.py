...
def ref_program(A, B):
    """CPU reference shared by every kernel below.

    Both argument orders appear here: the SIMT kernels take (vector, matrix)
    while gemv_alloc_reducer takes (matrix, vector).  We disambiguate by
    ndim: the vector is always 1-D.

    The comparison runs on the host because the device-side matvec path is not
    used as the correctness reference.
    """
    assert A.ndim in (1, 2) and B.ndim in (1, 2), f"Expected 1-D or 2-D inputs, got ndim=({A.ndim}, {B.ndim})"
    vec, mat = (A, B) if A.ndim == 1 else (B, A)
    assert vec.ndim == 1 and mat.ndim == 2, f"Expected (vector, matrix) after disambiguation, got ({vec.ndim}-D, {mat.ndim}-D)"
    return (mat.cpu().float() @ vec.cpu().float()).to(vec.dtype)


def torch_bench_program(A, B):
    """Device-side torch baseline used only for latency, never for correctness.

    Kept as a 2-D matmul so it stays separate from the matvec correctness path.
    """
    assert A.ndim in (1, 2) and B.ndim in (1, 2)
    vec, mat = (A, B) if A.ndim == 1 else (B, A)
...
def check_correctness_and_bench(kernel, N, K, do_bench=True):
    ...
    if do_bench:
        latency = profiler.do_bench(torch_bench_program, warmup=50)
...
def main(do_bench: bool = True):
    ...
    if do_bench:
        ...
        profiler = kernel.get_profiler()
        latency = profiler.do_bench(torch_bench_program, warmup=500)
        print(f"Torch Latency: {latency} ms")
