...
def matmul_dynamic(M, N, K, block_M, block_N, block_K, trans_A, trans_B, in_dtype, out_dtype, accum_dtype, num_stages, threads):
    ...
    if trans_A:
        A = torch.rand(K, M, device="ptpu", dtype=getattr(torch, in_dtype))
    else:
        A = torch.rand(M, K, device="ptpu", dtype=getattr(torch, in_dtype))
    if trans_B:
        B = torch.rand(N, K, device="ptpu", dtype=getattr(torch, in_dtype))
    else:
        B = torch.rand(K, N, device="ptpu", dtype=getattr(torch, in_dtype))
