...
def ref_program(cb, x, dt, dA_cumsum, C, prev_states, D):
    ...
    """
    device = x.device
    cb_cpu, x_cpu, dt_cpu = cb.cpu(), x.cpu(), dt.cpu()
    dA_cumsum_cpu = dA_cumsum.cpu()
    C_cpu, prev_states_cpu = C.cpu(), prev_states.cpu()
    D_cpu = D.cpu()
    return _ref_program_cpu(cb_cpu, x_cpu, dt_cpu, dA_cumsum_cpu, C_cpu, prev_states_cpu, D_cpu).to(device)


def _ref_program_cpu(cb, x, dt, dA_cumsum, C, prev_states, D):
...
def chunk_scan_fwd(
    ...
    with T.Kernel(nheads, T.ceildiv(chunk_size, block_M) * T.ceildiv(headdim, block_N), batch * nchunks, threads=threads) as (
        ...
        cb_local_shared = T.alloc_shared((block_M, block_K), dtype)
        ...
        for k in T.Pipelined(loop_range, num_stages=num_stages):
            ...
            T.copy(cb_local, cb_local_shared)
            T.gemm(cb_local_shared, x_shared, acc_o)
