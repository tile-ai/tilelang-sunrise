...
def tl_fused_chunk_fwd_kernel(
    ...
    def fused_chunk_linear_attn_fwd(
        ...
        with T.Kernel(NV, NK, B * H) as (i_v, i_k, i_bh):
            ...
            # T.use_swizzle(10)  # Disabled for PTPU: causes h_shared layout conflict
...
def tl_fused_chunk_fwd(q, k, v):
    ...
    o = torch.zeros((B, S, H, D), device="ptpu", dtype=torch.float32)
    h = kernel(q, k, v, o)
    return o, h
...
def main(B=1, S=512, H=16, D=128):
    ...
    # ref_program runs on CPU (torch.triu/cumsum not supported on ptpu)
    torch.ptpu.synchronize()
    q_cpu, k_cpu, v_cpu = q.cpu(), k.cpu(), v.cpu()
    o_ref, h_ref = ref_program(q_cpu, k_cpu, v_cpu)
    o_cpu, h_cpu = o.cpu(), h.cpu()

    assert torch.allclose(o_cpu, o_ref, atol=1e-2, rtol=1e-2), f"o max err: {(o_cpu - o_ref).abs().max()}"
    assert torch.allclose(h_cpu, h_ref, atol=1e-2, rtol=1e-2), f"h max err: {(h_cpu - h_ref).abs().max()}"
    print("Passed all tests!✅")

    t2 = do_bench(lambda: tl_fused_chunk_fwd(q, k, v), backend="event")
    if _HAS_FLA:
        t1 = do_bench(lambda: fused_chunk_linear_attn(q, k, v, output_final_state=True, normalize=False), backend="event")
...
def run_regression_perf(B=1, S=512, H=16, D=128):
    ...
    kernel = tl_fused_chunk_fwd_kernel(B, S, H, D, D)
    o = torch.zeros((B, S, H, D), device="ptpu", dtype=torch.float32)
    return do_bench(lambda: kernel(q, k, v, o), backend="event")
