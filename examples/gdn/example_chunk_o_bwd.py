...
def prepare_input(
    ...
    h = torch.randn(B, BS, H, DK, DV, dtype=input_dtype).to(DEVICE)
    G = F.logsigmoid(torch.randn(B, S, H, dtype=gate_dtype))
    G = G.reshape(B, BS, chunk_size, H).cumsum(2).reshape(B, S, H).to(DEVICE)
    dO = torch.randn(B, S, H, DV, dtype=input_dtype).to(DEVICE)
    dh = torch.randn(B, BS, H, DK, DV, dtype=input_dtype).to(DEVICE)
    dv = torch.randn(B, S, H, DV, dtype=output_dtype).to(DEVICE)
...
def run_test(
    ...
    hc, Gc, dOc = h.cpu().float(), G.cpu().float(), dO.cpu().float()
    dhc, dvc_all = dh.cpu().float(), dv.cpu().float()
    dq_ref = torch.zeros(B, S, H, DK)
    dk_ref = torch.zeros(B, S, H, DK)
    dw_ref = torch.zeros(B, S, H, DK)
    dg_ref = torch.zeros(B, S, H)
    m_A = torch.arange(CS)[:, None] >= torch.arange(CS)[None, :]
    for b in range(B):
        for hh in range(H):
            for c in range(NT):
                ...
                dhcc = dhc[b, c, hh]
                g = Gc[b, sl, hh]
                ds = do @ v.transpose(0, 1)
                b_dq = do @ hcc.transpose(0, 1)
                b_dk = v @ dhcc.transpose(0, 1)
                dg_last = (hcc * dhcc).sum()
                b_dw = -(dvc @ hcc.transpose(0, 1))
                dw_ref[b, sl, hh] = b_dw
                g_last = g[-1]
                dg_last = dg_last * torch.exp(g_last)
                b_dq = b_dq * torch.exp(g)[:, None] * scale
                b_dg = (b_dq * q).sum(dim=1)
                b_dk = b_dk * torch.exp(-g + g_last)[:, None]
                b_dg = b_dg - (k * b_dk).sum(dim=1)
                dg_last = dg_last + (b_dk * k).sum()
                ds = torch.where(m_A, ds * torch.exp(g[:, None] - g[None, :]), torch.zeros_like(ds)) * scale
                ds2 = ds * (q @ k.transpose(0, 1))
                b_dg = b_dg + ds2.sum(dim=1) - ds2.sum(dim=0)
                b_dq = b_dq + ds @ k
                b_dk = b_dk + ds.transpose(0, 1) @ q
                b_dg[CS - 1] = b_dg[CS - 1] + dg_last
                dq_ref[b, sl, hh] = b_dq
                dk_ref[b, sl, hh] = b_dk
                dg_ref[b, sl, hh] = b_dg

    assert_similar(dq_ref, dq_tilelang.cpu().float(), 1e-2, "torch-tilelang", data="dq", raise_assert=False)
    assert_similar(dk_ref, dk_tilelang.cpu().float(), 1e-2, "torch-tilelang", data="dk", raise_assert=False)
    if use_dw:
        assert_similar(dw_ref, dw_tilelang.cpu().float(), 1e-2, "torch-tilelang", data="dw", raise_assert=False)
    if use_g:
        # kernel dg is (NK,B,S,H); reduce over NK to (B,S,H)
        assert_similar(dg_ref, dg_tilelang.cpu().float().sum(dim=0), 1e-2, "torch-tilelang", data="dg", raise_assert=False)
