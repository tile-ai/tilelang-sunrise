...
def test_topk_sparse_attention():
    ...
    # Create inputs on the host (PTPU lacks scatter_/masked_select).
    q = torch.randn(BATCH, N_HEADS, SEQ_LEN, D_HEAD, dtype=torch.float16)
    k = torch.randn(BATCH, N_HEADS, SEQ_LEN, D_HEAD, dtype=torch.float16)
    v = torch.randn(BATCH, N_HEADS, SEQ_LEN, D_HEAD, dtype=torch.float16)
    ...
    x_ds = torch.randn([BATCH, N_HEADS, downsample_len, downsample_len], dtype=torch.bfloat16)
    x_ds[:, :, :, 0] = 100
    ...
    tilelang_output = kernel(q.to(device), k.to(device), v.to(device), block_mask.to(device))
    if is_ptpu_available():
        torch.ptpu.synchronize()
    tilelang_output = tilelang_output.cpu()

    # Compute reference
    # Expand block mask to full attention matrix
    full_mask = torch.kron(block_mask.float(), torch.ones(BLOCK, BLOCK))
