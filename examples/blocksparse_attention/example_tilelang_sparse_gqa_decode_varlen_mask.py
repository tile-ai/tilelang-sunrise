...
def main(batch=8, heads=32, heads_kv=8, max_cache_seqlen=8192, dim=128, dim_v=128, sparse_ratio=0.8, block_size=32):
    ...
    random_index = torch.randint(0, batch, (1,)).item()  # Select a random index
    cache_seqlens[random_index] = max_cache_seqlen  # Assign cache_seqlen to ensure at least one occurrence
    # cache_seqlens = torch.full((batch,), max_cache_seqlen, dtype=torch.int32)
    ...
    block_mask, cache_seqlens = block_mask.to(device), cache_seqlens.to(device)

    model = SparseFlashAttn(batch, heads, heads_kv, dim, dim_v, block_size)
    out = model(Q, K, V, block_mask, cache_seqlens)
    _sync_device()
    assert_close("output", ref, out.cpu(), atol=1e-3, rtol=1e-3)

    # FA latency reference (flash_attn, CUDA-only).
    try:
    ...
    if has_flash_attn:
        ...
        print("flash_attn not available, skipping dense latency reference")
...
def run_regression_perf(batch=8, heads=32, heads_kv=8, max_cache_seqlen=8192, dim=128, dim_v=128, sparse_ratio=0.8, block_size=32):
    ...
    max_selected_blocks = int(math.ceil(max_cache_seqlen * (1 - sparse_ratio) / block_size))
    dtype = torch.float16
    # Build inputs on host (PTPU lacks randperm).
    Q = torch.randn((batch, heads, dim), dtype=dtype)
    K = torch.randn((batch, max_cache_seqlen, heads_kv, dim), dtype=dtype)
    V = torch.randn((batch, max_cache_seqlen, heads_kv, dim_v), dtype=dtype)
    cache_seqlens = torch.randint(1, max_cache_seqlen, (batch,), dtype=torch.int32)
    random_index = torch.randint(0, batch, (1,)).item()
