...
class SparseFlashAttn(torch.nn.Module):
    def __init__(self, batch, heads, heads_kv, dim, dim_v, page_block_size, block_N, num_pages):
        ...
        self.block_H = 64
        props = torch.ptpu.get_device_properties(0) if is_ptpu_available() else torch.cuda.get_device_properties(torch.device("cuda:0"))
    ...
    def forward(self, query, key, value, block_indices, cache_seqlens, block_table):
        ...
        glse = torch.empty((batch, heads, num_split), dtype=torch.float32, device=query.device)
        output_partial = torch.empty((batch, heads, num_split, dim_v), dtype=torch.float32, device=query.device)
...
def main(
    ...
    # Generate inputs on the host (backend-agnostic).
    Q = torch.randn((batch, heads, dim), dtype=dtype)
    cache_seqlens = torch.randint(max_cache_seqlen // 2, max_cache_seqlen + 1, (batch,), dtype=torch.int32)
    print("cache_seqlens: ", cache_seqlens)

    K = torch.randn((batch, max_cache_seqlen, heads_kv, dim), dtype=dtype)
    V = torch.randn((batch, max_cache_seqlen, heads_kv, dim_v), dtype=dtype)

    # Create paged KV cache
    K_cache = torch.zeros((num_blocks, page_block_size, heads_kv, dim), dtype=dtype)
    V_cache = torch.zeros((num_blocks, page_block_size, heads_kv, dim_v), dtype=dtype)
    ...
    block_table = torch.zeros((batch, max_num_blocks_per_seq), dtype=torch.int32)
    block_indices = torch.zeros((batch, heads_kv, max_selected_blocks), dtype=torch.int32)
    ...
    # Reference on host (before inputs move to device).
    ...
    output_ref_fa = ref_program_fa(Q, K_cache, V_cache, cache_seqlens, block_table) if has_flash_attn else None

    device = get_current_device()
    Q_d, K_cache_d, V_cache_d = Q.to(device), K_cache.to(device), V_cache.to(device)
    block_indices_d, cache_seqlens_d = block_indices.to(device), cache_seqlens.to(device)
    block_table_d = block_table.to(device)

    sparse_attn = SparseFlashAttn(batch, heads, heads_kv, dim, dim_v, page_block_size, block_N, num_blocks)
    output_sparse = sparse_attn.forward(Q_d, K_cache_d, V_cache_d, block_indices_d, cache_seqlens_d, block_table_d)
    _sync_device()
    output_sparse = output_sparse.cpu()

    # Check correctness
    if sparse_ratio == 0.0 and has_flash_attn:
    ...
    assert max_diff < 1e-2, f"✗ Verification FAILED: max difference {max_diff:.6f} exceeds 1e-2"
    print("✓ Verification PASSED: Results match within tolerance")

    # Performance measurement
    kernel_time = do_bench(
        lambda: sparse_attn.forward(Q_d, K_cache_d, V_cache_d, block_indices_d, cache_seqlens_d, block_table_d),
        backend="event",
        warmup=5,
        rep=20,
        return_mode="median",
    ...
    # FA performance measurement (requires the CUDA-only flash_attn package)
    if has_flash_attn:
        kernel_time_fa = do_bench(
            lambda: ref_program_fa(Q_d, K_cache_d, V_cache_d, cache_seqlens_d, block_table_d),
            backend="event",
            warmup=5,
            rep=20,
            return_mode="median",
        ...
        print("flash_attn not available, skipping FA latency reference")
...
def run_regression_perf(
    ...
    max_selected_blocks = int(math.ceil(max_cache_seqlen / block_N))
    dtype = torch.float16
    # Built on the host, then moved to the device once the paged cache is filled.
    Q = torch.randn((batch, heads, dim), dtype=dtype)
    cache_seqlens = torch.randint(max_cache_seqlen // 2, max_cache_seqlen + 1, (batch,), dtype=torch.int32)
    K = torch.randn((batch, max_cache_seqlen, heads_kv, dim), dtype=dtype)
    V = torch.randn((batch, max_cache_seqlen, heads_kv, dim_v), dtype=dtype)
    K_cache = torch.zeros((num_pages, page_block_size, heads_kv, dim), dtype=dtype)
    V_cache = torch.zeros((num_pages, page_block_size, heads_kv, dim_v), dtype=dtype)
    max_num_blocks_per_seq = int(math.ceil(max_cache_seqlen / page_block_size))
    block_table = torch.zeros((batch, max_num_blocks_per_seq), dtype=torch.int32)
    block_indices = torch.zeros((batch, heads_kv, max_selected_blocks), dtype=torch.int32)
    total_blocks_needed = sum(int(math.ceil(cache_seqlens[seq_idx].item() / page_block_size)) for seq_idx in range(batch))
    ...
    device = get_current_device()
    Q, K_cache, V_cache = Q.to(device), K_cache.to(device), V_cache.to(device)
    block_indices, cache_seqlens = block_indices.to(device), cache_seqlens.to(device)
    ...
    glse = torch.empty((batch, heads, num_split), dtype=torch.float32, device=device)
    output_partial = torch.empty((batch, heads, num_split, dim_v), dtype=torch.float32, device=device)
    kernel = flashattn(
        batch,
        heads,
        heads_kv,
    ...
    return do_bench(run_kernel_only, backend="event", warmup=5, rep=20, return_mode="median")
