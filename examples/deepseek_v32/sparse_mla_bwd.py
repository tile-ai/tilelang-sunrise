...
def bwd(
    ...
    with T.Kernel(S, B, kv_group * NH, threads=threads) as (s_i, by, bz):
        Q_shared = T.alloc_shared([block_H, D if _IS_TANG else D + D_tail], dtype)
        if _IS_TANG:
        ...
        for i_i in T.Pipelined(NS, num_stages=num_stages):
            ...
            T.clear(acc_dp)
            T.gemm(dO_shared, KV_shared, acc_dp, transpose_B=True, policy=T.GemmWarpPolicy.FullCol)
            ...
            T.clear(acc_dkv)
            T.gemm(dP_shared_cast, Q_shared[:, :D], acc_dkv, transpose_A=True, policy=T.GemmWarpPolicy.FullCol)
...
def sparse_mla_bwd(q, kv, o, do, indices, lse, sm_scale=None, is_casual=True, return_kernel=False, delta=None, block_size=32, threads=256):
    ...
    dq = bwd(q, kv, do, indices, lse, delta, dkv, H, D, D_tail, topk, kv_group, sm_scale, is_casual, block_size=block_size, threads=threads)
...
def test_sparse_mla_bwd(
    ...
    # Prepare data
    q = torch.randn((B, S, H, DQKV), dtype=dtype, device="cpu").requires_grad_(True).ptpu()
    kv = torch.randn((B, SKV, HKV, DQKV), dtype=dtype, device="cpu").requires_grad_(True).ptpu()
    do = torch.randn((B, S, H, DV), dtype=dtype, device="cpu").ptpu()

    indices = torch.full((B, S, HKV, topk), SKV, dtype=torch.int32, device="ptpu")
    for b in range(B):
        for t in range(S):
            for h in range(HKV):
    ...
    tl_dq, tl_dkv = sparse_mla_bwd(q, kv, tl_out, do, indices, tl_lse, block_size=block_size, threads=threads)

    if check_correctness:
        ref_dq, ref_dkv = ref_sparse_mla_bwd_interface(q.cpu(), kv.cpu(), None, do.cpu(), indices.cpu(), None)
        assert_tensors_similar(tl_dq.cpu(), ref_dq.cpu(), eps=1e-4, name="dq")
        assert_tensors_similar(tl_dkv.cpu(), ref_dkv.cpu(), eps=1e-4, name="dkv")
    ...
    def fn():
        return sparse_mla_bwd(q, kv, tl_out, do, indices, tl_lse, block_size=block_size, threads=threads)
