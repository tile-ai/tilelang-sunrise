...
def test_topk_selector(batch=64, seq_len=32 * 1024, topk=2048):
    torch.manual_seed(1)
    input = torch.randn(batch, seq_len, dtype=torch.float32).ptpu()
    starts = torch.zeros(batch, dtype=torch.int32).ptpu()
    ends = torch.ones(batch, dtype=torch.int32).ptpu() * seq_len

    indexes = tl_topk(input, starts, ends, topk)
    print(indexes)

    indexes_ref = torch.topk(input.cpu(), topk, dim=-1)[1]
    print(indexes_ref)
    ...
    # Performance test with PTU events

    torch.ptpu.synchronize()
    start_event = torch.ptpu.Event(enable_timing=True)
    end_event = torch.ptpu.Event(enable_timing=True)
    ...
    start_event.record()
    for _ in range(n_iters):
        _ = torch.topk(input.cpu(), topk, dim=-1)[1]
    end_event.record()
    torch.ptpu.synchronize()
    elapsed_time_ms = start_event.elapsed_time(end_event)
