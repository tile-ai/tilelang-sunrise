...
def _run_varlen_case(lengths, heads, dim, dllm_block, block_size=64, dtype=torch.float16):
    ...
    cu = torch.tensor([0, *itertools.accumulate(lengths)], device=device, dtype=torch.int32)
    total = int(cu[-1].item())
    query = torch.randn(total, heads, dim, device=device, dtype=dtype)
    ...
    out_ref = block_causal_attention_varlen_ref(q_ref, k_ref, v_ref, cu.cpu(), dllm_block)
