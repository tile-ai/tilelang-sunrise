...
def _fwd_template(
    ...
    alloc_gemm_a = T.alloc_shared if target_is_tang(target) else T.alloc_fragment
    ...
    def fwd(
        ...
        with T.Kernel(T.ceildiv(seq_len, block_size), heads, batch, threads=threads) as (bx, by, bz):
            ...
            acc_s_cast = alloc_gemm_a([block_size, block_size], dtype)
...
def _bwd_dq_template(
    ...
    def dq_kernel(
        ...
        with T.Kernel(T.ceildiv(seq_len, block_size), heads, batch, threads=threads) as (bx, by, bz):
            ...
            ds_cast = alloc_gemm_a([block_size, block_size], dtype)
...
def _bwd_dkv_template(
    ...
    def dkv_kernel(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_size), batch, threads=threads) as (bx, by, bz):
            ...
            qkT_cast = alloc_gemm_a([block_size, block_size], dtype)
            dsT_cast = alloc_gemm_a([block_size, block_size], dtype)
...
def _dllm_mask(seq_len: int, dllm_block_size: int, device) -> torch.Tensor:
    half_len = seq_len // 2
    # Build the reference-only index mask on CPU. torch_ptpu does not provide
    # floor_divide for integer tensors, while the TileLang kernel under test
    # does not depend on this PyTorch reference implementation.
    ...
    return (block_diagonal | offset_causal | clean_causal).to(device)
...
def _run_fixed_case(batch, seq_len, heads, dim, dllm_block, dtype=torch.float16):
    ...
    query = torch.randn(batch, seq_len, heads, dim, device=device, dtype=dtype)
    key = torch.randn_like(query)
    value = torch.randn_like(query)
    grad = torch.randn_like(query)

    q_ref, k_ref, v_ref = (_clone_with_grad(t.cpu()) for t in (query, key, value))
    ...
    torch.testing.assert_close(out_tl.cpu(), out_ref, atol=2e-2, rtol=2e-2)

    out_ref.backward(grad.cpu())
    out_tl.backward(grad)
    if device.type == "ptpu":
        torch.ptpu.synchronize(device)
    torch.testing.assert_close(q_tl.grad.cpu(), q_ref.grad, atol=5e-2, rtol=5e-2)
    torch.testing.assert_close(k_tl.grad.cpu(), k_ref.grad, atol=5e-2, rtol=5e-2)
    torch.testing.assert_close(v_tl.grad.cpu(), v_ref.grad, atol=5e-2, rtol=5e-2)
