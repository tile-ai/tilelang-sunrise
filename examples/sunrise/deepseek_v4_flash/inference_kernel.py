            ...
            for _ in T.Pipelined(1, num_stages=num_stages):
                ...
                if inplace:
                    for i, j in T.Parallel(blk_m, group_size):
                        y_local[i, j] = T.Cast(
                            out_dtype,
                            T.Cast(compute_dtype, T.Cast(out_dtype, T.clamp(x_local[i, j] / s_local[i], fp8_min, fp8_max))) * s_local[i],
                        )
                else:
                    for i, j in T.Parallel(blk_m, group_size):
...
def sparse_attn(q: torch.Tensor, kv: torch.Tensor, attn_sink: torch.Tensor, topk_idxs: torch.Tensor, softmax_scale: float) -> torch.Tensor:
    ...
    if h < 16:
        ...
        attn_sink = torch.cat([attn_sink, attn_sink.new_zeros(16 - h)], dim=0)
...
def ref_sparse_attn(
    ...
    """
    Reference implementation of sparse attention with attention sink.
    This should match the kernel's computation exactly.

    Args:
        q: (B, S, H, D) - query tensor
        kv: (B, SKV, D) - key/value tensor
        attn_sink: (H,) - attention sink bias per head
        topk_idxs: (B, S, topk) - indices of top-k KV positions
    ...
    # Pad heads to 16 if needed (kernel does this)
    if H < 16:
        q = torch.cat([q, q.new_zeros(B, S, 16 - H, D)], dim=2)
    ...
    output = torch.zeros(B, S, H_padded, D, dtype=q.dtype, device=q.device)

    # Process each (batch, seq_pos)
    for b in range(B):
        for s in range(S):
            # Gather KV for this query position
            indices = topk_idxs[b, s, :]  # (topk,)

            # Get KV values: (topk, D)
            kv_gathered = kv[b, indices, :]  # (topk, D)

            # Get query: (H, D)
            q_curr = q[b, s, :, :]  # (H, D)

            # Compute Q @ K^T: (H, topk)
            # scores[h, k] = sum_d q[h, d] * kv_gathered[k, d]
            scores = torch.matmul(q_curr, kv_gathered.t())  # (H, topk)

            # Apply softmax scale
            scores = scores * softmax_scale

            # Online softmax with attention sink
            # The kernel uses: running max and exp normalization
            # First, set invalid positions to -inf
            scores = torch.where(indices.unsqueeze(0) >= 0, scores, scores.new_tensor(float("-inf")))

            # Online softmax computation
            # Step 1: get current max
            scores_max = scores.max(dim=-1, keepdim=True).values  # (H, 1)

            # Step 2: exp normalize
            scores_exp = torch.exp(scores - scores_max)  # (H, topk)

            # Step 3: sum
            sum_exp = scores_exp.sum(dim=-1, keepdim=True)  # (H, 1)

            # Step 4: add attention sink
            # The kernel adds: exp(attn_sink - scores_max) to sum_exp
            sink_exp = torch.exp(attn_sink.unsqueeze(1) - scores_max)  # (H, 1)
            sum_exp = sum_exp + sink_exp

            # Step 5: compute attention weights
            attn_weights = scores_exp / sum_exp  # (H, topk)

            # Step 6: compute output
            # O = attn_weights @ V = (H, topk) @ (topk, D) -> (H, D)
            o_curr = torch.matmul(attn_weights, kv_gathered)  # (H, D)

            output[b, s, :, :] = o_curr

    # Trim padding if needed
    if H < 16:
        output = output[:, :, :H, :].contiguous()
