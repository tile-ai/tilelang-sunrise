...
class MoETorch(nn.Module):
    ...
    def moe_infer(self, x: torch.Tensor, flat_expert_indices: torch.Tensor, flat_expert_weights: torch.Tensor) -> torch.Tensor:
        ...
        counts = flat_expert_indices.cpu().bincount().numpy()
        tokens_per_expert = counts.cumsum()
        num_per_tok = self.config["n_experts_per_token"]
        token_idxs = (idxs.cpu() // num_per_tok).to(x.device)
...
def generate_input(
    ...
    input_tensor = torch.randn((batch_size, seq_len, d_hidden), device=DEVICE, dtype=torch.float16, generator=gen).contiguous()

    # Initialize router weights
    weights["router.weight"] = torch.randn((num_experts, d_hidden), device=DEVICE, dtype=torch.float16, generator=gen) / math.sqrt(d_hidden)

    for i in range(num_experts):
        weights[f"experts.{i}.0.weight"] = torch.randn(
            (d_hidden, expert_dim), device=DEVICE, dtype=torch.float16, generator=gen
        ...
        weights[f"experts.{i}.2.weight"] = torch.randn(
            (expert_dim, d_hidden), device=DEVICE, dtype=torch.float16, generator=gen
        ) / math.sqrt(d_hidden)

    weights["shared_experts.0.weight"] = torch.randn(
        (d_hidden, expert_dim * n_shared_experts), device=DEVICE, dtype=torch.float16, generator=gen
    ...
    weights["shared_experts.2.weight"] = torch.randn(
        (expert_dim * n_shared_experts, d_hidden), device=DEVICE, dtype=torch.float16, generator=gen
    ) / math.sqrt(d_hidden)
