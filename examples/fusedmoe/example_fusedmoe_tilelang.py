...
from example_fusedmoe_torch import DEVICE

# Stream API lives on the accelerator module (torch.ptpu / torch.cuda).
ACCEL = getattr(torch, DEVICE.type)
...
def moe_forward_tilelang_shared(
    ...
    with T.Kernel(T.ceildiv(num_tokens, block_token), T.ceildiv(dexpert, block_dexpert), threads=threads) as (bx, by):
        ...
        input_shared = T.alloc_shared((block_token, block_dhidden), dtype=dtype)
    ...
    with T.Kernel(T.ceildiv(num_tokens, block_token), T.ceildiv(dhidden, block_dhidden), threads=threads) as (bx, by):
        up_logits_shared = T.alloc_shared((block_token, block_dexpert), dtype=dtype)
...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        token_idxs = (idxs.cpu() // num_per_tok).to(self.device)
        ...
        shared_stream = ACCEL.Stream()
        routed_stream = ACCEL.default_stream()
        ACCEL.synchronize()

        with ACCEL.stream(routed_stream):
        ...
        with ACCEL.stream(shared_stream):
            self.shared_kernel(
                x_flat,
...
def main(d_hidden=7168, d_expert=2048, n_routed_experts=8, n_shared_experts=1, n_experts_per_token=4, batch_size=1, seq_len=8192):
    ...
    torch.testing.assert_close(ref_output.cpu(), tilelang_output.cpu(), atol=1e-2, rtol=1e-2)
