...
if __name__ == "__main__":
    ...
    # randomly split the sequence into N segments (CPU)
    offsets = torch.cat(
        [
    ...
    perm_q = torch.randperm(C_SEQ_LEN)
    perm_k = torch.randperm(C_SEQ_LEN)
    perm_v = torch.randperm(C_SEQ_LEN)
    q_cpu = torch.linspace(0, 1, steps=C_SEQ_LEN, dtype=dtype)[perm_q].view(1, C_SEQ_LEN, 1, 1).expand(1, C_SEQ_LEN, HQ, D).clone()
    k_cpu = torch.linspace(0, 1, steps=C_SEQ_LEN, dtype=dtype)[perm_k].view(1, C_SEQ_LEN, 1, 1).expand(1, C_SEQ_LEN, H, D).clone()
    v_cpu = torch.linspace(0, 1, steps=C_SEQ_LEN, dtype=dtype)[perm_v].view(1, C_SEQ_LEN, 1, 1).expand(1, C_SEQ_LEN, H, D).clone()
    g_slc_cpu = torch.rand((1, C_SEQ_LEN, HQ), dtype=dtype)
    g_swa_cpu = torch.rand((1, C_SEQ_LEN, HQ), dtype=dtype)

    token_indices = prepare_token_indices(offsets).tolist()
    block_indices_cpu = torch.full((1, C_SEQ_LEN, H, S), C_SEQ_LEN, dtype=torch.long)
    for i in range(C_SEQ_LEN):
        _, t = token_indices[i]
        for h in range(H):
            i_i = torch.randperm(max(1, tilelang.cdiv(t, block_size)))[:S]
            block_indices_cpu[0, i, h, : len(i_i)] = i_i
    block_indices_cpu = block_indices_cpu.sort(-1)[0]
    block_counts_cpu = torch.randint(1, S + 1, (1, C_SEQ_LEN, H))

    # CPU reference (naive_nsa relies on einsum/softmax, not reliable on PTPU)
    ref = naive_nsa(
        q=q_cpu,
        k=k_cpu,
        v=v_cpu,
    ...
    k = k_cpu.to(device)
    v = v_cpu.to(device)
    g_slc = g_slc_cpu.to(device)
    g_swa = g_swa_cpu.to(device)
    block_indices = block_indices_cpu.to(device)
    block_counts = block_counts_cpu.to(device)
    offsets_ptpu = offsets.to(device)
