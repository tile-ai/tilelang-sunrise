...
def prepare_token_indices(cu_seqlens):
    # Local replacement for fla.ops.utils.prepare_token_indices (fla pulls in triton,
    # which is unavailable in the PTPU environment). Computes [seq_id, pos_id] per
    # token; the index math runs on CPU, the result lands on cu_seqlens.device.
    cu = cu_seqlens.cpu()
    lens = cu[1:] - cu[:-1]
    position_ids = torch.cat([torch.arange(int(n.item())) for n in lens])
    sequence_ids = position_ids.eq(0).cumsum(0) - 1
    return torch.stack([sequence_ids, position_ids], 1).to(cu_seqlens.device, dtype=torch.int32)


def contiguous(fn):
    # Local replacement for fla.utils.contiguous (input_guard): force tensor args
    # contiguous. The device-context branch is dropped since PTPU has no autograd ctx.
    @functools.wraps(fn)
    def wrapper(*args, **kwargs):
        args = tuple(i if not isinstance(i, torch.Tensor) else i.contiguous() for i in args)
        kwargs = {k: (v if not isinstance(v, torch.Tensor) else v.contiguous()) for k, v in kwargs.items()}
...
def autocast_custom_fwd(fn):
    # Local no-op replacement for fla.utils.autocast_custom_fwd (no autocast region in
    # the PTPU test, so the fp16 cast is a no-op).
    return fn
...
def tilelang_kernel_fwd(
    ...
    with T.Kernel(seq_len, NV, batch * head_kv, threads=threads) as (bx, by, bz):
        ...
        acc_s_cast = T.alloc_shared([G, BS], dtype, scope="shared")
...
def tilelang_kernel_bwd_dkv(
    ...
    with T.Kernel(NV, NS, B * H, threads=num_threads) as (i_v, i_s, i_bh):
        ...
        qkT_cast = T.alloc_shared([BS, G], dtype, scope="shared")
        dsT = T.alloc_fragment([BS, G], accum_dtype)
        dsT_cast = T.alloc_shared([BS, G], dtype, scope="shared")
...
def parallel_nsa_bwd(
    ...
    BS = block_size
    WS = window_size
    BK = tilelang.math.next_power_of_2(K)
    BV = min(128, tilelang.math.next_power_of_2(v.shape[-1]))
    NV = tilelang.cdiv(V, BV)

    assert window_size == 0, "Window size is not supported yet"
    delta_slc = tilelang_kernel_preprocess(o_slc, do_slc)

    dq = torch.zeros(NV, *q.shape, dtype=torch.float, device=q.device)
...
if __name__ == "__main__":
    ...
    # --- CPU inputs (reference naive_nsa relies on einsum/softmax, not reliable on PTPU) ---
    q_cpu = torch.randn((B, SEQ_LEN_Q, HQ, D), dtype=dtype).requires_grad_(True)
    k_cpu = torch.randn((B, SEQ_LEN_Q, H, D), dtype=dtype).requires_grad_(True)
    v_cpu = torch.randn((B, SEQ_LEN_Q, H, D), dtype=dtype).requires_grad_(True)
    g_slc_cpu = torch.ones((B, SEQ_LEN_Q, HQ), dtype=dtype).requires_grad_(True)
    g_swa_cpu = torch.ones((B, SEQ_LEN_Q, HQ), dtype=dtype).requires_grad_(True)
    do_cpu = torch.randn((B, SEQ_LEN_Q, HQ, D), dtype=dtype)

    block_indices_cpu = torch.full((B, SEQ_LEN_Q, H, S), SEQ_LEN_Q, dtype=torch.long)
    for b in range(B):
        for t in range(SEQ_LEN_Q):
            for h in range(H):
                i_i = torch.randperm(max(1, (t // block_size)))[:S]
                block_indices_cpu[b, t, h, : len(i_i)] = i_i
    block_indices_cpu = block_indices_cpu.sort(-1)[0]
    block_counts_cpu = torch.randint(1, S + 1, (B, SEQ_LEN_Q, H))
    ...
    ref = naive_nsa(
        ...
        block_counts=block_counts_cpu,
        block_size=block_size,
    )
    ref.backward(do_cpu)
    ref_dq = q_cpu.grad.clone()
    ref_dk = k_cpu.grad.clone()
    ref_dv = v_cpu.grad.clone()
    ref_dg_slc = g_slc_cpu.grad.clone()

    # --- PTPU inputs (same values) ---
    device = "ptpu"
    q = q_cpu.detach().to(device).requires_grad_(True)
    k = k_cpu.detach().to(device).requires_grad_(True)
    v = v_cpu.detach().to(device).requires_grad_(True)
    g_slc = g_slc_cpu.detach().to(device).requires_grad_(True)
    g_swa = g_swa_cpu.detach().to(device).requires_grad_(True)
    do = do_cpu.to(device)
    block_indices = block_indices_cpu.to(device)
    block_counts = block_counts_cpu.to(device)
    ...
    tri.backward(do)
    tri_dq = q.grad.clone().cpu()
    tri_dk = k.grad.clone().cpu()
    tri_dv = v.grad.clone().cpu()
    tri_dg_slc = g_slc.grad.clone().cpu()

    torch.testing.assert_close(ref, tri.cpu(), atol=1e-2, rtol=1e-2)
