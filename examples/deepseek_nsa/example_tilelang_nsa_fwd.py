...
def native_sparse_attention(batch, heads, seq_len, dim, is_causal, scale=None, block_size=64, groups=1, selected_blocks=16):
    ...
    def native_sparse_attention(
        ...
        with T.Kernel(seq_len, NV, batch * head_kv, threads=threads) as (bx, by, bz):
            Q_shared = T.alloc_shared([G, BK], dtype, scope="shared")
            K_shared = T.alloc_shared([BS, BK], dtype, scope="shared")
            V_shared = T.alloc_shared([BS, BV], dtype, scope="shared")
            O_shared = T.alloc_shared([G, BV], dtype, scope="shared")
...
def main():
    ...
    Q = torch.randn((B, SEQ_LEN, HQ, D), dtype=dtype).requires_grad_(True).ptpu()
    K = torch.randn((B, SEQ_LEN, H, D), dtype=dtype).requires_grad_(True).ptpu()
    V = torch.randn((B, SEQ_LEN, H, D), dtype=dtype).requires_grad_(True).ptpu()
    g_slc = torch.ones((B, SEQ_LEN, HQ), dtype=dtype, device="ptpu").requires_grad_(True)
    g_swa = torch.ones((B, SEQ_LEN, HQ), dtype=dtype, device="ptpu").requires_grad_(True)
    DO = torch.randn((B, SEQ_LEN, HQ, D), dtype=dtype).ptpu()

    block_indices = torch.full((B, SEQ_LEN, H, S), SEQ_LEN, dtype=torch.long, device="ptpu")
    block_counts = torch.zeros((B, SEQ_LEN, H), dtype=torch.long, device="ptpu")
    for b in range(B):
    ...
    block_indices = block_indices.cpu().sort(-1)[0].ptpu()
    ...
    # print("out", out)
    # print("ref", ref)
    torch.testing.assert_close(ref.clone().cpu(), out.clone().cpu(), atol=1e-2, rtol=1e-2)
