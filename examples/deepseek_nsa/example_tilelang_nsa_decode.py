...
def native_sparse_attention(
    ...
    def native_sparse_attention(
        ...
        with T.Kernel(1, NV, batch * head_kv, threads=threads) as (bx, by, bz):
            ...
            Q_shared = T.alloc_shared([G, BK], dtype, scope="shared")  # Current query block
            K_shared = T.alloc_shared([BS, BK], dtype, scope="shared")  # Current key block
            V_shared = T.alloc_shared([BS, BV], dtype, scope="shared")  # Current value block
            O_shared = T.alloc_shared([G, BV], dtype, scope="shared")  # Output accumulator

            # Attention computation buffers
            acc_s = T.alloc_fragment([G, BS], accum_dtype)  # QK^T scores
            acc_s_cast = T.alloc_shared([G, BS], dtype, scope="shared")  # Casted scores for softmax
...
def main():
    ...
    Q = torch.randn((B, SEQ_LEN_Q, HQ, D), dtype=dtype).requires_grad_(True).ptpu()
    ...
    mask = torch.randint(0, 2, (B, SEQ_LEN, groups)).ptpu()
    DO = torch.randn((B, SEQ_LEN_Q, HQ, D), dtype=dtype).ptpu()

    block_indices = torch.full((B, SEQ_LEN_Q, H, S), SEQ_LEN, dtype=torch.long, device="ptpu")
    for b in range(B):
    ...
    block_counts = torch.randint(1, S + 1, (B, SEQ_LEN_Q, H)).ptpu()
    ...
    torch.testing.assert_close(ref, out.cpu(), atol=1e-2, rtol=1e-2)
