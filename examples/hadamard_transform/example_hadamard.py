...
def ref_program(x: torch.Tensor):
    ...
    matrix = torch.tensor(scipy.linalg.hadamard(dim, dtype=float), dtype=x.dtype)
    return F.linear(x, matrix)


def run_hadamard(batch: int, dim: int, benchmark: bool = False):
    device = get_current_device()
    x_cpu = torch.randn((batch, dim), dtype=torch.float32)
    x = x_cpu.to(device)
    y = hadamard(x, dim, T.float32)
    y_ref = ref_program(x_cpu)
    torch.testing.assert_close(y.to("cpu"), y_ref, atol=1e-2, rtol=1e-2)

    if benchmark:
        from tilelang.profiler import do_bench

        latency = do_bench(lambda: hadamard(x, dim, T.float32))
...
def main():
    ...
    run_hadamard(batch=args.batch, dim=args.dim, benchmark=True)
