...
def get_heuristic_config() -> Tuple[Dict, int]:
    if is_ptpu_available():
        return dict(block_N=64, block_H=8, num_split=16, num_stages=1, threads=128), 0
...
def flashattn(batch, heads, groups, seqlen_kv, dim, block_N, block_H, num_split, num_stages, threads):
    ...
    def flashattn_gqa_decode_split(
        ...
        with T.Kernel(batch, heads // valid_block_H, num_split, threads=threads) as (bx, by, bz):
            ...
            for rep in T.serial(block_H // valid_block_H):
                T.copy(
                    Q[bid, hid * valid_block_H : (hid + 1) * valid_block_H, :],
                    Q_shared[rep * valid_block_H : (rep + 1) * valid_block_H, :],
                )
            T.fill(acc_o, 0)
            T.fill(logsum, 0)
            ...
            for k in T.Pipelined(loop_range, num_stages=num_stages):
                ...
                for i, j in T.Parallel(block_H, block_N):
                    acc_s[i, j] = T.if_then_else(
                        (mask_local[j] != 0) & (j < seqlen_kv // num_split) & (i < valid_block_H),
                        acc_s[i, j],
    ...
    def flashattn_gqa_decode_no_split(
        ...
        with T.Kernel(batch, heads // valid_block_H, num_split, threads=threads) as (bx, by, bz):
            ...
            for k in T.Pipelined(loop_range, num_stages=num_stages):
                ...
                for i, j in T.Parallel(block_H, block_N):
                    acc_s[i, j] = T.if_then_else(
                        (mask_local[j] != 0) & (i < valid_block_H),
                        acc_s[i, j],
                        -T.infinity(accum_dtype),
                    )
                T.copy(scores_max, scores_max_prev)
...
def flash_split_ref(Q, K, V, mask, num_split):
    batch = Q.size(0)
    nheads = Q.size(1)
    groups = K.size(2)
    dim = Q.size(-1)
    ...
    scale = (1.0 / dim) ** 0.5 * 1.44269504  # log2(e)
    acc_s = torch.empty((batch, num_head_groups, groups, block_N), dtype=torch.float)
    acc_s_cast = torch.empty((batch, num_head_groups, groups, block_N), dtype=torch.float16)
    acc_o = torch.empty((batch, num_head_groups, groups, dim), dtype=torch.float)
    scores_max = torch.empty((batch, num_head_groups, groups), dtype=torch.float)
    scores_max_prev = torch.empty((batch, num_head_groups, groups), dtype=torch.float)
    scores_scale = torch.empty((batch, num_head_groups, groups), dtype=torch.float)
    scores_sum = torch.empty((batch, num_head_groups, groups), dtype=torch.float)
    logsum = torch.empty((batch, num_head_groups, groups), dtype=torch.float)
    gacc_o = torch.empty((num_split, batch, nheads, dim), dtype=torch.float)
    glogsum = torch.empty((num_split, batch, nheads), dtype=torch.float)
...
def reduce_ref(Q, K, V, mask, glse, Output_partial, num_split):
...
def ref_split_program(Q, K, V, mask, num_split):
    glse, output_partial = flash_split_ref(Q, K, V, mask, num_split)
    return reduce_ref(Q, K, V, mask, glse, output_partial, num_split)
...
def main(
    ...
    if not tune:
        ...
        mask = torch.randint(0, 2, (batch, kv_seqlen, groups), dtype=torch.uint8)
        device = get_current_device()
        o = kernel(q.to(device), k.to(device), v.to(device), mask.to(device))
        o_ref = ref_program(q, k, v, mask)
        o_ref_split = ref_split_program(q, k, v, mask, config["num_split"])

        if is_ptpu_available():
            torch.ptpu.synchronize()
        o = o.cpu()

        assert_similar(o, o_ref, name="o_ref", assert_=True)
        assert_similar(o, o_ref_split, name="o_ref_split", assert_=True)
