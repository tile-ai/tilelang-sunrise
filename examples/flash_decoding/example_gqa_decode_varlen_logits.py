        ...
        with T.Kernel(batch, heads // valid_block_H, num_split, threads=threads) as (bid, hid, bz):
            ...
            for k in T.Pipelined(loop_range, num_stages=num_stages):
                ...
                for i, j in T.Parallel(block_H, block_N):
                    acc_s[i, j] = T.if_then_else(k * block_N + j < cur_seqlen_k, acc_s[i, j], T.cast(-10000.0, accum_dtype))
...
def test_varlen_decode_main(args):
    ...
    with torch.random.fork_rng(devices=cuda_devices):
        ...
        cu_seqlens_k[1:] = torch.cumsum(k_seqlens, dim=0).to(torch.int32).to(device)
        total_k_tokens = cu_seqlens_k[-1].item()

        # Generate input tensors
        q = torch.randn(batch_size, q_heads, head_size, device=device, dtype=dtype)
        k_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device=device, dtype=dtype)
        v_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device=device, dtype=dtype)
        sink = torch.randn(q_heads, device=device, dtype=torch.float32) * 0.1 if args.test_sink else None
    ...
    S_tl = torch.max_pool2d(S_tl.cpu(), kernel_size=(q_heads, 1), stride=(q_heads, 1)).to(device)
    ...
    k_padded = torch.zeros(batch_size, kv_heads, actual_max, head_size, device=device, dtype=dtype)
    v_padded = torch.zeros(batch_size, kv_heads, actual_max, head_size, device=device, dtype=dtype)
    for i in range(batch_size):
    ...
    # Compute reference on CPU — PTPU fast-math breaks softmax with -inf mask
    O_ref, attn_weights = ref_attention(
        q.cpu(),
        k_padded.cpu(),
        v_padded.cpu(),
        k_seqlens,
        q_heads,
        sink.cpu() if sink is not None else None,
    )
    O_ref = O_ref.to(device)
    S_ref = (
        torch.max_pool2d(attn_weights, kernel_size=(q_heads, block_size), stride=(q_heads, block_size), ceil_mode=True).to(dtype).to(device)
    )

    # Compare results
    num_blocks = math.ceil(actual_max / block_size)
    assert torch.allclose(O_tl.cpu(), O_ref.cpu(), atol=1e-2, rtol=1e-2), f"Output mismatch: {(O_tl.cpu() - O_ref.cpu()).abs().max()}"
    assert torch.allclose(S_tl[:, :, :num_blocks].cpu(), S_ref[:, :, :num_blocks].cpu(), atol=1e-2, rtol=1e-2), "Score mismatch"
...
def speed_benchmark_decode_comparison(args):
    ...
    # Generate tensors
    q_decode = torch.randn(batch_size, q_heads, head_size, device=device, dtype=dtype)
    k_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device=device, dtype=dtype)
    v_varlen = torch.randn(total_k_tokens, kv_heads, head_size, device=device, dtype=dtype)
    sink = torch.randn(q_heads, device=device, dtype=torch.float32) * 0.1 if args.test_sink else None
    if args.test_varlen:
