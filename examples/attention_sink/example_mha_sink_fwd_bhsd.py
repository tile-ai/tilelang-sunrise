...
def gen_inputs(B, H, Sq, Skv, D, dtype=torch.float16) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    query = torch.randn([B, H, Sq, D], dtype=dtype, device=device)
    key = torch.randn([B, H, Skv, D], dtype=dtype, device=device)
    value = torch.randn([B, H, Skv, D], dtype=dtype, device=device)
...
def main(
    ...
    if tune:
        ...
        out = kernel(Q, K, V, sinks)
        ref = ref_program(Q.cpu(), K.cpu(), V.cpu(), sinks.cpu(), window_size, dtype=torch_dtype)
        # Move ref back to device for comparison
        ref = ref.to(device=out.device)
        # Compare on CPU: readback needs an explicit sync on PTPU, otherwise the
        # kernel may still be in flight when the host reads the tensor.
