    ...
    def flash_fwd(
        ...
        with T.Kernel(T.ceildiv(seq_len, block_M), heads, batch, threads=threads) as (bx, by, bz):
            ...
            for k in T.Pipelined(start, end, num_stages=num_stages):
                ...
                for i in T.Parallel(block_M):
                    if window_size is not None:
                        scores_max[i] = T.if_then_else(scores_max[i] < T.cast(-10000.0, accum_dtype), T.cast(0, accum_dtype), scores_max[i])
...
def flashattn_bwd(batch, heads, seq_len, dim, groups, window_size=None, sm_scale=None, dtype=T.float16):  # None for full attention
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_M), batch, threads=threads) as (bx, by, bz):
            ...
            dsT_cast = T.alloc_shared([block_M, block_N], dtype)
            ...
            # NOTE: dK is emitted before dV on purpose.  On the STCU/TANG
            # backend, running two cross-block-contending atomic_add sequences
            # back-to-back in the same block (dV then dK) makes the *second*
            # sequence lose updates under GQA (multiple head blocks atomic into
            # the same kv-head dV/dK address).  Ordering dK first sidesteps the
            # hardware atomic-consistency bug; dv/dk are independent so the
            # reorder is semantically identical.
            T.copy(dk, dk_shared)
...
def main(
    ...
    Q = torch.randn(BATCH, H, N_CTX, D_HEAD, dtype=torch_dtype, device=get_current_device()).requires_grad_()
    K = torch.randn(BATCH, H // groups, N_CTX, D_HEAD, dtype=torch_dtype, device=get_current_device()).requires_grad_()
    V = torch.randn_like(K).requires_grad_()
    sinks = torch.randn(H, dtype=torch_dtype, device=get_current_device()).requires_grad_()
    ...
    Q_cpu = Q.detach().cpu().clone().requires_grad_(True)
    K_cpu = K.detach().cpu().clone().requires_grad_(True)
    V_cpu = V.detach().cpu().clone().requires_grad_(True)
    sinks_cpu = sinks.detach().cpu().clone().requires_grad_(True)
    dO_cpu = dO.detach().cpu().clone()
    O_ref = ref_program(Q_cpu, K_cpu, V_cpu, sinks_cpu, window_size, dtype=torch_dtype)
    O_ref.backward(dO_cpu, retain_graph=True)
    dQ_ref, dK_ref, dV_ref = Q_cpu.grad, K_cpu.grad, V_cpu.grad
    dsinks_ref = sinks_cpu.grad
    ...
    assert torch.allclose(O.cpu(), O_ref.cpu(), rtol=rtol, atol=atol), f"O max err: {(O.cpu() - O_ref.cpu()).abs().max()}"
    assert torch.allclose(dV.cpu(), dV_ref.cpu(), rtol=rtol, atol=atol), f"dV max err: {(dV.cpu() - dV_ref.cpu()).abs().max()}"
    assert torch.allclose(dK.cpu(), dK_ref.cpu(), rtol=rtol, atol=atol), f"dK max err: {(dK.cpu() - dK_ref.cpu()).abs().max()}"
    assert torch.allclose(dQ.cpu(), dQ_ref.cpu(), rtol=rtol, atol=atol), f"dq max err: {(dQ.cpu() - dQ_ref.cpu()).abs().max()}"
    assert torch.allclose(dsinks.cpu(), dsinks_ref.cpu(), rtol=rtol, atol=atol), (
        f"dsinks max err: {(dsinks.cpu() - dsinks_ref.cpu()).abs().max()}"
    )
...
def run_regression_perf(
    ...
    with torch.no_grad():
        Q = torch.randn(BATCH, H, N_CTX, D_HEAD, dtype=torch_dtype, device=get_current_device())
        K = torch.randn(BATCH, H // groups, N_CTX, D_HEAD, dtype=torch_dtype, device=get_current_device())
        V = torch.randn_like(K)
        sinks = torch.randn(H, dtype=torch_dtype, device=get_current_device())
        dO = torch.randn_like(Q)
        ...
        head_kv = H // groups
        kv_shape = (BATCH, head_kv, N_CTX, D_HEAD)
        dq = torch.zeros(q_shape, dtype=torch.float32, device=get_current_device())
        dk = torch.zeros(kv_shape, dtype=torch.float32, device=get_current_device())
        dv = torch.zeros(kv_shape, dtype=torch.float32, device=get_current_device())
