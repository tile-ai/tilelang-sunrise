...
def main(
    ...
    kernel = flashattn_sink(
        batch, groups, UQ, UKV, heads, dim, is_causal, window_size=window_size, block_M=64, block_N=64, num_stages=1, threads=128
    )

    out_unpad = kernel(q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, sinks)

    # Reference implementation on CPU: torch's exp/einsum produce NaN on the
    # ptpu backend for varlen shapes where a query precedes all its keys.
    ...
    torch.testing.assert_close(out_unpad.cpu(), ref_out_unpad.cpu(), rtol=1e-2, atol=1e-2)
