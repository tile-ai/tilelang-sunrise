...
def flashattn_fwd(
    ...
    def flash_fwd(
        ...
        with T.Kernel(T.ceildiv(seq_len, block_M), heads, batch, threads=threads) as (bx, by, bz):
            ...
            T.fill(acc_o, 0)
            T.fill(logsum, 0)
            T.fill(scores_max, T.cast(-10000.0, accum_dtype))
            ...
            for k in T.Pipelined(start, end, num_stages=num_stages):
                ...
                for i, j in T.Parallel(block_M, block_N):
                    ...
                    k_idx = k * block_N + j
                    if window_size is not None:
                        acc_s[i, j] = T.if_then_else(q_idx >= k_idx and q_idx < k_idx + window_size, 0, T.cast(-10000.0, acc_s.dtype))
                    else:
                        acc_s[i, j] = T.if_then_else(q_idx >= k_idx, 0, T.cast(-10000.0, acc_s.dtype))
...
def main(BATCH: int = 1, H: int = 1, N_CTX: int = 512, D_HEAD: int = 128, window_size: Optional[int] = None, dtype: T.dtype = T.float16):
    ...
    # Run reference on CPU — PTPU fast-math breaks softmax with -inf masks
