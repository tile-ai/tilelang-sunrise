...
def flashattn_bwd(
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(max_seq_len, block_M), batch_size, threads=threads) as (bx, by, bz):
            ...
            # NOTE: dK is emitted before dV on purpose (see example_gqa_sink_bwd_bhsd.py
            # for rationale): two back-to-back cross-block-contending atomic_add
            # sequences lose updates in the second one on the STCU/TANG backend.
            T.copy(dk, dk_shared)
...
def main(
    ...
    q_unpad_cpu = q_unpad.detach().cpu().clone().requires_grad_(True)
    k_unpad_cpu = k_unpad.detach().cpu().clone().requires_grad_(True)
    v_unpad_cpu = v_unpad.detach().cpu().clone().requires_grad_(True)
    sinks_cpu = sinks.detach().cpu().clone().requires_grad_(True)
    dO_unpad_cpu = dO_unpad.detach().cpu().clone()
    ...
    O_ref_unpad.backward(dO_unpad_cpu, retain_graph=True)
    dQ_ref = q_unpad_cpu.grad
    dK_ref = k_unpad_cpu.grad
    dV_ref = v_unpad_cpu.grad
    ...
    assert torch.allclose(O_unpad.cpu(), O_ref_unpad.cpu(), rtol=rtol, atol=atol), (
        f"O max err: {(O_unpad.cpu() - O_ref_unpad.cpu()).abs().max()}"
    )
